...
void CSfxTroStretching01::Process()
{
	...
	if(m_pSfxObj->Process())
	{
		...
		if( pObjSrc )
		{
			...
			CSfx *pSfx = CreateSfx( g_Neuz.m_pd3dDevice, pSkillProp->dwSfxObj2, v, pObjSrc->GetId(), D3DXVECTOR3( 0.0f, 0.0f, 0.0f ), NULL_ID, -1 );	// 머리위에 sfx생성.
		}
...
void CSfxItemYoyoAtk::MakePath(int nType)
{
	...
	for( int i = 0; i < nMaxVertex-1; i ++ )
	{
		...
		for( int j = 0; j < MAX_SF_SLERP+1; j ++ )
		{
			if( m_nMaxSpline >= 30 )
			{
				FLERROR_LOG( PROGRAM_NAME, _T( "CSfxItemYoyoAtk Spline : 범위 초과 %d" ), m_nMaxSpline );
				break;
			}
...
{
	m_SfxObj2.SetSfx(XI_INDEX( 1736, XI_SKILL_MAG_WIND_STRONGWIND01_01));		// 맞았을때 효과.
	m_bHit = FALSE;
	m_fAngle = 0;
}
CSfxSkillMagStrongWind::~CSfxSkillMagStrongWind()
...
{
	m_SfxObj2.SetSfx(XI_INDEX( 1739, XI_SKILL_MAG_FIRE_HOTAIR01_01 ));		// 타겟측에서 펑 터지며 회전하는 효과
	m_nDmgCnt = 0;
}
CSfxSkillMagHotAir::~CSfxSkillMagHotAir()
{
...
//sun, 11, 확율스킬 효과수정 world,neuz
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "스킬(%d)의 프로퍼티가 없다." ), m_dwSkill );
		Delete();
		return;
	}
	...
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "스킬(%d)의 애드 프로퍼티가 없다." ), m_dwSkill );
		Delete();
		return;
	}
...
//	pModel->GetEventPos( &vPos, 0 );	// 등드리 좌표
	...
	D3DXQuaternionRotationMatrix( &qRst, &mPartMatrix );			// 왼팔 매트릭스를 쿼터니언으로 바꿈.
