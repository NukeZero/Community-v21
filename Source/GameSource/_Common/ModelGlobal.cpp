...
extern CPartsFireDragon	g_FireDragon;		//sun: 메테오니커 파이어 발사!!!
...
HRESULT CModelGlobal::InvalidateDeviceObjects( LPDIRECT3DDEVICE9 pd3dDevice )
{
	...
	g_FireDragon.InvalidateDeviceObjects( pd3dDevice );		//sun: 메테오니커 파이어 발사!!!
...
#ifdef __CLIENT
CPartsLaser		g_Laser;
CPartsFireDragon	g_FireDragon;	//sun: 메테오니커 파이어 발사!!!
#endif
...
HRESULT CModelGlobal::RestoreDeviceObjects( LPDIRECT3DDEVICE9 pd3dDevice )
{
	...
	g_FireDragon.RestoreDeviceObjects( pd3dDevice );		//sun: 메테오니커 파이어 발사!!!
#endif // Client
	return S_OK;
}
...
{  
	...
	if( hr == E_OUTOFMEMORY )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "쉐도우 맵 생성중 메모리 부족 %d, %d" ), g_Option.m_nShadow, g_Option.m_nTextureQuality );
		DeleteShadowMap( pd3dDevice );
		return FALSE;
	}
	if ( hr == D3DERR_OUTOFVIDEOMEMORY )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "쉐도우 맵 생성중 비디오 메모리 부족 %d, %d" ), g_Option.m_nShadow, g_Option.m_nTextureQuality );
		DeleteShadowMap( pd3dDevice );
		return FALSE;
	}
	if( FAILED( hr ) )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "쉐도우 맵 생성 에러 %s, %d, %d" ), DXGetErrorString9(hr), g_Option.m_nShadow, g_Option.m_nTextureQuality );
		DeleteShadowMap( pd3dDevice );
		return FALSE;
	}
	if( g_pShadowTexture )
...
void	CPartsFire::Create( LPDIRECT3DDEVICE9 pd3dDevice, const D3DXVECTOR3 vPos, DWORD dwSfx, const D3DXVECTOR3 &vScale )
{
	...
	for( i = 0; i < MAX_PARTS_FIRE; i ++ )
	{
		...
		m_pList[i]->m_vScale = vScale;	//sun: 제련 이팩트 향상 버전
...
//	m_pSfx->m_vScale = vScale;	//sun: 제련 이팩트 향상 버전
//}
...
void	CPartsWeaponEffect::Render( LPDIRECT3DDEVICE9 pd3dDevice, const D3DXVECTOR3& kPos, const D3DXMATRIX *mWorld )
{
	...
	if( m_pSfx->Process() )	// 1play 끝남.
		m_pSfx->m_nCurFrame = 0;

	//이미 변환된 World position이므로 곧바로 대입
	m_pSfx->m_vPos = kPos;

	//너클만 반대 -_-; 원본을 못바꺼주겠단다.
	D3DXMATRIX mat;
	...
	D3DXQUATERNION qRst;
	D3DXVECTOR3 vYPW;
	D3DXQuaternionRotationMatrix( &qRst, &mat );			// 매트릭스를 쿼터니언으로 바꿈.
...
void CPartsFireDragon::Process( LPDIRECT3DDEVICE9 pd3dDevice )
{
	...
	if( FAILED(hr) )
		FLERROR_LOG( PROGRAM_NAME, _T( "CreateShadowMask : 버텍스 버퍼 생성 실패" ) );
...
HRESULT CPartsFireDragon::RestoreDeviceObjects( LPDIRECT3DDEVICE9 pd3dDevice )
{
	...
	if( hr == E_FAIL )
	{
		FLERROR_LOG( PROGRAM_NAME, _T( "%s 읽기 실패" ), MakePath( DIR_SFXTEX, szFileName ) );
	}
	return S_OK;
}
...
//		LPDIRECT3DVERTEXBUFFER9		m_pd3dVB;
		...
		if( FAILED(hr) )
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "락 실패" ) );
			return E_FAIL;
		}
