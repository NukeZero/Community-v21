...
//#define ID_Physique_BLENDED_LIST "*PHYSIQUE_BLENDED_LIST"
...
//	static char buff[256];
	...
	// Node name
	fprintf(pStream,"%s\t%s \"%s\"\n", indent.data(), ID_NODE_NAME, FixupName(node->GetName()));
//	sprintf( buff, "%s %d", FixupName(node->GetName()), node->IsNodeHidden() );
//	MessageBox( NULL,buff, "메시지", MB_OK );
/*
	int b = node->IsNodeHidden();
	if( b & PART_HIDESTATE )
	{
		int a;
		a++;
