...
extern BOOL	g_bWarning;		// 전역변수가 클리어 되지 않는다. 시작할때 클리어 해주자
extern BOOL	g_bWarning2;	// Identity검사 에러 
...
//	theResetXForm.BeginEditParams( ip, NULL );		// 03.04.25 추가....
//	theResetXForm.ResetSel();

	m_nMaxUnderline = 0;
	TCHAR *szMaxName = FixupName(ip->GetCurFileName());
	TCHAR a;
	while(1)		// 이름내 _갯수를 셈
	{
		a = *szMaxName++;
		if( a == 0 )	break;
		if( a == '_' )	m_nMaxUnderline ++;
	}
	
	g_bWarning = g_bWarning2 = FALSE;
...
//	sprintf(szFmtStr, "%%4.%df", nPrecision);
	...
	//Export Physique and Skin data
	nCurNode = 0;
	for (idx=0; idx<numChildren; idx++) {
		if (ip->GetCancel())
			break;
		ExportSkeletalDeformer(ip->GetRootNode()->GetChildNode(idx), 0);
	}
...
{
//	INodeTransformed Node = INodeTransformed( node, node->GetObjectTM(GetStaticFrame()) );
//	node = &Node;
...
BOOL AsciiExp::ExportSkeletalDeformer(INode *node, int indentLevel)
{
	if(exportSelected && node->Selected() == FALSE)
		return TREE_CONTINUE;

	nCurNode++;
//	ip->ProgressUpdate((int)((float)nCurNode/nTotalNodeCount*100.0f), TRUE, GetString(IDS_EXPORT_SKIN)); 
...
/*		Modifier *mod = FindPhysiqueModifier(node);
		if ( mod )
		{
			ExportPhysiqueData(node, mod, indentLevel);
		}
		else 
*/
		{
			...
			if ( mod )
			{
				ExportSkinData(node, mod, indentLevel);
			}
		}
	}
	...
	// until no more children are found.
	for (int c = 0; c < node->NumberOfChildren(); c++) {
		if (!ExportSkeletalDeformer(node->GetChildNode(c), indentLevel))
			return FALSE;
	}
	return TRUE;	
}
