...
static BOOL SelectedAncestor(INode *node)
	{
//	if (!node->GetParentNode()) return FALSE;				// 부모가 없으면 실행
	if ( node->GetParentNode()->IsRootNode() ) return FALSE;	// 부모가 없으면 실행
	//	if (node->GetParentNode()->Selected()) return TRUE;		// 부모가 선택되어 있다면 스킵
//	else return SelectedAncestor(node->GetParentNode());
	return TRUE;
	}

BOOL	ResetXForm::ResetNodeEnum( INode* node )
{

	if ( !(node->IsGroupMember() || node->IsGroupHead()) ) 
	{
		if (SelectedAncestor(node) == FALSE ) 
		{
...
//			ntm = ntm * piv;
	...
	{
		ResetNodeEnum(node->GetChildNode(c));
	}
...
void ResetXForm::ResetSel()
	...
	for (int idx=0; idx<numChildren; idx++) {
		ResetNodeEnum(ip->GetRootNode()->GetChildNode(idx));
	}
