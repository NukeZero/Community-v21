...
//extern const JOB g_job[ MAX_JOB ];
	...
	else
	{
		...
		for( int i = 0; i < nCount; i++ )
		{
			...
			if( m_aSlotApplet[nIndex].m_dwShortcut == SHORTCUT_CHAT)
				ar.ReadString( m_aSlotApplet[nIndex].m_szString );
		...
		for( i = 0; i < nCount; i++ )
		{
			...
			if( m_aSlotItem[nIndex][nIndex2].m_dwShortcut == SHORTCUT_CHAT )
				ar.ReadString( m_aSlotItem[nIndex][nIndex2].m_szString );
...
void CWndTaskBar::SetTaskBarTexture( LPSHORTCUT pShortcut )
{
	...
	else if ( pShortcut->m_dwShortcut == SHORTCUT_SKILL)
	{
		...
		{	
			...
			pShortcut->m_pTexture = m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ICON, lpSkill->GetProp()->szIcon/*pItemBase->GetProp()->szIcon*/), 0xffff00ff );
		}
	}
...
#else //__YESCAPE_MOTION
		string.Format( "#cff0000ff%s#nc\n%s",  GETTEXT( TID_GAME_MOTION ),  pMotionProp->szDesc );
			...
			if( lpShortcut->m_dwShortcut == SHORTCUT_ITEM )
			{
				...
				if( pItemElem )
				{
					...
					if( dwGroup )
					{
						...
						if( dwEnd != 0 && dwEnd > dwCur )  // raiders_fixme
						{
				...
				if( lpShortcut->m_dwId == MOT_BASE_ESCAPE )
				{
					...
					if( pItem && g_pPlayer )
					{
						...
						if( ct.GetTotalSeconds() )
						{
							point.y -= 1;
							point.x += 1;
							RenderRadar( m_p2DRender, point, pItem->dwCircleTime - ct.GetTotalSeconds(), pItem->dwCircleTime );
						}
					}
				}
			}
...
//#else
	//CRect rect( 5, 5, 754, 40 );
//#endif
	//m_wndMenu.Create( _T(""), 0, rect, this, 100);//,m_pSprPack,9);
	//m_wndMenu.SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "ButtStart.bmp" ), TRUE );
	//m_wndMenu.FitTextureSize();
//#ifdef __NEWINTERFACE
	//rect.SetRect( 624, 17, 641, 31 );//GetClientRect();
	//m_wndQuickList.Create( _T(""), 0, rect, this, 101);//,m_pSprPack,9);
	//m_wndQuickList.SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "WndButtQuickList.bmp" ), TRUE );
	//m_wndQuickList.FitTextureSize();
/*
	rect.SetRect( 624, 9, 624 + 16, 9 + 8 );
	m_wndQuickListUp.Create( _T(""), 0, rect, this, 102);//,m_pSprPack,9);
	m_wndQuickListUp.SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "WndButtQuickListUp.tga" ), TRUE );
	m_wndQuickListUp.FitTextureSize();

	rect.SetRect( 624, 33, 624 + 16, 33 + 8 );
	m_wndQuickListDn.Create( _T(""), 0, rect, this, 103);//,m_pSprPack,9);
	m_wndQuickListDn.SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "WndButtQuickListDn.bmp" ), TRUE );
	m_wndQuickListDn.FitTextureSize();
*/
//#endif
	//wndMenu.AddWndStyle( WBS_NODRAWFRAME );
	// Create Command button
	/*
	for(int i = 0; i < 10; i++)
		m_pWndCommand[i].Create(" ",0,CRect(60+i*18,3,76+i*18,21),this,i);
	m_pWndCommand[0].m_strTitle = "W";
	m_pWndCommand[1].m_strTitle = "R";
	m_pWndCommand[2].m_strTitle = "C";
*/
	/*
	int i;
	// Create Tray button
	int nMax = 3;
	int nStart = (rect.Width() - nMax * 18);
	for(i = 0; i < nMax; i++)
		m_pWndTray[i].Create(  _T(" ") ,0,CRect(nStart + i * 18, 3, nStart + 16 + i * 18, 21),this,i+100);
	m_pWndTray[0].m_strTitle =  _T("W");
	m_pWndTray[2].SetWndExecute(new CWndMessanger);
*/
	// shortcut 등록 

	//SetShortcut( m_awndShortCut.GetSize(), SHORTCUT_APPLET, 0, 0, APP_COMMUNICATION_CHAT );

	g_WndMng.m_pWndTaskBar = this;

#ifndef __ONLINE
	SetShortcut( 0, SHORTCUT_APPLET, 0,  APP_CHARACTER );
	SetShortcut( 1, SHORTCUT_APPLET, 0,  APP_INVENTORY );

	SetShortcut( 2, SHORTCUT_APPLET, 0,  APP_SKILL );
	SetShortcut( 3, SHORTCUT_APPLET, 0,  APP_HELPER_HELP );
	SetShortcut( 4, SHORTCUT_APPLET, 0,  APP_HELPER_FAQ );
	
	SetShortcut( 0, SHORTCUT_MOTION, 0, MOT_BASE_SIT, NULL, 1 );
	SetShortcut( 1, SHORTCUT_MOTION, 0, MOT_BASE_PICKUP, NULL, 1 );
	
#endif	// __ONLINE

	//SetShortcut( 0, SHORTCUT_APPLET,0, 0,  APP_NAVIGATOR );
	//SetShortcut( 0, SHORTCUT_APPLET,0,  0, APP_GAUGE );
	//SetShortcut( 3, SHORTCUT_APPLET,0,  0, APP_STYLE );
	//SetShortcut( 4, SHORTCUT_APPLET, 0, 0, APP_EMOTION );
	//SetShortcut( 0, SHORTCUT_APPLET,0, 0, APP_TRADE );
	//SetShortcut( 0, SHORTCUT_APPLET,0, 0,  APP_QUIT );
	...
	//SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, _T( "wndTaskBar.bmp" ) ), TRUE );
...
BOOL CWndTaskBar::OnChildNotify(UINT message,UINT nID,LRESULT* pLResult)
{
	...
	switch( nID )
	{
		...
		case WIDC_UP:
			m_nSlotIndex--;
			if( m_nSlotIndex < 0 ) m_nSlotIndex = 3;
			m_paSlotItem = m_aSlotItem[ m_nSlotIndex ];
			break;
		case WIDC_DOWN:
			m_nSlotIndex++;
			if( m_nSlotIndex > 3 ) m_nSlotIndex = 0;
			m_paSlotItem = m_aSlotItem[ m_nSlotIndex ];
			break;
...
void CWndTaskBar::OnRButtonUp( UINT nFlags, CPoint point )
{
	...
	if( rect.PtInRect( point ) )
	{
		...
		{
			/*
			CRect rectShortcut = m_menuShortcut.GetWindowRect();
			CPoint ptShortcut = point;
			ptShortcut.x *= ICON_SIZE;
			ptShortcut.y = rect.top;
			ptShortcut.x += rect.left;
			ptShortcut.y -= rectShortcut.Height();
			ClientToScreen( &ptShortcut );
			m_menuShortcut.Move( ptShortcut );
			m_menuShortcut.SetVisible( TRUE );
			m_menuShortcut.SetFocus();
			*/
#ifdef __MAINSERVER
...
#ifdef __ONLINE	
			// 아이템 삭제 서버로 전송
			g_DPlay.SendRemoveAppletTaskBar( point.x );
#endif
		}
	}
	rect = RECT_ITEM;
...
#ifdef __ONLINE	
			g_DPlay.SendRemoveItemTaskBar( m_nSlotIndex, point.x );
			// 아이템 삭제 서버로 전송
#endif
		}
...
BOOL CWndTaskBar::SetSkillQueue( int nIndex, DWORD dwType, DWORD dwId, CTexture* pTexture )
{
	...
	LPSHORTCUT pShortcut = NULL;
	CRect rect = GetWindowRect();
	DWORD dwSkill = g_pPlayer->GetSkill( 0, dwId )->dwSkill;
...
BOOL CWndTaskBar::OnDropIcon( LPSHORTCUT pShortcut, CPoint point )
{
	...
	if( pShortcut->m_pFromWnd == this )
	{
		...
		if( GetClientRect().PtInRect( point ) == FALSE )
		{
			...
			if( lpShortcut->m_dwData == 0 )
				g_DPlay.SendRemoveAppletTaskBar( lpShortcut->m_dwIndex );
			else
			if( lpShortcut->m_dwData == 1 )
				g_DPlay.SendRemoveItemTaskBar( m_nSlotIndex, lpShortcut->m_dwIndex );
			else
	...
	if( pWndFrame && ( pWndFrame->GetWndId() != APP_INVENTORY && pWndFrame != g_WndMng.m_pWndTaskBar && pWndFrame->GetWndId() != APP_COMMUNICATION_CHAT && pWndFrame->GetWndId() != APP_SKILL && pWndFrame->GetWndId() != APP_MOTION ) )
	{
		SetForbid( TRUE );
		return FALSE;
	}
...
#else
			OnEndSkill();	// 스킬바 사용에 실패했다면 다음으로 넘어가게 하자.
#endif
		} else
		{	// success
...
//			OnCancelSkill();													// 스킬사용 해제
	...
	else
	{
		...
		else
		{
			...
			{
				DWORD dwHotkey[10] = { 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79 };
				
				for( int i = 0; i < 10; i++ )
				{
					if( g_bKeyTable[ dwHotkey[i] ] )
					{
						LPSHORTCUT lpShortcut = &m_paSlotItem[ i ] ;
...
#ifdef __ONLINE	
#ifndef __XSYNC0711
	if( m_nExecute == 1 )	// 스킬바 실행전인데 첫스킬부터 실패 떨어짐.
	{
		OnCancelSkill();	// 스킬바 사용 취소
	} else
	if( m_nExecute == 2 )	// 스킬바 실행중이었다면.
	{
		m_nUsedSkillQueue ++;	// 다음 스킬실행으로 넘어감.
		...
		g_DPlay.SendActionPoint( nAP );

		LPSHORTCUT pShortcut = &m_aSlotQueue[ m_nUsedSkillQueue ];
		// 스킬바 슬롯이 끝났다면 종료 / 액션포인트가 부족하다면 다음슬롯 진행하지 않음.
//#ifdef __DEV
//		BOOL bResult = g_pPlayer->IsAuthHigher( AUTH_GAMEMASTER ) == TRUE ? FALSE : nAP < 0;
//#else
		BOOL bResult = nAP < 0;
//#endif
		if( ( m_nUsedSkillQueue >= MAX_SLOT_QUEUE) || pShortcut->IsEmpty() || bResult )	
		{
			OnCancelSkill();
		} 
		else
		// 다음 스킬 실행.
		{
			if( nAP < 0 )	nAP = 0;
			m_nActionPoint = nAP;
			TRACE( "OnEndSkill-%d ", pShortcut->m_dwId );
			if( g_pPlayer->CMD_SetUseSkill( m_idTarget, pShortcut->m_dwId, SUT_QUEUEING ) == 0 )		// 실행할 명령을 셋팅. 이동 + 스킬사용이 합쳐진 명령.
			{
				OnEndSkill();	// 스킬사용에 실패했다면 다음 스킬 사용하도록 넘어감.
			}
		}
	}
#endif // not xSync0711
...
{
//pWndButton = AppendMenu( 0, APP_EQUIPMENT , GETTEXT( TID_APP_EQUIPMENT     ) ); pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
//pWndButton = AppendMenu( 0, APP_JOB       , GETTEXT( TID_APP_JOB           ) ); pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
//pWndButton = AppendMenu( 0, APP_ADMIN     , GETTEXT( TID_APP_ADMIN         ) ); pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
...
//#ifndef __MASTER
	...
	{
		pWndButton = AppendMenu( this, 0, APP_DEBUGINFO     , GETTEXT( TID_APP_DEBUGINFO     ) ); //pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	}
//#endif
//	pWndButton = AppendMenu( this, 0, APP_WORLD     , GETTEXT( TID_APP_WORLD         ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	pWndButton = AppendMenu( this, 0, APP_STATUS    , GETTEXT( TID_APP_STATUS        ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	pWndButton = AppendMenu( this, 0, APP_NAVIGATOR , GETTEXT( TID_APP_NAVIGATOR     ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	pWndButton = AppendMenu( this, 0, APP_CHARACTER , GETTEXT( TID_APP_CHARACTER     ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );

	pWndButton = AppendMenu( this, 0, APP_INVENTORY , GETTEXT( TID_APP_INVENTORY     ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );

	pWndButton = AppendMenu( this, 0, APP_VENDOREX , GETTEXT( TID_APP_VENDOR     ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );

	pWndButton = AppendMenu( this, 0, APP_SKILL     , GETTEXT( TID_APP_SKILL         ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );

	pWndButton = AppendMenu( this, 0, APP_QUEST     , GETTEXT( TID_APP_QUEST         ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	/*
	if( g_eLocal.GetState( EVE_18 ) == 1 )
		pWndButton = AppendMenu( this, 0, APP_PVP		, GETTEXT( TID_APP_PVP     ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	*/
	
//	pWndButton = AppendMenu( this, 0, APP_COMMUNICATION_CHAT    , GETTEXT( TID_APP_COMMUNICATION_CHAT    ) ); pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );

	pWndButton = AppendMenu( this, 0, APP_MOTION   , GETTEXT( TID_APP_MOTION       ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	//pWndButton = AppendMenu( 0, APP_MACRO     , GETTEXT( TID_APP_MACRO         ) ); pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	//pWndButton = AppendMenu( 0, APP_STYLE     , GETTEXT( TID_APP_STYLE         ) ); pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	// 10번 - 아래는 추가 매뉴 설정 
	if( g_pPlayer->IsAuthHigher( AUTH_GAMEMASTER ) )
	{
		// 항목 만들기
		CWndButton* pWndButton0 = AppendMenu( this, 0, 0 , "Operation" ); pWndButton0->SetTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ICON, _T( "icon_Folder.dds" ) ) );
		// 메뉴 만들기 
		CWndMenu* pMenu0 = new CWndMenu;
		pMenu0->CreateMenu( this );
		pWndButton = AppendMenu( pMenu0, 0, APP_APPLET        , GETTEXT( TID_APP_APPLET        ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
		if( g_pPlayer->IsAuthHigher( AUTH_ADMINISTRATOR ) )
			pWndButton = AppendMenu( pMenu0, 0, APP_ADMIN_BALANCE , "Balance" ); //pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
		// 메뉴에 추가하기
		pWndButton0->SetMenu( pMenu0 );
	}
#ifdef __YCOMM_TASK_MENU
	...
	CWndButton* pWndButton4 = AppendMenu( this, 0, 0 , GETTEXT( TID_APP_OPTION        ) ); pWndButton4->SetTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ICON, _T( "icon_Folder.dds" ) ) );
	...
	//CWndButton* pWndButton7 = AppendMenu( this, 0, 0 , GETTEXT( TID_APP_EXTRA         ) ); pWndButton7->SetTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ICON, _T( "icon_Folder.dds" ) ) ); // 옵션 

#ifdef __N_OPTIONUNITY_041209
	CWndButton* pWndButton4 = AppendMenu( this, 0, APP_OPTIONEX, GETTEXT(TID_APP_OPTION) );
#endif//__N_OPTIONUNITY_041209

	pWndButton = AppendMenu( this, 0, APP_LOGOUT    , GETTEXT( TID_APP_LOGOUT      ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	pWndButton = AppendMenu( this, 0, APP_QUIT      , GETTEXT( TID_APP_QUIT          ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	...
	for(int i = 0; i < m_awndMenuItem.GetSize(); i++)
	{
		...
		//pWndButton->Move( CPoint( 2, 50 + 2 + ( i * 22 ) ) );
		pWndButton->SetWndRect( CRect(  10, 50 + ( i * 22 ), m_pTexture->m_size.cx - 20, 50 + 20 + ( i * 22 ) ) );
	}
	//SetWndRect( CRect( 0, 0, 150, 0 + 5 + 5 + ( ( i + 1 ) * 22 ) ) );
	//SetWndRect( CRect( 0, 0, 180, 50 + 5 + 5 + ( i * 22 ) ) );
	...
	CWndMenu* pMenu1 = new CWndMenu;
	pMenu1->CreateMenu( this );
	pWndButton = AppendMenu( pMenu1, 0, APP_COMMUNICATION_CHAT , GETTEXT( TID_APP_COMMUNICATION_CHAT    ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	pWndButton = AppendMenu( pMenu1, 0, APP_MESSENGER_         , GETTEXT( TID_APP_MESSENGER ) ); pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );

	pWndButton1->SetMenu( pMenu1 );

	CWndMenu* pMenu2 = new CWndMenu;
	pMenu2->CreateMenu( this );
	pWndButton = AppendMenu( pMenu2, 0, APP_PARTY      , GETTEXT( TID_APP_PARTY          ) ) ;//GETTEXT( TID_APP_PARTY  ) ); //pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	pWndButton = AppendMenu( pMenu2, 0, APP_GUILD      , GETTEXT( TID_APP_COMPANY         ) ) ;//GETTEXT( TID_APP_PARTY  ) ); //pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	
#ifdef __GUILDVOTE
	pWndButton = AppendMenu( pMenu2, 0, APP_GUILD_VOTE      , GETTEXT( TID_GAME_TOOLTIP_GUILDVOTE     ) ) ;//GETTEXT( TID_APP_PARTY  ) ); //pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
#endif
	
	pWndButton2->SetMenu( pMenu2 );

#ifndef __N_OPTIONUNITY_041209
	CWndMenu* pMenu4 = new CWndMenu;
	pMenu4->CreateMenu( this );
	pWndButton = AppendMenu( pMenu4, 0, APP_OPTION_VIDEO    , GETTEXT( TID_APP_OPTION_VIDEO    ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	pWndButton = AppendMenu( pMenu4, 0, APP_OPTION_ETC      , GETTEXT( TID_APP_OPTION_ETC      ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	pWndButton = AppendMenu( pMenu4, 0, APP_OPTION_GAME      , GETTEXT( TID_APP_OPTION_GAME      ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	pWndButton4->SetMenu( pMenu4 );
#endif //__N_OPTIONUNITY_041209

	CWndMenu* pMenu5 = new CWndMenu;
	pMenu5->CreateMenu( this );
	pWndButton = AppendMenu( pMenu5, 0, APP_INFO_NOTICE   , GETTEXT( TID_APP_INFO_NOTICE   ) ); //pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	pWndButton5->SetMenu( pMenu5 );

	CWndMenu* pMenu6 = new CWndMenu;
	pMenu6->CreateMenu( this );
	pWndButton = AppendMenu( pMenu6, 0, APP_HELPER_HELP , GETTEXT( TID_APP_HELPER_HELP ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	pWndButton = AppendMenu( pMenu6, 0, APP_HELPER_TIP  , GETTEXT( TID_APP_HELPER_TIP  ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	pWndButton = AppendMenu( pMenu6, 0, APP_HELPER_FAQ  , GETTEXT( TID_APP_HELPER_FAQ  ) );// pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	pWndButton6->SetMenu( pMenu6 );
	
#ifdef __YCOMM_TASK_MENU
	CWndMenu* pMenu7 = new CWndMenu;
	pMenu7->CreateMenu( this );
	pWndButton = AppendMenu( pMenu7, 0, APP_WEBBOX, GETTEXT( TID_APP_ITEMMALL ) ); pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	pWndButton = AppendMenu( pMenu7, 0, APP_COMM_ITEM , GETTEXT( TID_APP_ITEMTIME ) ); pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	pWndButton7->SetMenu( pMenu7 );
#endif //__YCOMM_TASK_MENU
...
//	Icon_CGMHelp.DDS
//	TID_APP_GMHELP
	pWndButton = AppendMenu( pMenu7, 0, APP_WEBBOX, GETTEXT( TID_APP_GMHELP ) ); pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
//	pWndButton = AppendMenu( pMenu7, 0, APP_COMM_ITEM , GETTEXT( TID_APP_ITEMTIME ) ); pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
	pWndButton7->SetMenu( pMenu7 );
#endif	// __WEB_GM_HELP1201

#ifdef __TAIWAN_WEBBOX
	CWndMenu* pMenu8 = new CWndMenu;
	pMenu8->CreateMenu( this );
	pWndButton = AppendMenu( pMenu8, 0, APP_WEBBOX2, GETTEXT( TID_APP_GMHELP ) ); // pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture( pWndButton );
...
// dlfakldgj;ladsjg;lkasdjtl;aj;sldfj;lkasj;jrwpoerjopsdjra;lfj;lscxnz; vmz

void CWndTaskBar::RenderCollTime(CPoint pt, DWORD dwSkillId, C2DRender* p2DRender )
{
	if( g_pPlayer->m_tmReUseDelay[ dwSkillId ] > 0 )
	{
		//p2DRender->TextOut( point.x, point.y, g_pPlayer->m_tmReUseDelay[ lpShortcut->m_dwId ] / 1000.0f );
#ifdef __YCOOLTIME_0214	
		LPSKILL lpSkill = &g_pPlayer->m_aJobSkill[ dwSkillId ];
		if( lpSkill )
			RenderRadar( p2DRender, pt, 
			             lpSkill->GetProp()->dwSkillReady - g_pPlayer->m_tmReUseDelay[ dwSkillId ], 
						 lpSkill->GetProp()->dwSkillReady );
/*
		CPoint cpt = pt;
		LPSKILL lpSkill;
		lpSkill = &g_pPlayer->m_aJobSkill[ dwSkillId ];
		...
		{
			BYTE bAlpha = 200;
			CPoint cpt_size = CPoint( 15, 15 );
			D3DCOLOR Color = D3DCOLOR_XRGB( 0, 0, 255 );
			
			int aaa = g_pPlayer->m_tmReUseDelay[ dwSkillId ];
			float fCoolTimePer = (float)aaa / (float)lpSkill->GetProp()->dwSkillReady;
			
			int nDrawIndex = 128 - (128 * fCoolTimePer);
			
			if( nDrawIndex >= 32 && nDrawIndex <= 63 )
			{
				p2DRender->RenderTextureEx( cpt, cpt_size, m_pCoolTexArry[31], bAlpha, 1.0f, 1.0f );
				cpt.y+=15;
			}
			else
			if( nDrawIndex >= 64 && nDrawIndex <= 95 )
			{
				p2DRender->RenderTextureEx( cpt, cpt_size, m_pCoolTexArry[31], bAlpha, 1.0f, 1.0f );
				cpt.y+=15;
				p2DRender->RenderTextureEx( cpt, cpt_size, m_pCoolTexArry[63], bAlpha, 1.0f, 1.0f );
				cpt.x-=15;
			}
			else
			if( nDrawIndex >= 96 && nDrawIndex <= 128 )
			{
				...
				p2DRender->RenderTextureEx( cpt, cpt_size, m_pCoolTexArry[63], bAlpha, 1.0f, 1.0f );
				cpt.x-=15;
				p2DRender->RenderTextureEx( cpt, cpt_size, m_pCoolTexArry[95], bAlpha, 1.0f, 1.0f );
				cpt.y-=15;
			}
			p2DRender->RenderTextureEx( cpt, cpt_size, m_pCoolTexArry[nDrawIndex], bAlpha, 1.0f, 1.0f );
		}					
*/		
