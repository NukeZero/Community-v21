...
BOOL TextCmd_ItemDropRate( CScanner& s )
{
#ifdef __WORLDSERVER
	CUser* pUser	= (CUser*)s.dwValue;
	FLOAT f	= 1.0f;
	BOOL bApplyAllMultiServer	= FALSE;
	...
	if( s.tok != FINISHED )
		bApplyAllMultiServer	= (BOOL)atoi( s.Token );
	...
	sprintf( lpOutputString, "TextCmd_ItemDropRate %f %d", f, (int)bApplyAllMultiServer );
	OutputDebugString( lpOutputString );

	if( bApplyAllMultiServer )
	{
...
BOOL TextCmd_GoldDropRate( CScanner& s )
{
#ifdef __WORLDSERVER
	CUser* pUser	= (CUser*)s.dwValue;
	...
	char lpOutputString[100]	= { 0, };
	sprintf( lpOutputString, "TextCmd_GoldDropRate %f %d", f, (int)bApplyAllMultiServer );
	OutputDebugString( lpOutputString );
...
BOOL TextCmd_MonsterExpRate( CScanner& s )
{
#ifdef __WORLDSERVER
	CUser* pUser	= (CUser*)s.dwValue;
	FLOAT f	= 1.0f;
	...
	char lpOutputString[100]	= { 0, };
	sprintf( lpOutputString, "TextCmd_MonsterExpRate %f %d", f, (int)bApplyAllMultiServer );
	OutputDebugString( lpOutputString );
...
BOOL TextCmd_ResistItem( CScanner& scanner )
{
	...
	pUser->UpdateItem( pItemElem0->m_dwObjId, UI_IR,  bItemResist );
	pUser->UpdateItem( pItemElem0->m_dwObjId, UI_RAO,  nResistAbilityOption );
	pUser->UpdateItem( pItemElem0->m_dwObjId, UI_AO,  nAbilityOption );
...
{ 
	...
	if( lstrlen( scanner.token ) > 260 )
		return TRUE;
	lstrcpy( lpString, scanner.token );
...
BOOL TextCmd_QueryMailBox( CScanner & s )
{
#ifdef __CLIENT
...
#ifdef __WORLDSERVER
	CUser* pUser	= (CUser*)scanner.dwValue;
	g_GuildCombatMng.GuildCombatRequest( pUser );
#endif // __WORLDSERVER
	return TRUE;
}
...
BOOL TextCmd_GuildCombatIn( CScanner& scanner )
{
	...
	g_GuildMng.m_AddRemoveLock.Leave( theLineFile );
//	g_GuildCombatMng.SetMaintenance();
#endif // __WORLDSERVER
	return TRUE;
}
...
//	ON_TEXTCMDFUNC( TextCmd_CreateCtrl		, _T( "createctrl"	)	,	_T( "ct"        ) , _T( "컨트롤생성"   )	,	_T( "컨생"	)	,	TCM_SERVER	,	AUTH_GAMEMASTER	,	"ctrl생성" )
	...
	ON_TEXTCMDFUNC( TextCmd_FallRain        , _T( "FallRain"       ) , _T( "frain"        ) , _T( "비와라"       ) , _T( "비와"	   		), TCM_SERVER   , AUTH_GAMEMASTER    ,  "비 내리기 토글"  )	
...
#ifdef __POST0906
	ON_TEXTCMDFUNC( TextCmd_PostMail	, _T( "PostMail"	) , _T( "pm"        ) , _T( "편지발송"         ) , _T( "발송"			    ), TCM_CLIENT   , AUTH_GAMEMASTER	     ,  "편지 발송" )
	ON_TEXTCMDFUNC( TextCmd_RemoveMail	, _T( "RemoveMail"	) , _T( "rm"        ) , _T( "편지삭제"         ) , _T( "편삭"			    ), TCM_CLIENT   , AUTH_GAMEMASTER	     ,  "편지 삭제" )
	ON_TEXTCMDFUNC( TextCmd_GetMailItem	, _T( "GetMailItem"	) , _T( "gm"        ) , _T( "소포받기"         ) , _T( "소포"			    ), TCM_CLIENT   , AUTH_GAMEMASTER	     ,  "소포 받기" )
	ON_TEXTCMDFUNC( TextCmd_GetMailGold	, _T( "GetMailgOLD"	) , _T( "gmg"        ) , _T( "수금"         ) , _T( "수금"			    ), TCM_CLIENT   , AUTH_GAMEMASTER	     ,  "수금" )
	ON_TEXTCMDFUNC( TextCmd_QueryMailBox	, _T( "QueryMailBox"	) , _T( "qm"        ) , _T( "편지확인"         ) , _T( "편확"			    ), TCM_CLIENT   , AUTH_GAMEMASTER	     ,  "편지 확인" )
#endif	// __POST0906
...
//		ON_TEXTCMDFUNC( TextCmd_SetMonsterRespawn,	_T( "setmonsterrespawn") ,	_T( "smr"	)	,	_T( "리스폰영역설정"  ) , _T( "리영설") , TCM_SERVER  , AUTH_ADMINISTRATOR		 , "리스폰 영역 설정"  )
	#endif // __S1108_BACK_END_SYSTEM
	#if !defined(__S1108_BACK_END_SYSTEM)
...
//		ON_TEXTCMDFUNC( TextCmd_MonsterHitRate  , _T( "MonsterHitRate"  ) , _T( "mdr"		) , _T( "몬스터히트률"    ) , _T( "몬히트"			) , TCM_SERVER  , AUTH_ADMINISTRATOR		 , "몬스터 히트률 조절 0.0 ~ 1.0 ~ 2.0 "  )
	#endif // !defined(__S1108_BACK_END_SYSTEM)
#ifdef __NPP_050308 
...
//		ON_TEXTCMDFUNC( TextCmd_RecoverMode	, _T( "RecoverMode"    ) , _T( "r_mode"	) , _T( "시련모드"    ) , _T( "시모") , TCM_CLIENT  , AUTH_GAMEMASTER		 , "시련모드설정"  )
		...
		ON_TEXTCMDFUNC( TextCmd_QuestState	, _T( "QuestState"     ) , _T( "qs"		) , _T( ""    ) , _T( "" ) , TCM_SERVER  , AUTH_ADMINISTRATOR		 , "퀘스트 설정 [ID] [State]"  )
...
#endif
#ifdef __SDEBUG_TOOLTIP_TEST
		ON_TEXTCMDFUNC( TextCmd_LoadToolTipColor , _T( "LoadToolTip"    ) , _T( "ltt"		) , _T( "로드툴팁"    ) , _T( "로툴팁"			) , TCM_CLIENT  , AUTH_ADMINISTRATOR		 , "로드 툴팁 컬러"  )
#endif // __SDEBUG_TOOLTIP_TEST
		
#ifdef __GUILDCOMBAT
		ON_TEXTCMDFUNC( TextCmd_GuildCombatRequest, _T( "GWFRequest" ), _T( "gwfr" ), _T( "길드워신청" ), _T( "길워신" ), TCM_BOTH, AUTH_GAMEMASTER, "길드워프리프 신청" )
		ON_TEXTCMDFUNC( TextCmd_GuildCombatCancel, _T( "GWFCancel" ), _T( "gwfcc" ), _T( "길드워탈퇴"), _T( "길워탈" ), TCM_BOTH, AUTH_GAMEMASTER, "길드워탈퇴" )
		ON_TEXTCMDFUNC( TextCmd_GuildCombatIn,	_T( "GWFIn" ), _T( "gwfi" ), _T( "길드워입장" ), _T( "길워입" ), TCM_BOTH, AUTH_GAMEMASTER, "길드워프리프 입장" )
		ON_TEXTCMDFUNC( TextCmd_GuildCombatOpen,	_T( "GWFOpen" ), _T( "gwfo" ), _T( "길드워오픈" ), _T( "길워오" ), TCM_BOTH, AUTH_GAMEMASTER, "길드워프리프 오픈" )
		ON_TEXTCMDFUNC( TextCmd_GuildCombatClose, _T( "GWFClose" ), _T( "gwfc" ), _T( "길드워닫기" ), _T( "길워닫" ), TCM_BOTH, AUTH_GAMEMASTER, "길드워프리프 닫기" )
#endif // __GUILDCOMBAT

		ON_TEXTCMDFUNC( TextCmd_ItemDropRate	, _T( "ItemDropRate"    ) , _T( "idr"		) , _T( ""    ) , _T( ""			) , TCM_SERVER  , AUTH_ADMINISTRATOR		 , ""  )
		ON_TEXTCMDFUNC( TextCmd_GoldDropRate 	, _T( "GoldDropRate"    ) , _T( "gdr"		) , _T( ""      ) , _T( ""			) , TCM_SERVER  , AUTH_ADMINISTRATOR		 , ""  )
		ON_TEXTCMDFUNC( TextCmd_MonsterExpRate 	, _T( "MonsterExpRate"  ) , _T( "mer"		) , _T( ""  ) , _T( ""		) , TCM_SERVER  , AUTH_ADMINISTRATOR		 , ""  )
...
//#ifndef __DEV
			...
			{
				char chMessage[128] = {0,};
				sprintf( chMessage, "Not Command!" );
				g_WndMng.PutString( chMessage );
				return TRUE;
			}				
#endif // __CLIENT
...
//#endif	// __DEV
	...
	TCHAR szTest[64] = {0};
	strcpy( szTest, lpszString );
	// 이모티콘 명령
	for( int j=0; j < MAX_EMOTICON_NUM; j++ )
	{
		if( stricmp( &(szTest[1]), g_DialogMsg.m_EmiticonCmd[ j ].m_szCommand ) == 0 )
		{
#ifdef __ONLINE
			g_DPlay.SendChat( (LPCSTR)lpszString );
#endif //__ONLINE
