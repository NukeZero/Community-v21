...
{ 
	return CWndNeuz::InitDialog( g_Neuz.GetSafeHwnd(), APP_GUILD_MERIT, 0, CPoint( 0, 0 ), pWndParent );
} 
...
{ 
	...
	switch( nID )
	{
	case WIDC_CHECK1:		// PXP 체크박스.
		{
			CWndButton* pChk = (CWndButton*)GetDlgItem( WIDC_CHECK1 );
			m_nPXP = pChk->GetCheck();		// 토글.
		}
		break;
	...
	case WIDC_BUTTON1:		// OK
		{
			...
			if( pGuild )
			{
				...
				if( m_nPXP )
				{
					if( g_pPlayer->GetExp2() >= g_pPlayer->GetMaxExp2() )
					{
						CONTRIBUTION_RESULT cbResult = pGuild->CanContribute( 1, 0, g_pPlayer->m_idPlayer );
						if( cbResult == CONTRIBUTION_OK )	// PXP 공헌가능.
							g_DPlay.SendGuildContribution( 1, 0 );
						pGuild->MeritResultMsg( cbResult );	// 안내메시지.
					} 
					else
						// PXP가 100%가 안됐다.
						g_WndMng.PutString( prj.GetText( TID_GAME_GUILDMERITLOWPXP ), NULL, prj.GetTextColor( TID_GAME_GUILDMERITLOWPXP ) );
...
void CWndGuildMeritCtrl::OnDraw( C2DRender* p2DRender )
{
	...
	for( int i = pt.y; i < m_pItemContainer.size(); i++ )
	{
		int x	= ( i - pt.x ) % 6;
		int y	= ( i - pt.y ) / 6;
...
void CWndGuildMeritCtrl::RestoreItem()
{
	...
	for( int i=0; i<m_pItemContainer.size(); i++ )
	{
