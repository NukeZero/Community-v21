...
#else//__THAILAND__
	p2DRender->TextOut( 165, 117, g_szVersion );
#endif//__THAILAND__
	p2DRender->SetFont( pOldFold );	
} 
...
{ 
	...
	if( lpWndApplet )
	{
		...
		SetTexture( m_pApp->m_pd3dDevice, MakePath( strDir, strName ), TRUE );
		*/
		SetTexture( m_pApp->m_pd3dDevice, MakePath( "Theme\\", prj.m_nLanguage, strName ), TRUE );
	}
...
/*
	...
	CWndListCtrl* pListCtrl = (CWndListCtrl*)GetDlgItem( WIDC_LISTCTRL1 );
	LV_COLUMN  pColumn;
	LV_ITEM	   lvitem;
	TCHAR      strNum[10];
	//
	...
	for(int i = 0;  i < 3; i++)
	{
		...
		pListCtrl->InsertColumn(i, &pColumn);  // assumes return value is OK.
	}
	//
	// 소스 아이템을 입력
	//
	for(i = 0; i <  256; i++ ) ///prj.m_terrainMng. GetItemPropSize(); i++) 
	{
		...
		if( lpTerrain->IsValid() )
		{
			...
			lvitem.iItem = pListCtrl->GetItemCount(); //(iSubItem == 0)? iItem : iActualItem;
			lvitem.iSubItem = 0;
