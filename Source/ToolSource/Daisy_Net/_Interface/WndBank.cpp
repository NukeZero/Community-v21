...
{ 
	...
	for( int i = 0 ; i < 3 ; ++i )
	{
		str[i].Format( _T( "%d" ),g_pPlayer->m_dwGoldBank[i] );
		pCost[i]->SetTitle( str[i] );	
...
#endif // __YSBANK_IMAGE1014
	
	
	
#ifndef __YSBANK_IMAGE1014	
	CString string[3];
	
	for( int i = 0 ; i < 3 ; ++i )
	{
		string[i].Format( _T( "%d" ),g_pPlayer->m_dwGoldBank[i] );
		pCost[i]->SetTitle( string[i] );	
		
		if( bUse[i] == FALSE )
		{
			p2DRender->RenderFillRect(  RectItemCtrl[i], D3DCOLOR_ARGB( 80, 64, 64, 64 ));
			p2DRender->RenderFillRect(  RectGold[i], D3DCOLOR_ARGB( 80, 64, 64, 64 ));
		}
	}
#endif // __YSBANK_IMAGE1014
} 
...
//	tabTabItem.mask = WTCIF_TEXT | WTCIF_PARAM;	
	...
	if( g_WndMng.m_pWndTrade || g_WndMng.m_pWndShop || g_WndMng.m_pWndGuildBank || g_WndMng.GetWndBase(APP_VENDOREX)  )
	{
		Destroy();
		return;
	}
	...
	if( ptInventory.x > m_pWndRoot->GetWndRect().Width() / 2 )
		ptMove = ptInventory - CPoint( rectInventory.Width() * 2.525, 0 );
	else
...
#ifdef __S1105_BANKOPEN
		if( g_pPlayer->m_idPlayerBank[i] == 0 )
#else //__S1105_BANKOPEN
		if( g_pPlayer->m_idPlayerBank[i] == 0 || !( g_pPlayer->IsSMMode( SM_BANK15 ) || g_pPlayer->IsSMMode( SM_BANK30 ) ) )
#endif // __S1105_BANKOPEN
		{
			...
			if( pWndFrame->GetWndId() == APP_INVENTORY )
			{
				...
				if( lpShortcut->m_dwData != 0 )
				{
					...
					else
					{
						...
						{
#ifdef __ONLINE
							g_DPlay.SendPutItemBank( nSlot, lpShortcut->m_dwId, 1 );
#endif
						}
			...
			else if( pWndFrame->GetWndId() == APP_COMMON_BANK )
			{
				...
				if( lpShortcut->m_dwData != 0 )
				{
					...
					else
					{
						...
						{
#ifdef __ONLINE
							g_DPlay.SendPutItemBankToBank( nPutSolt, nSlot, lpShortcut->m_dwId, 1 );
#endif
						}
		...
		if( m_pPass )
		{
			...
			if( strlen( szPass ) == 4 )
			{
				...
				{ 
					...
					g_DPlay.SendConfirmBank( szPass );
#endif	// __INSTANT_BANK0715
				} 
				else
				{
...
//			strncpy( szConfirmPass, m_pConfirmPass->GetString(), 10 );
			...
			if( strlen( szLastPass ) == 4 && strlen( szNewPass ) == 4 && strlen( szConfirmPass ) == 4)
			{
				if( 0 == strcmp( szNewPass, szConfirmPass ) )
				{
					...
					for( int i = 0 ; i < 4 ; i++ )
					{
						if( (isdigit( szLastPass[i] ) == FALSE) || (isdigit( szNewPass[i] ) == FALSE) || (isdigit( szConfirmPass[i] ) == FALSE) )
						{
							bisdigit = FALSE;
							break;
						}
...
#else	// __INSTANT_BANK0715
						g_DPlay.SendChangeBankPass( szLastPass, szNewPass );
#endif	// __INSTANT_BANK0715
					}
					else
					{
