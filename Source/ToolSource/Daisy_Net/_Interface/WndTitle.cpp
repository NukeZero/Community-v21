...
#ifndef __NPROTECT_VER
	#include "NPGameLib.h"
#else	// __NPROTECT_VER
	#if __NPROTECT_VER == 2
		#include "NPGameLib2.h"
	#elif __NPROTECT_VER == 3
		#include "NPGameLib3.h"
	#elif __NPROTECT_VER == 4
		...
		case IDCANCEL:   
			{
				...
				g_dpCertified.SendCertify( g_Neuz.m_szAccount, g_Neuz.m_szPassword );
#ifdef __USE_NPROTECT
				npgl.Send( (LPCSTR)g_Neuz.m_szAccount );
#endif	// __USE_NPROTECT
...
#if 0 //__VMULTILANG
	pAccount->SetFontColor ( 0xff000000 );
	pAccount->EnableModeChange( TRUE );
	//pAccount->AddWndStyle( WBS_NODRAWFRAME );
	pAccount->SetTabStop( TRUE );
	pAccount->SetNativeMode();

	pPassword->AddWndStyle( EBS_PASSWORD );
//	pPassword->AddWndStyle( WBS_NODRAWFRAME );
...
//	pPassword->AddWndStyle( WBS_NODRAWFRAME );
	...
	if( g_Option.m_szAccount[ 0 ] )
		...
		pAccount->SetFocus();
#endif	
	//pOk->AddWndStyle( WBS_NODRAWFRAME );
	//pQuit->AddWndStyle( WBS_NODRAWFRAME );


//	SetTitle( "Login" );

	//m_wndAccount.Create ( g_Neuz.GetSafeHwnd(), 0, CRect( 185, 155, 325, 175 ), this, 1000);
	//m_wndPassword.Create( g_Neuz.GetSafeHwnd(), EBS_PASSWORD, CRect( 185, 180, 325, 200 ), this, 1001 );

	//m_wndAccount.EnableModeChange( FALSE );

	//m_wndSaveAccount.Create( _T( "Save Account" ), WBS_CHECK, CRect( 80, 210, 220, 230 ), this, 100 ); 
	//m_wndSavePassword.Create( _T( "Save Password" ), WBS_CHECK, CRect( 220, 210, 370, 230 ), this, 101 ); 

	//m_wndFindAccount.AddWndStyle( WBS_NODRAWFRAME );
	//m_wndFindAccount.Create( "Forget Password", 0, CRect( 150, 230, 300, 250 ), this, 102 );

	//rect.SetRect( rect.left + 50, 280, rect.left + 160, 305 );
	//m_wndLogin.  Create( "Login", 0, rect, this, 100 ); rect.OffsetRect( 120,0 );
	//m_wndQuit.   Create( "Quit", 0, rect, this, 102 );

	//m_Texture.LoadTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, _T( "clockworks.bmp" ) ), 0x00000000 );
//
	...
	if( prj.m_nLanguage == LANG_KOR )
	{
		...
		if( g_Neuz.m_bHanGame )
		{
			rectWindow = CRect( rectWindow.left, rectWindow.top, rectWindow.right - 120, rectWindow.bottom - 115 );
			SetWndRect( rectWindow );
			rectWindow.left += 55;
		}
		rectWindow.top = 420 * rectRoot.Height() / 768;
...
{
	//CRect rect = m_pWndRoot->MakeCenterRect(250,130);
	//CRect rect = m_pWndRoot->MakeCenterRect( 470, 350 );
	//SetTitle( _T( "Login" ) );
	//return CWndNeuz::Create( WBS_MOVE | WBS_SOUND | WBS_CAPTION | WBS_THICKFRAME | WBS_MAXIMIZEBOX, rect, pWndParent, APP_LOGIN );
...
BOOL CWndLogin::OnChildNotify(UINT message,UINT nID,LRESULT* pLResult)
{
	switch(nID)
	{
		...
		case WIDC_OK: // 접속 
			{
#ifdef __TEST_CSAUTH2
				g_dpLoginClient.ConnectToServer( "192.168.1.103", 28000, TRUE, CSock::crcWrite );
				return TRUE;
#endif	// __TEST_CSAUTH2
...
//#ifdef __USE_NPROTECT
//				npgl.Send( (LPCTSTR)strAccount );
//#endif	// __USE_NPROTECT
 
				strcpy( g_Option.m_szAccount, pAccount->GetString() );
				 
#ifdef __CRYPTPW
				g_dpCertified.SendCertify( pAccount->GetString(), pPassword->GetString(), TRUE );
#else	// __CRYPTPW
				g_dpCertified.SendCertify( pAccount->GetString(), pPassword->GetString() );
#endif	// __CRYPTPW

#ifdef __USE_NPROTECT
				npgl.Send( (LPCSTR)strAccount );
#endif	// __USE_NPROTECT
...
//					g_WndMng.OpenMessageBox( _T( "계정은 3~16자 영어, 숫자를 사용할 수 있고, 숫자로 시작할 수 없습니다." ) );
				...
				if( IsPwdValid( pPassword->GetString() ) == FALSE )
				{
					g_WndMng.OpenMessageBox( _T( prj.GetText(TID_DIAG_0029) ) );
//					g_WndMng.OpenMessageBox( _T( "암호는 3~16자 영어, 숫자, 특수문자를 사용할 수 있고, 제어 문자와 한글은 사용할 수 없습니다." ) );
					break;
				}

				g_dpCertified.SendNewAccount( pAccount->GetString(), pPassword->GetString() );
				g_WndMng.OpenCustomBox( NULL, new CWndConnectingBox );
				break;
			}
#endif
...
//	m_wndServerList.Create( WLVS_REPORT, CRect( rect.left + 5, rect.top + 20, rect.right - 5, rect.bottom - 65), this, 11 );
	...
	TCHAR szServer[6][32] = 
	{
		_T( "clockworks.Vampyre" ), 
		_T( "clockworks.Amaki"   ),
		_T( "clockworks.Xuzhu"   ),
		_T( "clockworks.Kentauji"),
		_T( "clockworks.Jeff"    ),
		_T( "clockworks.Zodiakus")
	};
#else	// __ONLINE
	...
	for( int j = 0; j < g_dpCertified.m_dwSizeofServerset; j++ )
	{
		...
		{
#ifdef __J0227
			if( g_dpCertified.m_aServerset[j].lEnable != 0L || g_Neuz.m_bDeveloper )
			{
		...
		else if( g_dpCertified.m_aServerset[j].lEnable != 0L || g_Neuz.m_bDeveloper )
		{
...
#else // __S0602
				LPSERVER_DESC pServerDesc	= (LPSERVER_DESC)pWndList->GetItemData( 0 );
#endif // __S0602
...
#else	// __MSVR
		_tcscpy( szServer[j], g_dpCertified.m_aServerset[j].lpName );	
		pWndList->AddString(  szServer[j] );
#endif	// __MSVR
	}
#ifdef __MSVR
...
/*
	...
	for(int i = 0;  i < 3; i++)
	{
		...
		pColumn.iSubItem = i;
		if( i == 0 )
			pColumn.cx = 180;  // SubItem is twice as large
		else
			pColumn.cx = 30;  // SubItem is twice as large
		pColumn.pszText = szTitle[i];	
		m_wndServerList.InsertColumn(i, &pColumn);  // assumes return value is OK.
	}
	//
	// 소스 아이템을 입력
	//
	...
	for(i = 0; i <  6; i++) 
#else	// __ONLINE
	for( i = 0; i < g_dpCertified.m_dwSizeOfSrvr; i++ )
#endif	// __ONLINE
	{
		lvitem.mask = LVIF_TEXT | LVIF_PARAM; // | (iSubItem == 0? LVIF_IMAGE : 0);
		lvitem.iItem = m_wndServerList.GetItemCount(); //(iSubItem == 0)? iItem : iActualItem;
		lvitem.iSubItem = 0;
		lvitem.lParam = 0;
		lvitem.pszText = szServer[ i ];
		m_wndServerList.InsertItem(&lvitem);

		lvitem.mask = LVIF_TEXT;
		lvitem.iSubItem = 1; 
		_itoa(10,strNum,10);
		lvitem.pszText = strNum;
		m_wndServerList.SetItem(&lvitem);

		lvitem.iSubItem = 2; 
		lvitem.pszText = "b";
		m_wndServerList.SetItem(&lvitem);
	}
	//m_wndServerList.SetWndRect( GetClientRect() );
	//pWndList->AddString( _T( "beta server 1" ) );
*/
...
/*
	...
	m_wndServerList.AddString( _T( "nnp://clockworks.Vampyre" ) );
	m_wndServerList.AddString( _T( "nnp://clockworks.Amaki" ) );
	m_wndServerList.AddString( _T( "nnp://clockworks.Xuzhu" ) );
	m_wndServerList.AddString( _T( "nnp://clockworks.Kentauji" ) );
	m_wndServerList.AddString( _T( "nnp://clockworks.Jeff" ) );
	m_wndServerList.AddString( _T( "nnp://clockworks.Zodiakus" ) );
*/
//ect.SetRect( 20, rect.bottom - 30, 140, rect.bottom - 5 );
//	m_wndBack.Create( _T( "Back"   ), 0, rect, this, 100 ); rect.OffsetRect( 130, 0 );
//_wndAccept.Create( _T( "Accept" ), 0, rect, this, 101 ); rect.OffsetRect( 130, 0 );
...
#else
void CWndSelectServer::AdditionalSkinTexture( LPWORD pDest, CSize size )
#endif
{
#ifdef __YBANNER
	CPoint pt;
...
void CWndSelectServer::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if( nChar == VK_DOWN )	
	{
		...
		if( dwIndex < pWndListMulti->GetCount()-1 )
			pWndListMulti->SetCurSel(++dwIndex);
	}
}
			...
			case 188:
				{
					...
					for( int j = 0; j < g_dpCertified.m_dwSizeofServerset; j++ )
					{
						if( g_dpCertified.m_aServerset[j].dwParent == pServerDesc->dwID && ( g_dpCertified.m_aServerset[j].lEnable != 0L || g_Neuz.m_bDeveloper ) )
						{
							lCount	= g_dpCertified.m_aServerset[j].lCount;
							lMax	= g_dpCertified.m_aServerset[j].lMax;
#ifndef __J0108
							...
							long lBusy	= lMax * 0.8;
#endif	// __TAIWAN__
							if( lCount < lBusy )	strcpy( lpStrtmp, prj.GetText(TID_GAME_NORMAL));//strcpy( lpStrtmp, "정상" );
							else if( lCount < lMax )		strcpy( lpStrtmp, prj.GetText(TID_GAME_BUSY));//strcpy( lpStrtmp, "혼잡" );
#ifdef __X1006
							else	lstrcpy( lpStrtmp, prj.GetText(TID_GAME_FULL) );
#else	// __X1006
							else	strcpy( lpStrtmp, "Full" );
#endif	// __X1006
							sprintf( lpString, "%s(%s)", g_dpCertified.m_aServerset[j].lpName, lpStrtmp );
//							sprintf( lpString, "%s(%d)", g_dpCertified.m_aServerset[j].lpName, g_dpCertified.m_aServerset[j].lCount );
							int nIndex	= pWndListMulti->AddString( lpString );
...
#else	// __SCHOOL0701
#ifdef __DEV1020
					if( pDesc->lCount > pDesc->lMax && !g_Option.m_bDev )
#else	// __DEV1020
...
#ifdef __BUGFIX_SELECTSERVER1102
...
#endif	// __BUGFIX_SELECTSERVER1102
...
#		endif	// __CRC
#else	// __MSVR
			LPSERVER_DESC pServerDesc	= g_dpCertified.m_aServerset + 0;//m_wndServerList.GetCurSel();
			g_Neuz.m_dwSys	= pServerDesc->dwID;

#		ifdef __CRC
			if( !g_dpLoginClient.ConnectToServer( pServerDesc->lpAddr, PN_LOGINSRVR, TRUE, CSock::crcWrite ) )
#		else	// __CRC
			if( !g_dpLoginClient.ConnectToServer( pServerDesc->lpAddr, PN_LOGINSRVR, TRUE ) )
#		endif	// __CRC
#endif	// __MSVR
			{
				// Can't connect to server
...
//			g_WndMng.OpenCustomBox( _T( "접속중입니다. 잠시만 기다려 주십시오." ), new CWndConnectingBox );
			g_Neuz.m_dwTempMessage = 1;
			g_Neuz.m_timerConnect.Set( 1 );
#endif
			break;
...
#else
void CWndDeleteChar::AdditionalSkinTexture( LPWORD pDest, CSize size )
#endif
{
	if( g_Neuz.m_bHanGame )
	{
...
//			sprintf( s, "%s %s", GETTEXT( TID_HANGAME_DELETECHAR ), g_Neuz.m_apPlayer[pWnd->m_nSelectCharacter]->GetName() );
	...
	if( nID == WIDC_OK )
	{
		...
		if( pWnd )
		{
			...
			if( pWndEdit )
			{
...
#ifdef __SJAPAN
				if( g_Neuz.m_bHanGame )
					szNo	= g_Neuz.m_szHanPassword;

				if( 0 < strlen( szNo ) && strlen( szNo ) < 64 )
				{
					g_dpLoginClient.SendDeletePlayer( pWnd->m_nSelectCharacter, szNo );
				...
				{
					pWndEdit->SetString( "" );
					//g_WndMng.OpenMessageBox( "64보다 큽니다. 다시 입력해주세요" );
				}				
#else // __SJAPAN
				...
				else
				{
					if( strlen(szNo) == 7 )
					{
						...
						else
						{
							pWndEdit->SetString( "" );
							g_WndMng.OpenMessageBox( _T( prj.GetText(TID_DIAG_0044) ) );
	//						g_WndMng.OpenMessageBox( "주민번호 숫자 7자리로 넣어야 합니다. 다시 입력해주세요" );
						}
					}
					else
					{
...
CWndSelectChar::~CWndSelectChar()
{
	ConnectLog( "select char destroy" );
...
/*			
			...
			{
				pElem1 = pModel->GetParts( nArryEquip1[nArry] );
				pElem2 = pModel->GetParts( nArryEquip2[nArry] );
...
*/
	g_Neuz.m_apPlayer[i]->OverCoatItemRenderCheck(pModel);
			...
			DWORD dwId	= g_Neuz.m_apPlayer[i]->m_adwEquipment[PARTS_CAP];
		...
		#else	// __FINITEITEM0705
				dwId	= g_Neuz.m_apPlayer[i]->m_adwEquipment[PARTS_HAT];
		#endif	// __FINITEITEM0705
				if( dwId != NULL_ID )
				{
		#ifdef __FINITEITEM0705
		...
		#else	// __FINITEITEM0705
					if( !(g_Neuz.m_apPlayer[i]->m_adwEquipment[PARTS_HAT].byFlag & CItemElem::expired) )
		#endif	// __FINITEITEM0705
					{
						pItemProp	= prj.GetItemProp( dwId );
						if( pItemProp && pItemProp->dwBasePartsIgnore != -1 )
		...
		{
			if( m_nSelectCharacter == i )
				p2DRender->TextOut( rect.left, rect.bottom + 10, "Empty", 0xff6060ff );
			else
				p2DRender->TextOut( rect.left, rect.bottom + 10, "Empty", 0xff505050 );
		}			
#else
		/*
		p2DRender->TextOut( rect.left, rect.bottom + 10, _T( "Name : Rachel" ) );
		p2DRender->TextOut( rect.left, rect.bottom + 20, _T( "HP   : 100 / 100" ) );
		p2DRender->TextOut( rect.left, rect.bottom + 30, _T( "MP   : 100 / 100" ) );
		p2DRender->TextOut( rect.left, rect.bottom + 40, _T( "FP   : 100 / 100" ) );
		p2DRender->TextOut( rect.left, rect.bottom + 50, _T( "STR  : 100 / 100" ) );
		p2DRender->TextOut( rect.left, rect.bottom + 60, _T( "STR  : 100 / 100" ) );
		p2DRender->TextOut( rect.left, rect.bottom + 70, _T( "STR  : 100 / 100" ) );
		p2DRender->TextOut( rect.left, rect.bottom + 80, _T( "STR  : 100 / 100" ) );
		p2DRender->TextOut( rect.left, rect.bottom + 90, _T( "Lv.  : 12" ) );
		*/
#endif
		if( m_nSelectCharacter == i )
		{
			...
			//rect.DeflateRect( 1, 1 );
			//p2DRender->RenderRect( rect, D3DCOLOR_ARGB( 255, 255, 255, 255 ) ); 
...
#else	// __FINITEITEM0705
			CMover::UpdateParts( pMover->GetSex(), pMover->m_dwSkinSet, pMover->m_dwFace, pMover->m_dwHairMesh, pMover->m_dwHeadMesh, pMover->m_adwEquipment, m_pBipedMesh[ i ], NULL/*&pMover->m_Inventory*/ );
#endif	// __FINITEITEM0705
		}
	}
}
...
//				pWndBase->SetVisible( FALSE );
		...
		case WIDC_ACCEPT: // Accept
			...
			else
			{
				...
				u_short uPort = PN_CACHESRVR;
	#if __CURRENT_LANG == LANG_JAP || __CURRENT_LANG == LANG_ENG
				uPort = 15400;
	#endif

				if( g_DPlay.Connect( g_Neuz.m_lpCacheAddr, uPort ) )
				{						
					...
					if( m_nSelectCharacter != -1 && g_Neuz.m_apPlayer[m_nSelectCharacter] )
					{
						...
						//g_dpLoginClient.SendJoin( g_Neuz.m_apPlayer[m_nSelectCharacter]->m_szName );
						//Destroy();
					}
				}
				else
				{
					...
					ConnectLog( "Can't connect to server." );
					// ata2k - 캐시 서버가 실행중이지 않드므로 연결할 수 없다.
				}
#else
				g_Neuz.m_dwTempMessage = 1;
				g_Neuz.m_timerConnect.Set( 1 );
	...
	if( m_nSelectCharacter != i )
	{
		...
		if( pMover )
		{
			...
			if( g_Option.m_nLanguage == LANG_JAP )
			{
				...
				g_SoundMng.m_nSoundVolume = -(1.0f-fVolume)*5000;
			}
		}
	}
}
...
void CWndCreateChar::OnInitialUpdate()
{
	CWndNeuz::OnInitialUpdate();
/*
	CWndComboBox* pComboBox = (CWndComboBox*) GetDlgItem( WIDC_JOB );
	pComboBox->m_wndListBox.DelWndStyle( WBS_NODRAWFRAME );
	pComboBox->m_wndButton.AddWndStyle( WBS_NODRAWFRAME );


	pComboBox->m_wndListBox.DelWndStyle( WBS_NODRAWFRAME );
*/
//	CWndEdit* pEdit = (CWndEdit*) GetDlgItem( WIDC_EDIT );
	//pComboBox->AddString( _T( "머서너리" ) );
	//pComboBox->AddString( _T( "매지션" ) );
	...
	//pWndHairFaceLeft->EnableWindow( FALSE );  
	//pWndHairFaceRight->EnableWindow( FALSE ); 
	/*
	CRect rect( 120, 56, 265, 77 );
	m_wndName.Create( g_Neuz.GetSafeHwnd(), 0, rect, this, 1000 );	m_wndName.SetAlphaNumericMode(); rect.OffsetRect( 0, 50 ); 
	//m_wndJob.Create( g_Neuz.GetSafeHwnd(), 0, rect, this, 1001 ); m_wndJob.SetAlphaNumericMode(); rect.OffsetRect( 0, 30 ); 
	m_wndComboJob.Create( 0, rect, this, 10 ); rect.OffsetRect( 0, 30 );
	m_wndComboJob.AddString( _T( "머서너리" ) );
	//m_wndComboJob.AddString( _T( "아크로벳" ) );
	//m_wndComboJob.AddString( _T( "어시스트" ) );
	m_wndComboJob.AddString( _T( "매지션" ) );
	//m_wndComboJob.AddString( _T( "엔지니어" ) );
	m_wndComboJob.SetCurSel( 0 );
	m_Player.m_byJob = 0;

	m_wndName.PutString( _T( "레이첼" ) );

	CRect rect1( 118, 136, 185, 157 );	
	CRect rect2( 195, 136, 265, 157 );
	m_wndMale      .Create( _T( "Male"   ), 0, rect1, this, 50 ); rect1.OffsetRect( 0, 30 ); 
	m_wndFemale    .Create( _T( "Female" ), 0, rect2, this, 51 ); rect2.OffsetRect( 0, 30 ); 
	m_wndLHair     .Create( _T( "<"      ), 0, rect1, this, 52 ); rect1.OffsetRect( 0, 30 ); 
	m_wndRHair     .Create( _T( ">"      ), 0, rect2, this, 53 ); rect2.OffsetRect( 0, 30 ); 
	m_wndLHairColor.Create( _T( "<"      ), 0, rect1, this, 54 ); rect1.OffsetRect( 0, 30 ); 
	m_wndRHairColor.Create( _T( ">"      ), 0, rect2, this, 55 ); rect2.OffsetRect( 0, 30 ); 
	m_wndLFace     .Create( _T( "<"      ), 0, rect1, this, 56 ); rect1.OffsetRect( 0, 30 ); 
	m_wndRFace     .Create( _T( ">"      ), 0, rect2, this, 57 ); rect2.OffsetRect( 0, 30 ); 
	//m_wndLCostume  .Create( _T( "<"      ), 0, rect1, this, 58 ); rect1.OffsetRect( 0, 30 ); 
	//m_wndRCostume  .Create( _T( ">"      ), 0, rect2, this, 59 ); rect2.OffsetRect( 0, 30 ); 
	//m_wndMale  .Create( _T( "Male"   ), 0, rect1, this, 50 ); rect1.OffsetRect( 0, 50 ); 
	//m_wndFemale.Create( _T( "Female" ), 0, rect2, this, 51 ); rect2.OffsetRect( 0, 50 ); 

	rect = GetClientRect();
	rect.SetRect( 138, 348, 238, 368 );
	m_wndCreate.Create( _T( "Ok" ), 0, rect, this, 101 );
	rect.SetRect( 278, 348, 378, 368 );
	m_wndCancel.Create( _T( "Cancel" ), 0, rect, this, 100 ); 
...
#ifndef __ONLINE
	CRect rect;
	rect.SetRect( 0, 300, 100, 320 );
	m_wndAccept.Create( _T( "Accept" ), 0, rect, this, 10002 );//ect.OffsetRect( 130, 0 );
#endif
...
BOOL CWndCreateChar::LoadInvalidName( void )
{
	CScanner s;
	TCHAR szName[64];
...
BOOL CWndCreateChar::IsInvalidName( LPCSTR sName )
{
	TCHAR pszName[ 64 ];
...
#else	// __FINITEITEM0705
	memset( m_Player.m_adwEquipment, 0xff, sizeof( m_Player.m_adwEquipment ) );
#endif	// __FINITEITEM0705
...
#else	// __FINITEITEM0705
			m_Player.m_adwEquipment[ pItemProp->dwParts ] = dwEquip;
#endif	// __FINITEITEM0705
		}
	}
#ifdef __FINITEITEM0705
...
#else	// __FINITEITEM0705
	CMover::UpdateParts( m_Player.m_bySex, 0, m_Player.m_bySkinSet, m_Player.m_byHairMesh, m_Player.m_byHeadMesh, m_Player.m_adwEquipment, m_pModel, NULL );
#endif	// __FINITEITEM0705
}
...
BOOL CWndCreateChar::Initialize( CWndBase* pWndParent, DWORD dwStyle )
{
	//CRect rect = m_pWndRoot->MakeCenterRect(250,130);
	CRect rect = m_pWndRoot->MakeCenterRect( 590, 400 );
	//SetTitle( _T( "Create Character" ) );
	//return CWndNeuz::Create( /*WBS_MODAL |*/ WBS_MOVE | WBS_SOUND | WBS_CAPTION | WBS_THICKFRAME | WBS_MAXIMIZEBOX, rect, pWndParent, APP_CREATE_CHAR );
	return CWndNeuz::InitDialog( g_Neuz.GetSafeHwnd(), APP_CREATE_CHAR, WBS_KEY, CPoint( 0, 0 ), pWndParent );

	//return CWndNeuz::Create(dwStyle|WBS_MOVE|WBS_SOUND|WBS_CAPTION|WBS_THICKFRAME|WBS_MAXIMIZEBOX,rect,pWndParent,10);
}
#ifndef __ONLINE
void CWndCreateChar::Connected()
{
	CWndEdit* pEdit = (CWndEdit*)GetDlgItem( WIDC_NAME );
	_tcscpy( m_Player.m_szName, pEdit->m_string );
	g_WndMng.CloseMessageBox();
	g_Neuz.PlayerSetting( &m_Player );
	g_WndMng.OpenField();
	Destroy();
}
#endif
...
{
	/*
	m_Player.m_bySkinSet = SKINSET_00;
	m_Player.m_byHairMesh = HAIRMESH_00;
	m_Player.m_byHairColor = 0;
	m_Player.m_bySex = SEX_MALE;
	m_Player.m_byCostume = 0;
*/
	/*
	if( message == WNM_SELCHANGE )
	{
		...
		//m_Player.m_byJob = m_wndComboJob.GetCurSel();
	...
	if( message == WNM_CLICKED )
	{
...
#else	// __FINITEITEM0705
			CMover::UpdateParts( m_Player.m_bySex, m_Player.m_bySkinSet, m_Player.m_byFace, m_Player.m_byHairMesh, m_Player.m_byHeadMesh, m_Player.m_adwEquipment, m_pModel, NULL );
#endif	// __FINITEITEM0705
			break;
		...
		case WIDC_FACE_LEFT: // head
			...
			if( m_Player.m_byHeadMesh < 0 )
				m_Player.m_byHeadMesh = MAX_HEAD - 1;
#ifdef __FINITEITEM0705
			CMover::UpdateParts( m_Player.m_bySex, m_Player.m_bySkinSet, m_Player.m_byFace, m_Player.m_byHairMesh, m_Player.m_byHeadMesh, m_Player.m_aEquipInfo, m_pModel, NULL );
#else	// __FINITEITEM0705
			CMover::UpdateParts( m_Player.m_bySex, m_Player.m_bySkinSet, m_Player.m_byFace, m_Player.m_byHairMesh, m_Player.m_byHeadMesh, m_Player.m_adwEquipment, m_pModel, NULL );
		...
		case WIDC_FACE_RIGHT: // head
			m_Player.m_byHeadMesh++;
			if( m_Player.m_byHeadMesh >= MAX_HEAD )
				m_Player.m_byHeadMesh = 0;
#ifdef __FINITEITEM0705
			CMover::UpdateParts( m_Player.m_bySex, m_Player.m_bySkinSet, m_Player.m_byFace, m_Player.m_byHairMesh, m_Player.m_byHeadMesh, m_Player.m_aEquipInfo, m_pModel, NULL );
#else	// __FINITEITEM0705
		...
		case 55: // hairColor
			break;
		case 56: // skin
			m_Player.m_bySkinSet--;
			if( m_Player.m_bySkinSet < 0 )
				m_Player.m_bySkinSet = MAX_SKIN - 1;
				CMover::UpdateParts( m_Player.m_bySex, m_Player.m_bySkinSet, m_Player.m_byFace, m_Player.m_byHairMesh, m_Player.m_adwEquipment, m_pModel, NULL );
			break;
		case 57: // skin
			m_Player.m_bySkinSet++;
			if( m_Player.m_bySkinSet >= MAX_SKIN )
				m_Player.m_bySkinSet = 0;
				CMover::UpdateParts( m_Player.m_bySex, m_Player.m_bySkinSet, m_Player.m_byFace, m_Player.m_byHairMesh, m_Player.m_adwEquipment, m_pModel, NULL );
			break;
		case 58: // costume
			break;
...
//				g_WndMng.OpenMessageBox( _T( "이름을 입력하십시오." ) );
				return TRUE;
			}
			CHAR c = string[ 0 ];
...
//				g_WndMng.OpenMessageBox( _T( "명칭에 첫글자를 숫자로 사용할 수 없습니다." ) );
					...
					default:
						{
							if( IsMultiByte( iter ) ) 
							{
							//	wchar_t ch = (wchar_t) ( ((wchar_t) bytes[0] << 8) | (wchar_t) bytes[1] );
		...
		//					g_WndMng.OpenMessageBox( _T( "명칭에 콘트롤이나 스페이스, 특수 문자를 사용할 수 없습니다." ) );
			...
			if( string.GetLength() > 16 )
				Error( "CWndCreateChar::OnChildNotify에서 이상 캐릭터 명 A: %s", string );
...
//				g_WndMng.OpenMessageBox( _T( "사용할수 없는 이름입니다" ) );
			...
			if( string.GetLength() > 16 )
				Error( "CWndCreateChar::OnChildNotify에서 이상 캐릭터 명 B: %s", string );
			_tcscpy( m_Player.m_szName, string );
			if( strlen( m_Player.m_szName ) > 16 )
				Error( "CWndCreateChar::OnChildNotify에서 이상 캐릭터 명 : %s", string );
			...
			g_dpLoginClient.SendCreatePlayer( m_Player.m_uSlot, m_Player.m_szName/*, m_Player.m_adwEquipment*/, m_Player.m_byFace, m_Player.m_byCostume, m_Player.m_bySkinSet, m_Player.m_byHairMesh, dwHairColor, m_Player.m_bySex, m_Player.m_byJob, m_Player.m_byHeadMesh );
	#else
			Destroy();
			g_WndMng.ObjectExecutor( SHORTCUT_APPLET, APP_SELECT_CHAR );
	#endif
