...
#include "../Daisy/Properties.h"
#include "../Daisy/MainFrm.h"
#include "../Daisy/ResourceView.h"
#include "../Daisy/DaisyDoc.h"
#include "../Daisy/DaisyView.h"
...
int       CWndBase::m_nLayout = LAYOUT_GRID; // rulers, grid
int       CWndBase::m_nGridWidth = 2;
int       CWndBase::m_nGridHeight = 2;
#else
BOOL      CWndBase::m_bCling   = TRUE;
#endif
...
//	m_pFont        = NULL ;
	...
	m_hWnd = g_pMainFrame->m_pDaisyView->CView::GetSafeHwnd();
...
void CWndBase::RemoveWnd(CWndBase* pWndBase)
{
	...
	if(pWndBase == m_pWndFocus)
	{
		...
		CWndBase* pWndBaseTemp = m_pWndFocus->GetParentWnd(); //m_wndOrder.GetSize() ? (CWndBase*)m_wndOrder[ i ] : NULL;
		if(pWndBaseTemp) pWndBaseTemp->SetFocus();
		//else m_pWndFocus->OnKillFocus(NULL);
#else
		CWndBase* pWndBaseTemp = m_wndOrder.GetSize() ? (CWndBase*)m_wndOrder[m_wndOrder.GetUpperBound()] : NULL;
		int nSize = m_wndOrder.GetSize();
		if( nSize )
		{
			int nidx = m_wndOrder.GetUpperBound();
			if( nidx < 0 || nidx >= nSize )
			{
				LPCTSTR szErr = Error( "CWndBase::RemoveWnd m_wndOrder 범위침범 %d %d", nidx, nSize );
				ADDERRORMSG( szErr );
			}
		}
		if(pWndBaseTemp) pWndBaseTemp->SetFocus();
...
//	m_nFontColorR = m_pParentWnd->m_nFontColorR;
//	m_nFontColorG = m_pParentWnd->m_nFontColorG;
//	m_nFontColorB = m_pParentWnd->m_nFontColorB;

	m_bNoCloseButton = m_pParentWnd->m_bNoCloseButton;
...
void CWndBase::RenderWnd()
{
	...
	if( m_bDragControl /*&& m_bLButtonDown*/ )
		__asm nop;
...
//{{AFX
CWndBase* CWndBase::GetChildFrameFocus( CWndBase* pWndBase, POINT point )
{
	CWndBase* pWndSelect = pWndBase;
	CWndBase* pWndTemp = NULL;
	for( int i = 0; i < pWndBase->m_wndArray.GetSize(); i++ )
	{
		...
		if( pWndCur->IsWndStyle( WBS_CHILD ) && pWndCur->IsVisible() && pWndCur->IsWindowEnabled() )
		{
			CPoint ptWindow = point;
			pWndCur->ScreenToWindow( &ptWindow );
			if( pWndCur->IsWndStyle( WBS_CHILDFRAME ) )
			{
				if( pWndCur->IsPickupSpace( ptWindow ) )
					pWndSelect = pWndCur;
			}
			pWndTemp = GetChildFrameFocus( pWndCur, point );
			if( pWndTemp != pWndCur )
				pWndSelect = pWndTemp;
		}
	}
...
/*
...
#ifdef __DAISY
			if( pt != GetWindowRect( TRUE ).TopLeft() ) //GetWindowRect( TRUE ).TopLeft() );
			{
				if( m_nLayout == LAYOUT_GRID && IsWndStyle( WBS_CHILD ) )
				{
					pt.x = ( pt.x / m_nGridWidth  ) * m_nGridWidth;
					pt.y = ( pt.y / m_nGridHeight ) * m_nGridHeight;
				}
#endif
				//rectWnd += pt;
				int w = rectWnd.Width();
				...
				{
#ifdef __DAISY
					CRect rect = m_pParentWnd->GetLayoutRect();
#else
	...
	//else if( m_bDragControl && m_bLButtonDown )
	//{
	//	RenderHelper_DragIcon::Instance()->SetDragTexture( m_pTexture );
	//	RenderHelper_DragIcon::Instance()->UpdateVertexFromPoint( CPoint( 10, 10 ) );
	//}
...
#ifdef __DAISY
			if( m_bEdit && m_pParentWnd->IsWndStyle( WBS_CHILD) == FALSE )
			{
				if( m_pCurFocus == this && m_pWndFocus != this )
				{
					if(m_nResizeDir == 0)
					{
						if( IsPickupSpace( ptWindow ) )
						{
//							if( !IsWndStyle(  WSS_PICTURE  ) )
							{
								m_bPickup = TRUE;
...
#else
						CWndBase* pWndTemp = m_pWndFocusChild ? this : m_pParentWnd;
#endif
						CWndBase* pWndBase;
						int nCurIdx = -1;
						int i = 0;
...
#else
//{{AFX
	if( IsWndStyle( WBS_CHILD ) && !IsWndStyle( WBS_CHILDFRAME) )
//}}AFX
#endif
	{
	...
	else
	{
...
#else
							if( ( pWnd2->IsWndStyle( WBS_TOPMOST ) || pWnd2->IsWndStyle( WBS_POPUP ) || pWnd2->IsWndStyle( WBS_MODAL ) ) && pWnd2->IsVisible() )
#endif
								break;
						}
 		...
 		if(m_pWndFocus != this)
		{
			...
			// 윈도 킬, 셋 
			if(m_pWndFocus) 
				m_pWndFocus->OnKillFocus(this);
			//OnSetFocus(m_pWndFocus);
			CWndBase* pWndOld = m_pWndFocus;
			// 컨트롤 킬, 셋 
			if(m_pWndFocus && m_pWndFocus->m_pWndFocusChild) 
...
{ 
	...
	if( m_bTile == FALSE )
	{
		...
		{
			lpImage = new IMAGE;
			if( LoadImage( m_pApp->GetDevice(), MakePath( DIR_THEME, CWndBase::m_resMng.GetSelectLang(),m_strTexture ), lpImage ) == FALSE )
	...
	CString m_strTile = m_strTexture;
	CString strFileName;
	
	int nImgBit = IMGBIT_24;
	for( int i = 0; i < 8; i++ )
	{
		...
		{
			lpImage[i] = new IMAGE;
			if( LoadImage( m_pApp->GetDevice(), MakePath( DIR_THEME, CWndBase::m_resMng.GetSelectLang(),strFileName ), lpImage[i] ) == FALSE )
...
#else //__V050322_CACHEQUEST
//{{AFX
void CWndBase::AdditionalSkinTexture( LPWORD pDest, CSize sizeSurface )
{ 
	for( int i = 0; i < m_wndArray.GetSize(); i++ )
	{
		CWndBase* pWndBase = (CWndBase*)m_wndArray.GetAt( i );
		if( pWndBase->IsDestroy() == FALSE && pWndBase->IsVisible() )
			pWndBase->AdditionalSkinTexture( pDest, sizeSurface );
	}
}
//}}AFX
#endif // __V050322_CACHEQUEST
...
//		}
		...
		//D3DXSaveTextureToFile( "d:\\UI.tga", D3DXIFF_TGA, pTexture->m_pTexture, NULL );
	}
#endif
}
