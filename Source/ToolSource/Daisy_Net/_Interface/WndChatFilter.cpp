...
void CWndChatFilter::SetButtonStatus()
{
...
#else
	if( dwChatFilter & CHATSTY_HELP_CLIENT   ) pWndCheck6->SetCheck( TRUE );
	if( dwChatFilter & CHATSTY_HELP_CHAT     ) pWndCheck7->SetCheck( TRUE );
	if( dwChatFilter & CHATSTY_GAME_CLIENT   ) pWndRadio1->SetCheck( TRUE );
	if( dwChatFilter & CHATSTY_GAME_CHAT     ) pWndRadio2->SetCheck( TRUE );
	if( dwChatFilter & CHATSTY_SYSTEM_CLIENT ) pWndRadio3->SetCheck( TRUE );
	if( dwChatFilter & CHATSTY_SYSTEM_CHAT   ) pWndRadio4->SetCheck( TRUE );
#endif
	...
	if( g_Option.m_dwChatFilter & CHATSTY_GENERAL     ) pWndCheck1->SetCheck( TRUE );
	if( g_Option.m_dwChatFilter & CHATSTY_WHISPER     ) pWndCheck2->SetCheck( TRUE );
	if( g_Option.m_dwChatFilter & CHATSTY_SHOUT       ) pWndCheck3->SetCheck( TRUE );
	if( g_Option.m_dwChatFilter & CHATSTY_PARTY       ) pWndCheck4->SetCheck( TRUE );
	if( g_Option.m_dwChatFilter & CHATSTY_GUILD       ) pWndCheck5->SetCheck( TRUE );

	if( g_Option.m_dwChatFilter & CHATSTY_HELP_CLIENT ) pWndCheck6->SetCheck( TRUE );
	if( g_Option.m_dwChatFilter & CHATSTY_HELP_CHAT   ) pWndCheck7->SetCheck( TRUE );
	if( g_Option.m_dwChatFilter & CHATSTY_GAME_CLIENT   ) pWndRadio1->SetCheck( TRUE );
	if( g_Option.m_dwChatFilter & CHATSTY_GAME_CHAT     ) pWndRadio2->SetCheck( TRUE );
	if( g_Option.m_dwChatFilter & CHATSTY_SYSTEM_CLIENT ) pWndRadio3->SetCheck( TRUE );
	if( g_Option.m_dwChatFilter & CHATSTY_SYSTEM_CHAT   ) pWndRadio4->SetCheck( TRUE );

	if( pWndChat->m_nChatChannel == CHATSTY_GENERAL ) pWndCheck1->EnableWindow( FALSE );
	if( pWndChat->m_nChatChannel == CHATSTY_WHISPER ) pWndCheck2->EnableWindow( FALSE );
	if( pWndChat->m_nChatChannel == CHATSTY_SHOUT   ) pWndCheck3->EnableWindow( FALSE );
	if( pWndChat->m_nChatChannel == CHATSTY_PARTY   ) pWndCheck4->EnableWindow( FALSE );
	if( pWndChat->m_nChatChannel == CHATSTY_GUILD   ) pWndCheck5->EnableWindow( FALSE );
#endif
...
#else
	return CWndNeuz::InitDialog( g_Neuz.GetSafeHwnd(), APP_CHAT_FILTER, 0, CPoint( 0, 0 ), pWndParent );
#endif
} 
/*
  직접 윈도를 열때 사용 
...
//{{AFX
	case WIDC_CHECK6 :
		if( pWndCheck6->GetCheck() )
			g_Option.m_dwChatFilter[nChannel] |= CHATSTY_HELP_CLIENT;
		else
			g_Option.m_dwChatFilter[nChannel] &= ~CHATSTY_HELP_CLIENT;
		SetButtonStatus();
		break;
	case WIDC_CHECK7 :
		if( pWndCheck6->GetCheck() )
			g_Option.m_dwChatFilter[nChannel] |= CHATSTY_HELP_CHAT;
		else
			g_Option.m_dwChatFilter[nChannel] &= ~CHATSTY_HELP_CHAT;
		SetButtonStatus();
		break;
	/////////////////////////////////////////////////
	case WIDC_RADIO1 :
		g_Option.m_dwChatFilter[nChannel] |= CHATSTY_GAME_CLIENT;
		g_Option.m_dwChatFilter[nChannel] &= ~CHATSTY_GAME_CHAT;
		break;
	case WIDC_RADIO2 :
		g_Option.m_dwChatFilter[nChannel] |= CHATSTY_GAME_CHAT;
		g_Option.m_dwChatFilter[nChannel] &= ~CHATSTY_GAME_CLIENT;
		break;
	case WIDC_RADIO3 :
		g_Option.m_dwChatFilter[nChannel] |= CHATSTY_SYSTEM_CLIENT;
		g_Option.m_dwChatFilter[nChannel] &= ~CHATSTY_SYSTEM_CHAT;
		break;
	case WIDC_RADIO4 :
		g_Option.m_dwChatFilter[nChannel] |= CHATSTY_SYSTEM_CHAT;
		g_Option.m_dwChatFilter[nChannel] &= ~CHATSTY_SYSTEM_CLIENT;
...
/*
	case WIDC_CHECK7 :
		if( pWndCheck7->GetCheck() )
			g_Option.m_dwChatFilter |= CHATSTY_GAME ;
		else
			g_Option.m_dwChatFilter &= ~CHATSTY_GAME ;
		break;
	case WIDC_CHECK8 :
		if( pWndCheck8->GetCheck() )
			g_Option.m_dwChatFilter |= CHATSTY_SYSTEM;
		else
			g_Option.m_dwChatFilter &= ~CHATSTY_SYSTEM;
		break;
	case WIDC_CHECK9 :
		//if( pWndCheck9->GetCheck() )
		//	g_Option.m_dwChatFilter |= CHATSTY_CLIENTMSG ;
		//else
		//	g_Option.m_dwChatFilter &= ~CHATSTY_CLIENTMSG ;
		break;
		*/
	...
	case WIDC_CHECK6 :
		if( pWndCheck6->GetCheck() )
			g_Option.m_dwChatFilter |= CHATSTY_HELP_CLIENT;
		else
			g_Option.m_dwChatFilter &= ~CHATSTY_HELP_CLIENT;
		SetButtonStatus();
		break;
	case WIDC_CHECK7 :
		if( pWndCheck6->GetCheck() )
			g_Option.m_dwChatFilter |= CHATSTY_HELP_CHAT;
		else
			g_Option.m_dwChatFilter &= ~CHATSTY_HELP_CHAT;
		SetButtonStatus();
		break;
	/////////////////////////////////////////////////
	case WIDC_RADIO1 :
		g_Option.m_dwChatFilter |= CHATSTY_GAME_CLIENT;
		g_Option.m_dwChatFilter &= ~CHATSTY_GAME_CHAT;
		break;
	case WIDC_RADIO2 :
		g_Option.m_dwChatFilter |= CHATSTY_GAME_CHAT;
		g_Option.m_dwChatFilter &= ~CHATSTY_GAME_CLIENT;
		break;
	case WIDC_RADIO3 :
		g_Option.m_dwChatFilter |= CHATSTY_SYSTEM_CLIENT;
		g_Option.m_dwChatFilter &= ~CHATSTY_SYSTEM_CHAT;
		break;
	case WIDC_RADIO4 :
		g_Option.m_dwChatFilter |= CHATSTY_SYSTEM_CHAT;
		g_Option.m_dwChatFilter &= ~CHATSTY_SYSTEM_CLIENT;
