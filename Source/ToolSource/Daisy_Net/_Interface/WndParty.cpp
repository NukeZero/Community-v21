...
//	pWndTabCtrl->SetButtonLength( 53 );
...
{ 
	...
	return CWndNeuz::InitDialog( g_Neuz.GetSafeHwnd(), APP_PARTY, 0, CPoint( 0, 0 ), pWndParent );
} 
/*
  직접 윈도를 열때 사용 
BOOL CWndParty::Initialize( CWndBase* pWndParent, DWORD dwWndId ) 
...
void CWndPartyInfo::OnDraw( C2DRender* p2DRender ) 
{
	...
	if( m_nSelected != -1 ) 
	{
		CRect rect( 5, 8 + m_nSelected * 15, 315, 22 + m_nSelected * 15 );
	...
	//for( int i = 0; i < 8; i++ )//g_Party.m_nSizeofMember; i++ ) 
	{
		...
		else
		{
			...
			if( g_Party.m_aMember[ i ].m_bRemove ) 
				colorStatus = 0xff000000; // 서버에 업ㅂ은놈
		}
		...
		if( strName.GetLength() > 8 ) 
		{
			for( int i = 0; i < 8; )
			{
				if( IsDBCSLeadByte( strName[ i ] ) )
					i+=2;
				else
					i++;
			}
			strName = strName.Left( i ); // 10바이트째에서 한글이 두동강날거같으면 9바이트에서 짜르기
		...
		rect.SetRect( 200, y, 200 + 110, y + 12 );
		rectTemp = rect; 
		...
		for( int i = 0; i < 3; i++ )
		{
			for( int j = 0; j < 3; j++, nCount++ )
			{
				...
				if( pItemProp ) //&& g_Party.m_nSizeofMember >= 2 )
				{
					...
					CPoint point1 = CPoint( j * nWidth + 3 + 35 , i * nHeight + 6 );
