...
void CWndCommItemCtrl::OnMouseWndSurface( CPoint point )
{
	...
	for( int i = 0; i < m_nMaxDraw; i++ ) 
	{
		...
		if( rectHittest.PtInRect( point ) )
		{
	...
	#else // __SM_ITEM_2ND
			ItemProp* pItem = prj.GetItemProp( g_AddSMMode.dwSMItemID[m_dwDraw[i]] );
	#endif // __SM_ITEM_2ND
			if( pItem == NULL )
				continue;
			...
			else
			{
				...
				CTimeSpan ct( lData );
			#else
				CTimeSpan ct( pItem->dwCircleTime );
			#endif
			...
			if( pItem->dwID == II_SYS_SYS_SCR_AMPESA || pItem->dwID == II_SYS_SYS_SCR_AMPESB || pItem->dwID == II_SYS_SYS_SCR_AMPESC || pItem->dwID == II_SYS_SYS_SCR_AMPESD )
			{
				str += strEnter;
				strTemp.Format( prj.GetText( TID_GAME_EXP_LEVELUSE ), pItem->szCommand );	// 레벨에 따라 중복가능
				str += strTemp;
			}				
	...
	#else // __SM_ITEM_2ND
			g_toolTip.PutToolTip( g_AddSMMode.dwSMItemID[m_dwDraw[i]], str, rectHittest, point, 0 );
	#endif // __SM_ITEM_2ND
		}
		pt.y += m_nFontHeight;
	}
...
int CWndCommItemCtrl::GetMaxBuff()
{
	...
	for( i=0 ; i<MAX_SKILLINFLUENCE; ++i )
	{
		...
		if( pSkill->wID == 0 || pSkill->wType != BUFF_ITEM )	
			continue;
...
void CWndCommItemCtrl::DrawSM( C2DRender* p2DRender, CPoint* pPoint, int x, int &nScroll )
{
	for( int i = 0 ; i < SM_MAX; ++i )
	{
		...
		if( pItem->dwID == II_SYS_SYS_SCR_BLESSING )
			...
			string.Format( prj.GetText( TID_TOOLTIP_DATE ), ct.GetDays(), ct.GetHours(), ct.GetMinutes(), ct.GetSeconds() );
...
void CWndCommItemCtrl::DrawSkill( C2DRender* p2DRender, CPoint* pPoint, int x, int &nScroll )
{
	...
	BOOL bExpRander[4];
	for( int iRander = 0 ; iRander < 4 ; ++iRander )
	...
	for( int i=0 ; i<MAX_SKILLINFLUENCE; ++i )
	{
		...
		if( dwSkillID == II_SYS_SYS_SCR_AMPESA || dwSkillID == II_SYS_SYS_SCR_AMPESB || dwSkillID == II_SYS_SYS_SCR_AMPESC || dwSkillID == II_SYS_SYS_SCR_AMPESD )
		{
...
#endif // __SM_ITEM_2ND
#ifdef __OVERLAPPING_EXPITEM_S0601
		else if( dwSkillID == II_SYS_SYS_SCR_AMPESA || dwSkillID == II_SYS_SYS_SCR_AMPESB || dwSkillID == II_SYS_SYS_SCR_AMPESC || dwSkillID == II_SYS_SYS_SCR_AMPESD )
		{
			string = "";
...
//{{AFX
	...
	int nMax =  nCount;//Gg_Party.m_nSizeofMember;
	// 눈에 보이는 갯수가 페이지라인수 보다 크면 보이는 갯수를 페이지라인수로 조정 
	if( nMax - m_wndScrollBar.GetScrollPos() > m_wndScrollBar.GetScrollPage() )
		nMax = m_wndScrollBar.GetScrollPage() + m_wndScrollBar.GetScrollPos();
	if( nMax < m_wndScrollBar.GetScrollPos() )
	...
	for( int i = 0; i < m_wndScrollBar.GetScrollPos() && i < nCount; ++i );
	...
	memset( m_dwDraw, 0, sizeof( int ) * SM_MAX );
	m_nMaxDraw = 0;
	int nScroll = 0;
	for( i = 0 ; i < SM_MAX; ++i )
	{
		...
		{
			if( i == SM_RESIST_ATTACK_LEFT || i == SM_RESIST_ATTACK_RIGHT || i == SM_RESIST_DEFENSE )
				p2DRender->RenderTexture( CPoint( 2, pt.y ), pWndWorld->m_dwSMResistItemTexture[nResistTexture], 192 );
			else
				p2DRender->RenderTexture( CPoint( 2, pt.y ), pWndWorld->m_dwSMItemTexture[i], 192 );
			
			m_dwDraw[m_nMaxDraw] = i;
			++m_nMaxDraw;
		}

		DWORD dwColor = 0xff000000;
		CString string;
		string.Format( "%s", pItem->szName );
		p2DRender->TextOut( x + 40, pt.y + 3, string, 0xff0000ff );
		CTimeSpan ct( g_pPlayer->m_dwSMTime[i] );
		CString strMessage;
		...
		strMessage += string;
		p2DRender->TextOut( x + 40, pt.y + 18, strMessage, dwColor );
