	...
	for( int i = m_wndScrollBar.GetScrollPos(); i < nMax; i++ ) 
	{
		...
		if( IsValidObj(pObjMember) )
		{
			if( pObjMember->GetHitPoint() == 0 ) 
				...
				if( ((FLOAT)pObjMember->GetHitPoint()) / ((FLOAT)pObjMember->GetMaxHitPoint()) <.1f ) 
					dwColor = 0xffffff00; // HP 10% 이하인놈
			strMember.Format( "%d %s", pObjMember->GetLevel(), g_Party.m_aMember[ i ].m_szName );
		}
		else
		{
			dwColor = 0xff878787; // 디폴트는 주위에 없는놈
			if( g_Party.m_aMember[ i ].m_bRemove ) 
				dwColor = 0xff000000; // 서버에 업ㅂ은놈
		...
		if( MAX_EXPERT <= g_Party.m_aMember[ i ].m_nJob )
		{
			pWndWorld->m_texMsgIcon.MakeVertex( p2DRender, CPoint( 2, pt.y ),  ( 70 + g_Party.m_aMember[ i ].m_nJob - 6 ) + ( 8 * g_Party.m_aMember[ i ].m_nSex ), &pVertices, 0xffffffff );
		}
		else
		{
			pWndWorld->m_texMsgIcon.MakeVertex( p2DRender, CPoint( 2, pt.y ),  12 + g_Party.m_aMember[ i ].m_nJob + ( 6 * g_Party.m_aMember[ i ].m_nSex ), &pVertices, 0xffffffff );
		}
