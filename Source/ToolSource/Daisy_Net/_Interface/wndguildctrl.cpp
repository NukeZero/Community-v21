	...
	if( pGuild )
	{
		...
		for( int j = 0 ; j < m_nServerCount[g_Neuz.m_uIdofMulti-1] && i < nMax; ++j, ++i )
		{
			...
			if( szMember )
			{
				...
				// 직업과 성별 아이템 생성
				if( MAX_EXPERT <= pGuildMember->m_nJob )
				{
					pWndWorld->m_texMsgIcon.MakeVertex( p2DRender, CPoint( 2, pt.y ),  ( 70 + pGuildMember->m_nJob - 6 ) + ( 8 * pGuildMember->m_dwSex ), &pVertices, 0xffffffff );
				}
				else
				{
					pWndWorld->m_texMsgIcon.MakeVertex( p2DRender, CPoint( 2, pt.y ), 12 + pGuildMember->m_nJob + ( 6 * pGuildMember->m_dwSex ), &pVertices, 0xffffffff );
				}
				// 멀티 서버 아이템 생성
			...
			for( int k = 0 ; k < m_nServerCount[j] && i < nMax ; ++k , ++i )
			{
				...
				if( szMember )
				{
					...
					if( j == 10 )	// OFFLINE
					{
						...
						if( MAX_EXPERT <= pGuildMember->m_nJob )
						{
							pWndWorld->m_texMsgIcon.MakeVertex( p2DRender, CPoint( 2, pt.y ),  ( 70 + pGuildMember->m_nJob - 6 ) + ( 8 * pGuildMember->m_dwSex ), &pVertices, 0xffff6464 );
						}
						else
						{
							pWndWorld->m_texMsgIcon.MakeVertex( p2DRender, CPoint( 2, pt.y ), 12 + pGuildMember->m_nJob + ( 6 * pGuildMember->m_dwSex ), &pVertices, 0xffff6464 );
						}
					}
					else
					{
