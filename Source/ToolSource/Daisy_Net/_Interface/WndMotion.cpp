...
void CWndEmoticon::OnDraw(C2DRender* p2DRender)
{
	...
	for( int i=0; i < g_DialogMsg.m_texEmoticonUser.GetNumber(); i++ )
	{
...
{ 
	...
	for( int i = 0; i < m_motionArray.GetSize(); i++ )
	{
		...
		if( pMotionProp )
		{
			...
			ny = j / 10;
			p2DRender->RenderTexture( CPoint( ( j - ( ny * 10 ) ) * 32 + 10, 7 + ( ny * 32 ) ), pMotionProp->pTexture );
#else // __S0126_MOTION_AUTORUN
			p2DRender->RenderTexture( CPoint( j * 32 + 8, 7 ), pMotionProp->pTexture );
...
//	if( m_pSelectMotion )
	{
		...
		pt.y /= 32;
#ifdef __S0126_MOTION_AUTORUN
		int nMaxMotion = m_motionArray.GetSize() / 10 + 1;
		// pt.x >= 0 && pt.x < 10 && 0 <= pt.y && pt.y < nMaxMotion && 
		if( 0 <= pt.x + ( 10 * pt.y ) && pt.x + ( 10 * pt.y ) < m_motionArray.GetSize() )
		{
			MotionProp* pMotionProp = (MotionProp*)m_motionArray.GetAt( pt.x + ( 10 * pt.y ) );
			if( pMotionProp )
			{
				CRect rect( 8 + ( pt.x * 32 ), 7 + ( 32 * pt.y ), 8 + ( ( pt.x +1 ) * 32 ), 7 + ( 32 * ( pt.y + 1) ) );
				ClientToScreen( &point );
				ClientToScreen( &rect );
				g_toolTip.PutToolTip( pt.x + 10000, pMotionProp->szDesc, rect, point, 0 );
			}
		}
#else // __S0126_MOTION_AUTORUN
		int nMaxMotion = m_motionArray.GetSize();
		if( pt.x >= 0 && pt.x < nMaxMotion )
		{
			MotionProp* pMotionProp = (MotionProp*)m_motionArray.GetAt( pt.x );
			if( pMotionProp )
			{
				CRect rect( pt.x * 32 + 8, 7, pt.x * 32 + 8 + 32, 7 + 32);
				ClientToScreen( &point );
				ClientToScreen( &rect );
				g_toolTip.PutToolTip( pt.x + 10000, pMotionProp->szDesc, rect, point, 0 );
			}
...
{ 
	...
	Move( point );
#ifdef __S0126_MOTION_AUTORUN
	int nSize = prj.m_aPropMotion.GetSize() / 10;
	SetWndRect( CRect( point.x, point.y, point.x + ( 32 * 11 ), point.y + 80 + ( 32 * nSize ) ) );
#else // __S0126_MOTION_AUTORUN
	int nSize = prj.m_aPropMotion.GetSize();
	SetWndRect( CRect( point.x, point.y, point.x + (32 * nSize), point.y + 80) );
#endif // __S0126_MOTION_AUTORUN
	MoveParentCenter();
} 
// 처음 이 함수를 부르면 윈도가 열린다.
...
#ifdef __S0126_MOTION_AUTORUN
	int nMaxMotion = m_motionArray.GetSize() / 10 + 1;
	if( 0 <= point.x + ( 10 * point.y ) && point.x + ( 10 * point.y ) < m_motionArray.GetSize() )
	{
		MotionProp* pMotionProp = (MotionProp*)m_motionArray.GetAt( point.x + ( 10 * point.y ) );
		if( pMotionProp == m_pSelectMotion )
		{
			g_WndMng.ObjectExecutor( SHORTCUT_MOTION, pMotionProp->dwID );
		}
	}
#else // __S0126_MOTION_AUTORUN
	int nMaxMotion = m_motionArray.GetSize();
	if( point.x >= 0 && point.x < nMaxMotion )
	{
		MotionProp* pMotionProp = (MotionProp*)m_motionArray.GetAt( point.x );
		if( pMotionProp == m_pSelectMotion )
		{
			g_WndMng.ObjectExecutor( SHORTCUT_MOTION, pMotionProp->dwID );
		}
...
void CWndMotion::OnMouseMove( UINT nFlags, CPoint point )
{
	if( m_pSelectMotion && IsPush())
	{
		...
		m_GlobalShortcut.m_pTexture   = m_pSelectMotion->pTexture;
		m_GlobalShortcut.m_dwData     = (DWORD) m_pSelectMotion;
//		_tcscpy( m_GlobalShortcut.m_szString, m_pSelectMotion->szName );
	}
}
