	...
	if( 1 ) //m_dwListCtrlStyle == WLVS_REPORT )
	{
		...
		for( int i = 0; i < MAX_SKILL_JOB; i++ ) 
		{
			...
			if( dwSkill != NULL_ID )
			{
				...
				nWidth	= ((float)pSkill->dwExp * 100 / (float)prj.m_aExpSkill[pSkill->dwLevel + 1]) * rect.Width() / 100;//rect.Width() * pSkill->dwExp / 10;// : 0 );
				CRect rectTemp = rect; 
				...
				{
					char cSkillExp[16] = {0,};
					sprintf(cSkillExp, "%7.2f %%", (float)pSkill->dwExp * 100 / (float)prj.m_aExpSkill[pSkill->dwLevel + 1] );
					p2DRender->TextOut( rect.left + 113 - size.cx + ax / 2, rect.top + 30 - size.cy, cSkillExp, dwColor);

					// 스킬 스톱버튼 출력
					if( CheckSkill( i ) )
						m_aSkillLevel[ pSkill->bStop ]->Render( p2DRender, CPoint( rect.left + 50 - size.cx + ax / 2, rect.top + 3 ) );
					else
						m_aSkillLevel[ 2 ]->Render( p2DRender, CPoint( rect.left + 50 - size.cx + ax / 2, rect.top + 3 ) );
				}
				else
				{ 
...
void CWndSkillCtrl::OnLButtonDblClk( UINT nFlags, CPoint point )
{
	...
	if( m_pFocusItem )
	{
		...
		if( pCtrl && pCtrl->GetType() != OT_OBJ )
			dwObjId = pCtrl->GetId();
#ifndef __ONLINE
		g_pPlayer->DoUseSkill( m_nJob, m_nCurSelect, dwObjId );
#else	// __ONLINE
		g_DPlay.SendUseSkill( m_nJob, m_nCurSelect, dwObjId );
#endif	// __ONLINE
	}
	//}}AFX
#endif
