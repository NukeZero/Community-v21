...
{ 
	...
	if( idEnemyGuild != NULL_ID )
	{
		...
		{
			Message( "CWndGuildTabWar : 적길드를 찾을수 없음." );
			return;
		}
	} else
	{
...
void CWndGuildTabWar::UpdateData( void )
{
	...
	if( pGuild )
	{
		...
		if( g_Option.m_nLanguage != LANG_KOR )
		{
			...
			strText.Format( "%d Win", pGuild->m_nWin );
			pWndText->SetTitle( strText );		// x 승
			pWndText = GetDlgItem( WIDC_GUILDWARLOSE );
			strText.Format( "%d Lose", pGuild->m_nLose );
			pWndText->SetTitle( strText );		// x 패
			pWndText = GetDlgItem( WIDC_GUILDWARGIVEUP );
			strText.Format( "%d Surrender", pGuild->m_nSurrender );
			pWndText->SetTitle( strText );		// x 회
		}
		else
		{
			pWndText = GetDlgItem( WIDC_GUILDWARWIN );
			strText.Format( "%d 승", pGuild->m_nWin );
			pWndText->SetTitle( strText );		// x 승
			pWndText = GetDlgItem( WIDC_GUILDWARLOSE );
			strText.Format( "%d 패", pGuild->m_nLose );
			pWndText->SetTitle( strText );		// x 패
			pWndText = GetDlgItem( WIDC_GUILDWARGIVEUP );
			strText.Format( "%d 회", pGuild->m_nSurrender );
			pWndText->SetTitle( strText );		// x 회
		}
	}
}
