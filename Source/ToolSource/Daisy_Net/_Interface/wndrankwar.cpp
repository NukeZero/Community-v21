...
{ 
	...
	for( i = m_nCurrentList; i < m_nCurrentList + 10; ++i )
	{
		...
		if( pGuildRank->m_Ranking[nFlag][i].m_dwLogo != 0 )
		{
			...
			CPoint point = CPoint( sx + 45, sy - 5 );
		...
		// 길드명
		p2DRender->TextOut( sx + 80, sy, pGuildRank->m_Ranking[nFlag][i].m_szGuild, dwColor );
		
		// 마스터명
		p2DRender->TextOut( sx + 170, sy, pGuildRank->m_Ranking[nFlag][i].m_szName, dwColor );
		
		p2DRender->TextOut( sx + 280, sy, pGuildRank->m_Ranking[nFlag][i].m_nSurrender, dwColor );
...
{ 
	...
	for( i = m_nCurrentList; i < m_nCurrentList + 10; ++i )
	{
		...
		p2DRender->TextOut( sx + 260, sy, strWin, dwColor );
