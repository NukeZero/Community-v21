...
{
    //lpCreateStruct->style &= ~CBRS_ALL; // keep only the generic window styles
    //lpCreateStruct->style |= WS_CLIPCHILDREN; // prevents flashing
...
{	
	...
	pMember->pWnd=pWnd;
	_tcscpy_s(pMember->szLabel, _countof(pMember->szLabel), lpszLabel);
...
void CSizingTabCtrlBar::OnShowWindow(BOOL bShow, UINT nStatus)
{
