...
//		Error( "file not found: %s", pszFileName );
	...
	//Radian *= 2.0f;
		...
		vPos.x = (int)(sin( a ) * fRadian );	// 여기 (int)캐스팅 바꾸지 말것. (int)안하면 컴터 리붓됨.
		vPos.z = (int)(cos( a ) * fRadian );
		vPos.y = (int) vPos.y;
   ...
   pSound->GetBuffer(pSound->GetFreeBufferIndex())->SetVolume( fVolume );
