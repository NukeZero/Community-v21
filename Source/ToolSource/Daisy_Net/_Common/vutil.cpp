...
BOOL LoadImage( IDirect3DDevice9* pDevice, LPCTSTR lpszFileName, LPIMAGE lpImage ) //LPBYTE* lppData, SIZE* pSize, int* pnImgBit )
{
	TCHAR szFileName[ 128 ] = { NULL, };
	_tcscpy_s( szFileName, _countof(szFileName), lpszFileName );
	_strlwr_s( szFileName, _countof(szFileName) );
	BOOL bResult;
	if( strstr( szFileName, ".tga" ) )
	{
		bResult = LoadTGA( lpszFileName, lpImage );
	}
	else
	if( strstr( szFileName, ".bmp" ) )
	{
		bResult = LoadBMP( lpszFileName, lpImage );
	}
	if( strstr( szFileName, ".png" ) )
	{
		bResult = LoadPNG( pDevice, lpszFileName, lpImage );
	}
	return bResult;
}
BOOL LoadTGA( LPCTSTR lpszFileName, LPIMAGE lpImage ) //LPBYTE* lppData, SIZE* pSize )
{
	...
	CTarga targa;
	bResult = targa.Load( lpszFileName, &lpImage->lpData, &lpImage->size );
#endif
	return bResult;
}
...
BOOL LoadPNG( IDirect3DDevice9* pDevice, LPCTSTR lpszFileName, LPIMAGE lppImage )
{
	...
	D3DLOCKED_RECT LockReck;
	D3DSURFACE_DESC ScrDsc;
	 pTexture->GetLevelDesc(0, &ScrDsc);
	if( ( hr = pTexture->LockRect( 0, &LockReck, 0, 0 ) ) == D3D_OK )// Lock
	{
		int nByte = LockReck.Pitch / ScrDsc.Width;

		lppImage->size.cx = ScrDsc.Width;
		lppImage->size.cy = ScrDsc.Height;

		if(4==nByte)							// 32Bit
		{
			lppImage->nImgBit = IMGBIT_32;
			//DWORD*  pARGB = (DWORD*)rc.pBits;
		}

		else if(2 == nByte)						// 16 Bit
		{
			lppImage->nImgBit = IMGBIT_16;
			//WORD*  pARGB = (WORD*)rc.pBits;
		}
		else if(1 == nByte)						// 8 Bit
		{
			lppImage->nImgBit = IMGBIT_8;
			//BYTE*  pARGB = (BYTE*)rc.pBits;
		}

		lppImage->lpData = new BYTE[ ScrDsc.Width * ScrDsc.Height * 4 ];
		DWORD* pScrPixel = (DWORD*)LockReck.pBits;

		int nByteY = 0;
		for( int y = 0; y < (int)ScrDsc.Height; ++y )
		{
			int nByteX = 0;
			for( int x = 0; x < (int)ScrDsc.Width; ++x )
			{
				int nIndex_DWORD = y * ScrDsc.Width + x;
				int nIndex_Byte = nByteY * ScrDsc.Width + nByteX;

				D3DXCOLOR xcolor = pScrPixel[ nIndex_DWORD ];

				lppImage->lpData[ nIndex_Byte + 0 ] = BYTE( xcolor.b * 255.f );
				lppImage->lpData[ nIndex_Byte + 1 ] = BYTE( xcolor.g * 255.f );
				lppImage->lpData[ nIndex_Byte + 2 ] = BYTE( xcolor.r * 255.f );
				lppImage->lpData[ nIndex_Byte + 3 ] = BYTE( xcolor.a * 255.f );
