...
//    pd3dDevice->SetRenderState( D3DRS_AMBIENT, 0xffffffff );
	...
	if( dwNumTailsToRender == 1 )
		Message( "TailRender : 렌더할 꼬리개수가 넘 작은경우 발생. %d", dwNumTailsToRender );
#endif
...
HRESULT CTailEffectModel::FrameMove( void )
{
	...
	for( int i=0; i< m_vecTail.size(); i++ )
	{
		if( m_vecTail[i].m_nFactor < 10 )
			continue;
			
		m_vecTail[i].m_nFactor -= m_fFadeSpeed;
	}
...
void CTailEffect::Create( int nType, FLOAT fFadeSpeed )
{
	if( m_pPool == NULL )
		Init();
...
// 꼬리 하나를 생성시킴.
//
int	CTailEffect::CreateTail( const D3DXVECTOR3 &vPos1, const D3DXVECTOR3 &vPos2 )
{
	if( m_bActive == FALSE )	return 1;
	if( m_pTexture == NULL )	return 1;
	TAIL *pTail;
...
//
//
HRESULT CTailEffect::FrameMove( void )
{
	if( m_bActive == FALSE )	return S_OK;
	if( m_pTexture == NULL )	return S_OK;
...
//
//---------------------------------------------------------------------
HRESULT CTailEffect::InitDeviceObjects( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR szFileName )
{
	HRESULT hr;
...
HRESULT CTailEffect::RestoreDeviceObjects( LPDIRECT3DDEVICE9 pd3dDevice )
{
	if( m_bActive == FALSE )	return S_OK;
	if( m_pTexture == NULL )	return S_OK;
	if( m_pVB )					
...
HRESULT CTailEffect::ChangeTexture( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR szFileName, int nType )
{
	SAFE_RELEASE( m_pTexture );
	m_nType = nType;
	return InitDeviceObjects( pd3dDevice, szFileName );
...
HRESULT CTailEffect::InvalidateDeviceObjects()
{
    SAFE_RELEASE( m_pVB );
...
HRESULT CTailEffect::Render( LPDIRECT3DDEVICE9 pd3dDevice )
{
	if( m_bActive == FALSE )	return E_FAIL;
	if( m_pTexture == NULL )	return E_FAIL;
	if( m_nMaxTail < 2 )		return E_FAIL;		// 꼬리는 최소 2개노드(버텍스4개)가 있어야 삼각형스트립 2개를 만들어 낼수 있다.
...
{
	int		i;
	CTailEffect	*pTailEffect;

	for( i = 0; i < MAX_TAILEFFECT; i ++ )
	{
		pTailEffect = &m_TailEffects[ i ];
		if( pTailEffect->m_bActive == TRUE )	continue;

		pTailEffect->Create( nType, fFadeSpeed );		// 꼬리메모리 할당하고
		pTailEffect->InitDeviceObjects( pd3dDevice, szFileName );	// 텍스쳐 읽고
		pTailEffect->RestoreDeviceObjects( pd3dDevice );	// 버텍스 버퍼 할당하고.
		m_nMaxType ++;
		return pTailEffect;
	}
...
int		CTailEffectMng::Delete( CTailEffect *pTail )
{
	...
	for( i = 0; i < MAX_TAILEFFECT; i ++ )
	{
		if( &m_TailEffects[i] == pTail )
		{
			m_TailEffects[i].Clear();		// 메모리는 재할당하지 않고 초기화만 시킨다.
			m_nMaxType --;
			return 1;
		}
	}
...
void CTailEffectMng::Process( void )
{
	...
	for( i = 0; i < MAX_TAILEFFECT; i ++ )
	{
		if( m_TailEffects[i].m_bActive == FALSE )	continue;
		m_TailEffects[i].FrameMove();
	}
}
