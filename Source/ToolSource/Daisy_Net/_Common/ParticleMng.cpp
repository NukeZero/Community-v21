...
HRESULT CParticles::InitDeviceObjects( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR szFileName )
{
	...
	return E_FAIL;
  #endif 
	// Create the texture using D3DX - 콜라에서 파티클 테스트 할려고 넣은거다.
	hr = D3DXCreateTextureFromFileEx( pd3dDevice, _T(szFileName), 
									   D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, 
									   D3DPOOL_MANAGED, D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 
									   D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 0, NULL, NULL, &m_pParticleTexture );
	if( hr == E_FAIL )
	{
		Error( "%s 읽기 실패", "Particle.bmp" );
		m_bActive = FALSE;
	}
	return hr;
#endif
...
#else
		case 0:	pParticles->m_fSize = 0.3f;	strcpy( szFileName, "etc_Particle.bmp" );	break;
#endif
