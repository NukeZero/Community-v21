...
void CCommonCtrl::_ProcessWall( void )
{
	...
	#else // __NPP_050308
								pTarget->DropItem( pAttacker );				// 몬스터였다면 아이템 드랍.
	#endif // __NPP_050308
...
void CCommonCtrl::_ProcessAction()
{
	...
	m_dwTrapProcessTime = timeGetTime() + m_CtrlElem.m_dwTrapDelay;
...
void CCommonCtrl::_ProcessTrap()
{
	...
	// 트랩발동시간을 체크하여 발생함
	if( m_dwTrapProcessTime < timeGetTime() )
	{
		CtrlProp* pProp = GetProp();
		if( pProp == NULL )
			return;
		...
		// 링크된 오브젝트(컨트롤) 발동
		if( _tcslen( m_CtrlElem.m_strLinkCtrlKey ) != NULL )
		{
...
void CCommonCtrl::_CreateTrapSkill()
{
	...
	for( int i=0; i<MAX_TRAP; i++ )
	{
		...
		{
			CCtrl* pCtrl = prj.GetCtrl( m_nMoverID );	// 2006.01.05 raiders
...
void CCommonCtrl::DropNPC()
{
	for( int i=0; i<MAX_CTRLDROPMOB; i++ )
	{
		...
		// 선공 몹 생성
		for( j=0; j<m_CtrlElem.m_dwMonActAttack[i]; j++ )
		{
			CObj* pObj	= CreateObj( D3DDEVICE, OT_MOVER, pMoverProp->dwID );
			if( NULL == pObj )	
				return;
