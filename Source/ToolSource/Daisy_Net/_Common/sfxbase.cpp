...
void CSfxPart::AddKeyFrame(WORD nFrame)
{
	...
	if(pPrevKey==NULL || pNextKey==NULL) {
		...
		*(pKey)=*(pPrevKey);
		if(dFrame!=0) {
			pKey->vPos+=(pNextKey->vPos-pPrevKey->vPos)*(nFrame-pPrevKey->nFrame)/dFrame;
			pKey->vPosRotate=(pNextKey->vPosRotate-pPrevKey->vPosRotate)*(nFrame-pPrevKey->nFrame)/dFrame;
			pKey->vRotate+=(pNextKey->vRotate-pPrevKey->vRotate)*(nFrame-pPrevKey->nFrame)/dFrame;
			pKey->vScale+=(pNextKey->vScale-pPrevKey->vScale)*(nFrame-pPrevKey->nFrame)/dFrame;
...
void CSfxPartBill::Render2( D3DXVECTOR3 vPos, WORD nFrame, D3DXVECTOR3 fAngle, D3DXVECTOR3 vScale )
{
	...
	if( dFrame != 0 ) 
	{
		Key.vPos+=(pNextKey->vPos-pPrevKey->vPos)*(nFrame-pPrevKey->nFrame)/dFrame;
		Key.vPosRotate+=(pNextKey->vPosRotate-pPrevKey->vPosRotate)*(nFrame-pPrevKey->nFrame)/dFrame;
		Key.vRotate+=(pNextKey->vRotate-pPrevKey->vRotate)*(nFrame-pPrevKey->nFrame)/dFrame;
...
void CSfxPartBill::RenderOutLine(D3DXVECTOR3 vPos,WORD nFrame,FLOAT fAngle,D3DXVECTOR3 vScale)
{
	...
	if(dFrame!=0) {
		Key.vPos+=(pNextKey->vPos-pPrevKey->vPos)*(nFrame-pPrevKey->nFrame)/dFrame;
		Key.vRotate.z+=(pNextKey->vRotate.z-pPrevKey->vRotate.z)*(nFrame-pPrevKey->nFrame)/dFrame;
		Key.vScale.x+=(pNextKey->vScale.x-pPrevKey->vScale.x)*(nFrame-pPrevKey->nFrame)/dFrame;
		Key.vScale.y+=(pNextKey->vScale.y-pPrevKey->vScale.y)*(nFrame-pPrevKey->nFrame)/dFrame;
		Key.nAlpha+=(pNextKey->nAlpha-pPrevKey->nAlpha)*(nFrame-pPrevKey->nFrame)/dFrame;
	}
