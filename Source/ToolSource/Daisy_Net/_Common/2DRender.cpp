...
void C2DRender::TextOut_EditString( int x,int y, CEditString& strEditString, int nPos, int nLines, int nLineSpace )
{
	...
	if( m_clipRect.RectLapRect( rect ) )
	{
		...
		y += m_ptOrigin.y;
		int nBegin = x;
		m_pFont->DrawText( (float)( x ), (float)( y ), 0xFFFFFFFF, strEditString, nPos, nLines, nLineSpace );
		if( nLines == 0 )
...
//{{AFX
			...
			for( int j = 0; j < nLength; j++ )
			{
				...
				{
					// 문제가 있음. 마지막 문제가 1바이트 word char이다.
					if( j == nLength - 1 )
						continue;
					strHan[ 0 ] = string[ j++ ];
					strHan[ 1 ] = string[ j   ];
				...
				DWORD dwColor = strEditString.m_adwColor[ dwCurOffset ];
				DWORD dwStyle = strEditString.m_abyStyle[ dwCurOffset ];

				if( dwStyle & ESSTY_BOLD )
					m_pFont->DrawText( x + 1, y, dwColor, strHan );
				if( dwStyle & ESSTY_UNDERLINE )
					RenderLine( CPoint( x - m_ptOrigin.x, y + size.cy - m_ptOrigin.y ), CPoint( x + size.cx - m_ptOrigin.x, y + size.cy - m_ptOrigin.y ), dwColor );
...
void C2DRender::TextOut( int x,int y, FLOAT fXScale, FLOAT fYScale, LPCTSTR pszString, DWORD dwColor, DWORD dwShadowColor  )
{
	if( m_pFont )
	{
		...
		if( m_clipRect.RectLapRect( rect ) )
		{
			...
			if( m_pFont->m_nOutLine > 0 )
			{
				m_pFont->DrawText( (float)( x + m_ptOrigin.x + 1 ), (float)( y + m_ptOrigin.y ), 0xff000000, (TCHAR*)pszString);
				m_pFont->DrawText( (float)( x + m_ptOrigin.x - 1 ), (float)( y + m_ptOrigin.y ), 0xff000000, (TCHAR*)pszString);
				m_pFont->DrawText( (float)( x + m_ptOrigin.x  ), (float)( y + m_ptOrigin.y + 1 ), 0xff000000, (TCHAR*)pszString);
				m_pFont->DrawText( (float)( x + m_ptOrigin.x  ), (float)( y + m_ptOrigin.y - 1 ), 0xff000000, (TCHAR*)pszString);

				m_pFont->DrawText( (float)( ( x + m_ptOrigin.x ) + 1 ), (float)( y + m_ptOrigin.y + 1 ), 0xff000000, (TCHAR*)pszString);
				m_pFont->DrawText( (float)( x + m_ptOrigin.x - 1 ), (float)( y + m_ptOrigin.y + 1 ), 0xff000000, (TCHAR*)pszString);
				m_pFont->DrawText( (float)( x + m_ptOrigin.x + 1  ), (float)( y + ( m_ptOrigin.y - 1 ) ), 0xff000000, (TCHAR*)pszString);
				m_pFont->DrawText( (float)( x + m_ptOrigin.x - 1 ), (float)( y + ( m_ptOrigin.y - 1 ) ), 0xff000000, (TCHAR*)pszString);
			}
...
void C2DRender::TextOut( int x,int y, int nValue, DWORD dwColor, DWORD dwShadowColor )
{
	if( m_pFont )
	{
		TCHAR szString[ 32 ] = { NULL, };
		_itot_s( nValue, szString, _countof(szString), 10 );
...
//			if( m_pFont->m_nOutLine > 0 )
//			{
//				m_pFont->DrawText( (float)( x + m_ptOrigin.x + 1 ), (float)( y + m_ptOrigin.y ), 0xff000000, (TCHAR*)szString);
//				m_pFont->DrawText( (float)( x + m_ptOrigin.x - 1 ), (float)( y + m_ptOrigin.y ), 0xff000000, (TCHAR*)szString);
//				m_pFont->DrawText( (float)( x + m_ptOrigin.x  ), (float)( y + m_ptOrigin.y + 1 ), 0xff000000, (TCHAR*)szString);
//				m_pFont->DrawText( (float)( x + m_ptOrigin.x  ), (float)( y + m_ptOrigin.y - 1 ), 0xff000000, (TCHAR*)szString);
//
//				m_pFont->DrawText( (float)( ( x + m_ptOrigin.x ) + 1 ), (float)( y + m_ptOrigin.y + 1 ), 0xff000000, (TCHAR*)pszString);
//				m_pFont->DrawText( (float)( x + m_ptOrigin.x - 1 ), (float)( y + m_ptOrigin.y + 1 ), 0xff000000, (TCHAR*)pszString);
...
{
//#if defined( __CLIENT ) || defined( __DAISY )
	//HRESULT hr = pd3dDevice->CreateTexture( nWidth, nHeight, 
	//	0, Usage/*D3DUSAGE_RENDERTARGET*/, /*g_pD3dApp->m_d3dpp.BackBufferFormat*/Format, Pool, &m_pTexture, NULL );
...
/*
CSize CTexture::ComputeSize( CSize size )
{
	// 사이즈를 2의 자승으로 바꾸기 
	WORD dwTemp = 0x8000;
	for( int i = 0; i < 16; i++ )
...
*/
//LoadTextureFromRes( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR pFileName, D3DCOLOR d3dKeyColor )
/*
BOOL CTexture::LoadTextureFromRes( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR pFileName, D3DCOLOR d3dKeyColor, BOOL bMyLoader )
{
	...
	return LoadTexture( pd3dDevice, pFileName, d3dKeyColor, bMyLoader );
#endif
}
*/
BOOL CTexture::LoadTexture( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR pFileName, D3DCOLOR d3dKeyColor, BOOL bMyLoader )
{
...
#endif
	SIZE size;
	TCHAR szFileName[ 128 ] = { NULL, };
	DWORD dwOffsetDest = 0;
	DWORD dwOffsetSrc = 0;
	_tcscpy_s( szFileName, _countof(szFileName), pFileName );
	_tcslwr_s( szFileName, _countof(szFileName) );
	...
	if( bMyLoader == FALSE ) // dds only
	{
		...
		{
#ifdef __ONLINE
			LPCTSTR szErr = Message( "CTexture::LoadTexture : %s read error", pFileName );
			ADDERRORMSG( szErr );
#endif
			return FALSE;		
		}
	...
	if( strstr( szFileName, ".bmp" ) )
	{
		...
		//#define BMP32BIT
			...
			Message( "CTexture::LoadTexture : %s read error", pFileName );
	#endif
...
//{{AFX
	if( strstr( szFileName, ".bmp" ) )
	{
		IMAGE image;
		LPBYTE lpData = NULL;
		if( LoadBMP( pFileName, &image ) == TRUE ) //&lpData, &m_size ) == TRUE )
		{
			lpData = image.lpData;
			m_size = image.size;
			size = m_size;
			//m_size = ComputeSize( m_size );
			AdjustSize( &m_size );
//#define BMP32BIT
#ifdef BMP32BIT
			CreateTexture( pd3dDevice, m_size.cx, m_size.cy, D3DX_DEFAULT, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED );
			...
			{
#ifdef BMP32BIT
				LPBYTE pDest = (LPBYTE)lockedRect.pBits;
#else
				LPWORD pDest = (LPWORD)lockedRect.pBits;
#endif
				for(int y = 0; y < size.cy; y++ )
				{
					...
					{
						DWORD dwOffsetSrc = ( y * size.cx + x ) * 3;
						BYTE byData1 = lpData[ dwOffsetSrc + 0 ]; 
						BYTE byData2 = lpData[ dwOffsetSrc + 1 ]; 
						BYTE byData3 = lpData[ dwOffsetSrc + 2 ]; 
#ifdef BMP32BIT
						DWORD dwOffsetDest = ( y * m_size.cx + x ) * 4;
						if( byData1 == 255 && byData2 == 0 && byData3 == 255 )
						{
							pDest[ dwOffsetDest + 0 ] = 0;
							pDest[ dwOffsetDest + 1 ] = 0;
							pDest[ dwOffsetDest + 2 ] = 0;
							pDest[ dwOffsetDest + 3 ] = 0x00;
						}
						else
						{
							pDest[ dwOffsetDest + 0 ] = byData1;
							pDest[ dwOffsetDest + 1 ] = byData2;
							pDest[ dwOffsetDest + 2 ] = byData3;
							pDest[ dwOffsetDest + 3 ] = 0xff;
						}
#else
						DWORD dwOffsetDest = ( y * m_size.cx + x );
						if( byData1 == 255 && byData2 == 0 && byData3 == 255 )
							pDest[ dwOffsetDest] = 0x0000;
						else
							pDest[ dwOffsetDest] = 0x8000 | ( ( byData3 >> 3 ) << 10 ) | ( ( byData2 >> 3 ) << 5 ) | ( byData1 >> 3 );
			...
			Message( "CTexture::LoadTexture : %s읽기 실패", pFileName );
#endif
	...
	if( strstr( szFileName, ".tga" ) )
	{
		LPBYTE lpData = NULL;
		CTarga targa;
		//targa.Load( pFileName, lpData, &size );
		//if( LoadBMP( pFileName, &lpData, &m_size ) == TRUE )
		if( targa.Load( pFileName, &lpData, &m_size ) == TRUE )
		{
			size = m_size;
			//m_size = ComputeSize( m_size );
...
//				return FALSE;
			...
			if( ( hr = m_pTexture->LockRect( 0, &lockedRect, 0, 0 ) ) == D3D_OK )// Lock
			{
				...
				for(int y = 0; y < size.cy; y++ )
				{
					for(int x = 0; x < size.cx; x++)
					{
						DWORD dwOffsetSrc = ( y * size.cx + x ) * 4;
						BYTE byData1 = lpData[ dwOffsetSrc + 0 ]; 
						BYTE byData2 = lpData[ dwOffsetSrc + 1 ]; 
						BYTE byData3 = lpData[ dwOffsetSrc + 2 ]; 
						BYTE byData4 = lpData[ dwOffsetSrc + 3 ]; 
...
//}}AFX
	...
	if( strstr( szFileName, ".png" ) )
	{
		...
		if( SUCCEEDED ( D3DXCreateTextureFromFileEx( pd3dDevice, szFileName,  D3DX_DEFAULT, D3DX_DEFAULT, 1, 0, D3DFMT_UNKNOWN,
			D3DPOOL_MANAGED, 0x0000001, 0x0000001, 0x00FFFFFF, &pImgInf, NULL, &m_pTexture )  ) )
		{
			m_size.cx = pImgInf.Width;
			m_size.cy = pImgInf.Height;
		...
		/*	size.cx = pImgInf.Width;
		size.cy = pImgInf.Height;
		m_size = size;*/
...
BOOL CTexturePack::LoadScript( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR pszFileName )
{
	...
	{
		Message( "%s not found", pszFileName );
		return FALSE;
	}
	...
	do {
		...
		{	
			scanner.GetToken();	
			TCHAR strFileName[128] = { NULL, };
			_tcscpy_s( strFileName, _countof(strFileName), scanner.token );	
			D3DCOLOR d3dKeyColor = scanner.GetHex();
			// 여기서 텍스춰 생성 
			//D3DUtil_CreateTexture( g_neuz.GetDevice(), _T(strFileName), &m_pTexture, D3DFMT_A1R5G5B5, d3dKeyColor );
...
CTexture* CTextureMng::AddTexture( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR pFileName, D3DCOLOR d3dKeyColor, BOOL bMyLoader )
{
	...
	if(pd3dDevice == NULL) return NULL;
...
//	else nAlpha = 255 - ( 255.0f / 80 ) * ( m_nFrame - 40 );
	...
	if( m_nAttribute < 4 ) { // 숫자인 경우
		...
		for( int i = 0; i < nLength; i++ )
		{
			...
			{
				textPackNum->Render( &g_Neuz.m_2DRender, CPoint( fX, fY ), nIndex ,(DWORD)nAlpha,fScaleX,fScaleY);
			} 
			else
			{
			...
			fX+=textPackNum->m_ap2DTexture[nIndex].m_size.cx*0.5*fScaleX;
		}
	}
	else 
	if( m_nAttribute == 4 ) // miss인 경우
	{
		fX-=textPackNum->m_ap2DTexture[10].m_size.cx*0.5/2*fScaleX;
		textPackNum->Render( &g_Neuz.m_2DRender, CPoint( fX, fY ), 10 ,nAlpha,fScaleX,fScaleY);
	}
	else 
	if( m_nAttribute == 5 ) // good인 경우
	{
		fX-=textPackNum->m_ap2DTexture[11].m_size.cx*0.5/2*fScaleX;
		textPackNum->Render( &g_Neuz.m_2DRender, CPoint( fX, fY ), 11 ,nAlpha,fScaleX,fScaleY);
	}
	else 
	if( m_nAttribute == 6 ) // nice인 경우
	{
		fX-=textPackNum->m_ap2DTexture[12].m_size.cx*0.5/2*fScaleX;
		textPackNum->Render( &g_Neuz.m_2DRender, CPoint( fX, fY ), 12 ,nAlpha,fScaleX,fScaleY);
	}
	else 
	if( m_nAttribute == 7 ) // yeah~인 경우
	{
		fX-=textPackNum->m_ap2DTexture[13].m_size.cx*0.5/2*fScaleX;
		textPackNum->Render( &g_Neuz.m_2DRender, CPoint( fX, fY ), 13 ,nAlpha,fScaleX,fScaleY);
	}
