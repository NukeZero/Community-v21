...
void CShip::Control( void )
{
	...
	BOOL bLeft = g_bKeyTable[ 'A' ];
	BOOL bRight = g_bKeyTable[ 'D' ];
	BOOL bUp = g_bKeyTable[ 'S' ];
	BOOL bDown = g_bKeyTable[ 'W' ];
...
void CShip::Process( FLOAT fElapsedTime )
{
	...
	{
		g_nFlySpeed = (fLenSq * 60.0f) * 60.0f * 60.0f;
		g_nFlySpeed /= 200.0f;
	}
...
//
//
void CShipLoop::Process( FLOAT fElapsedTime )
{
	D3DXMatrixInverse( GetInvTM(), NULL, &m_matWorld );		// 좌표변환이 이뤄지기전 매트릭스로 역행렬을 구해놔야 한다.
