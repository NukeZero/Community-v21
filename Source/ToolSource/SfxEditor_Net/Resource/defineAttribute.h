...
#define DST_SPEED_PENALTY           49  // SpeedPenalty
#define DST_CHR_STEALHP_IMM         50  // 대상으로부터 지정된 양의 hp를 흡수
#define DST_ABRASION                51  // Abrasion
#define DST_ADJROLL                 52  // AdjRoll
#define DST_AP                      53  // 최대치까지 상승하게 되면 공격이 저지된다.(연속공격이 저지되며 ReAttackDelay 적용)	<-사용중
#define DST_GROGGY                  54  // 피해동작 발생에 필요한 수치(공격이 정지되면서 피해동작을 취하게 된다. ReAttackDelay 동시 적용)
...
#define DST_REATTACKDELAY           67  // 재공격 속도를 변경
...
#define MAX_ADJPARAMARY             78

// ADJPARAMARRAY에는 추가되지 않고, 현재값의 설정과 반환( GetPointParam, SetPointParam )에 사용하기 위한 식별자
#define	DST_GOLD			(MAX_ADJPARAMARY + 1)
#define	DST_PXP				(MAX_ADJPARAMARY + 2)
#define DST_RESIST_ALL     	    81							// Resist ALL - (MAX_ADJPARAMARY + 3)과 같은 방식은 프로퍼티에서 인식하지 못함.
#define DST_STAT_ALLUP	   	    82      //모든 Status를 +1만큼 올려준다.
#define DST_HPDMG_UP	   	    83 
#define DST_DEFHITRATE_DOWN	    84
#define DST_CURECHR             85		// DST_CHRSTATE 상태 제거
#define DST_HP_MAX_RATE			86		// DST_HP_MAX 퍼센트 버전
#define DST_MP_MAX_RATE			87		// 동일 
#define DST_FP_MAX_RATE			88		// 동일
#define DST_HP_RECOVERY_RATE	89  // HPRecovery Rate
#define DST_MP_RECOVERY_RATE	90  // MPRecovery Rate
#define DST_FP_RECOVERY_RATE	91  // FPRecovery Rate
#define	DST_LOCOMOTION			92	// JUMP & SPEED
