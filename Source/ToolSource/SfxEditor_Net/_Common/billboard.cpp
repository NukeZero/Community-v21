...
HRESULT CBillboard::RestoreDeviceObjects()
{
	...
	{
		fxsize = m_pTexture->m_size.cx;
		fysize = m_pTexture->m_size.cx;
	}
	else
	{
		fxsize = m_pTexture->m_size.cy;
		fysize = m_pTexture->m_size.cy;
	}
...
CAniBill::CAniBill()
{
	m_nModelType = 0;// MODELTYPE_ANIMATED_BILLBOARD;
}
CAniBill::~CAniBill()
{
}
void CAniBill::AddFrame( FLOAT fAddFrame )
{
	SetFrame( m_fFrameCurrent + fAddFrame );
}
void CAniBill::SetFrame( FLOAT fFrame )
{	
	m_fFrameCurrent = fFrame;
}
void CAniBill::FrameMove()
{
	AddFrame( 0.2f );

	CBillboard* pBillboard = (CBillboard*) m_pModel;

	if( (int)m_fFrameCurrent >= (int)pBillboard->m_pBillboard->dwFrame )
	{
		m_fFrameCurrent = 0.0f;
		m_pObj->MessageFromModel( MODELMSG_ENDFRAME );
		/*
		switch( m_pBillboard->dwAnimationMethod )
		{
		case 0:	pAnimation->m_fFrameCurrent = 0.0f; break; // 루핑 
		case 1: m_pObj->MessageFromModel( MODELMSG_DESTROYMODEL ); break;
		}
		*/
	}
}
BOOL CAniBill::Render( LPDIRECT3DDEVICE9 pd3dDevice, const D3DXMATRIX* pmatWorld )
{
	m_pModel->m_fFrameCurrent = m_fFrameCurrent;
	m_pModel->SetBlendFactor(GetBlendFactor());
	return m_pModel->Render( pd3dDevice );
}