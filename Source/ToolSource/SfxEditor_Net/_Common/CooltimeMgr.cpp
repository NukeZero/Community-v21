...
#else	// !__ITEMCOOLTIME_VER2_050308
		switch( pItemProp->dwID )
		{
		case II_GEN_FOO_PIL_YELLOW:	
		case II_GEN_FOO_PIL_BLUE:	
		case II_GEN_FOO_PIL_RED:
		case II_GEN_FOO_PIL_GOLD:
			dwCooltimeItemGroup = 1;
			break;
		}
...
{
	ASSERT( dwGroup > 0 );
	return timeGetTime() > GetTime( dwGroup );		// 재사용 시각을 넘으면 사용가능 
}
