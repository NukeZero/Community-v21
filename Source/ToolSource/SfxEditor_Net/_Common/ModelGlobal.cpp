...
HRESULT CTextureSurface::Create( LPDIRECT3DDEVICE9 pd3dDevice, D3DFORMAT backBufferFormat, int nWidth, int nHeight, BOOL bDepth )
{
...
#if 1
	{
		...
		vTris[1].x = nWidth;	vTris[1].y = 0;		vTris[1].z = 0;		vTris[1].w = 1.0f;		
		vTris[1].tu = 1.0f;		vTris[1].tv = 0;		// 우상귀
		
		vTris[2].x = nWidth;	vTris[2].y = nHeight;		vTris[2].z = 0;		vTris[2].w =  1.0f;
		vTris[2].tu = 1.0f;		vTris[2].tv = 1.0f;		// 우하귀
		
		vTris[3].x = 0;			vTris[3].y = nHeight;		vTris[3].z = 0;		vTris[3].w = 1.0f;
...
//		LPDIRECT3DVERTEXBUFFER9		m_pd3dVB;
		...
		if( FAILED(hr) )
		{
			Message( "CTextureSurface::Create : 락 실패" );
			return E_FAIL;
		}
...
void CTextureSurface::BeginScene( void )
{
	...
	if( FAILED(hr) )
		Message( "CTextureSurface:BeginScene : %s", DXGetErrorString9(hr) );
}
...
void CTextureSurface::RenderTargetBlur( LPDIRECT3DDEVICE9 pd3dDevice, CTextureSurface *pDst)
{
	...
	if( 0 )
	{
		...
		vTris[1].x = pDst->m_nWidth-0.0;	vTris[1].y = -0.0;		vTris[1].z = 0;		vTris[1].w = 1.0f;		
		vTris[1].tu = 1.0f;		vTris[1].tv = 0.0;		// 우상귀
		
		vTris[2].x = pDst->m_nWidth-0.0;	vTris[2].y = pDst->m_nHeight-0.0;		vTris[2].z = 0;		vTris[2].w =  1.0f;
		vTris[2].tu = 1.0f;		vTris[2].tv = 1.0f;		// 우하귀
		
		vTris[3].x = -0.0;			vTris[3].y = pDst->m_nHeight-0.0;		vTris[3].z = 0;		vTris[3].w = 1.0f;
		vTris[3].tu = 0.0f;		vTris[3].tv = 1.0f;		// 좌하귀
...
//	m_Surface[3].Create( pd3dDevice, backBufferFormat, nWidth, nHeight );
	...
	if( FAILED(hr) )
	{
		Message( "blur.vs 읽기 실패" );
	}
	hr = g_BlurPS.Create( pd3dDevice, "blur.ps" );
	if( FAILED(hr) )
	{
		Message( "blur.ps 읽기 실패" );
	}
#else
