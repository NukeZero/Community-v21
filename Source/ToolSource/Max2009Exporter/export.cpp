...
//Added by sun BEGIN---------------------------------------------------------------------------
#define ID_Physique "*PHYSIQUE"
#define ID_Physique_NV "*PHYSIQUE_NUMVERTEXASSIGNMENT"
#define ID_Physique_NodeName "*PHYSIQUE_NONBLENDED_RIGIDTYPE"
#define ID_Physique_BLENDED_LIST "*PHYSIQUE_BLENDED_RIGIDTYPE_LIST"

TCHAR buff[256];
BOOL	g_bWarning = FALSE;		// 전역변수가 클리어 되지 않는다. 시작할때 클리어 해주자
BOOL	g_bWarning2 = FALSE;	// Identity검사 에러 
...
void AsciiExp::ExportPhysiqueData(INode* node, Modifier* mod)
{
	...
	float veri = (float)ver/10.0f;
	fprintf(pStream,"\t\tVersion\t%.2f\n",veri);
	//Version 체크를 하고 출력...
		...
		if( vi )
		{
			...
			switch (type)
			{
				case RIGID_BLENDED_TYPE: 
					...
					fprintf(pStream,"\t\t%s %d {\n",ID_Physique_BLENDED_LIST,i);
					fprintf(pStream,"\t\t\t*PHYSIQUE_VERTEXASSIGNMENT_NUMNODE\t%d\n",rb_vtx->GetNumberNodes());
					fprintf(pStream,"\t\t\t*PHYSIQUE_VERTEXASSIGNMENT_NODELIST {\n",rb_vtx->GetNumberNodes());
...
//						totalWeight += weight;
						...
						fprintf(pStream,"\t\t\t\t*PHYSIQUE_VERTEXASSIGNMENT_NODE %d %f \"%s\"\n",x,normalizedWeight,nodeName);

				
			//		*PHYSIQUE_VERTEXASSIGNMENT_NODE 0  0.666667  "Bip01 R Clavicle"
				//	*PHYSIQUE_VERTEXASSIGNMENT_NODE 1  0.333333  "Bip01 R UpperArm"
...
//					normalizedWeight = r_vtx-						rb_vtx->GetWeight(x, &weight);
					
					totalWeight += 1.0f;

					offsetVector = r_vtx->GetOffsetVector();
					fprintf(pStream,"\t\t%s\t%d\t\"%s\"\n",ID_Physique_NodeName,i,nodeName);
...
//	fprintf(pStream,"%s\t}\n", indent.data());
	...
	fprintf(pStream,"%s\t%s\t%d {\n", indent.data(), ID_SKIN_VERTICES, skinMC->GetNumPoints() );
	for(i = 0 ; i < skinMC->GetNumPoints() ; i++)
	{
		//print the vertex number
		fprintf(pStream,"%s\t\t%s\t%s %d\t{\n", indent.data(), ID_SKIN_VERTEX_NUM, Format(i), skinMC->GetNumAssignedBones(i) );
		for(int j = 0 ; j < skinMC->GetNumAssignedBones(i) ; j++)
		{
			...
			fprintf(pStream, "%s\t\t\t%s\t\"%s\"\t%s\n", indent.data(), ID_SKIN_BONE_NAME, nodename.data(), Format(weight));
...
void AsciiExp::ExportGeomObject(INode* node, int indentLevel)
{
	...
	//Added Sun BEGIN------------------------------------------------------------
	char szNodeName[128];
	strcpy( szNodeName, FixupName(node->GetName()) );	// 오브젝트 이름
	strlwr(szNodeName);		// 오브젝트명을 소문자로 바꿈.
	
	// 뼈대/특수목적오브젝이 아닌일반 오브젝트중에서 히든된건 출력안함.
	if( (strncmp(szNodeName, "bip", 3) != 0 && strncmp(szNodeName, "bone", 4) != 0) &&
		szNodeName[0] != '#' )
	{
		if( node->IsNodeHidden() != 0 )		// hidden된 오브젝트는 출력안함.
		{
			return;
		}
	}

	BOOL	bCheck = FALSE;		// 말머리 검사여부
	Modifier *phyMod=FindPhysiqueModifier(node);
	INode* pParent = node->GetParentNode();
	if( m_nMaxUnderline < 2 )	// max파일명에 _가 2개이상이면 모션파일로 간주. 오브젝트말머리를 검사하지 않는다.
	{
		if( phyMod )							// 피지크가 있는 오브젝트중
		{
			bCheck = TRUE;		// 피지크오브젝트는 검사해야함
		} else
		// 피지크가 없는것(일반,링크)
		{
			if( pParent->IsRootNode() == 0 )		// 부모가 있고 부모가 바에피드는 링크.  검사해야함
			{
				char	str[256], *str2;
				strcpy( str, FixupName(pParent->GetName()) );
				str2 = strlwr( str );		// 부모이름 소문자 변환
				if( strncmp(str2, "bip", 3) == 0 && strncmp(str2, "bone", 4) == 0 )		// 바이페드에 링크된것이라면 말머리 검사해야함.
				{
					bCheck = TRUE;
				}
			}
	...
	//Added Sun BEGIN------------------------------------------------------------
	char* szName;
	strcpy( buff, FixupName(node->GetName()) );
	szName = strlwr( buff );
	if( szName[0] == '_' )	
	{
		return;		// 이름의 앞자가 _이면 출력안함
	}
		 
	if( g_bWarning == FALSE && bCheck )
	{
		if( strncmp(szName, "item_", 5) != 0 &&			// 검사대상중 item_도 아니고  bip도 아니면
			strncmp(szName, "part_", 5) != 0 &&
			strncmp(szName, "mvr_", 6) != 0 &&
			strncmp(szName, "bip", 3)   != 0 &&
			strncmp(szName, "bone", 4)   != 0 )
		{
			Error( "경고:오브젝트이름의 말머리가 잘못되었다. %s", szName );
			g_bWarning = TRUE;		// 경고는 한번만 내고 끝내게..
		}
	}
	BOOL	bBiped = FALSE;
	if( strncmp( szName, "bip", 3 ) == 0 || strncmp( szName, "bone", 4 ) == 0 )
		bBiped = TRUE;
	//Added Sun END------------------------------------------------------------
	...
	if( row0.x == 1.0f && fabs(row0.y) < 0.0001f && fabs(row0.z) < 0.0001f )
		if( fabs(row1.x) < 0.0001f && row1.y == 1.0f && fabs(row1.z) < 0.0001f )
			if( fabs(row2.x) < 0.0001f && fabs(row2.y) < 0.0001f && row2.z == 1.0f )
//	if( row0.x = 1.0f && fabsrow0.y == 0 && row0.z == 0 )
//		if( row1.x == 0 && row1.y == 1.0f && row1.z == 0 )
//			if( row2.x == 0 && row2.y == 0 && row2.z == 1.0f )
				bIdentity = TRUE;
	if( bBiped )	// 만일 node가 바이페드면 Identity안되어 있어도 된다.
		bIdentity = TRUE;
	if( pParent->IsRootNode() == 0 )	// 부모가 있다면 Identity안되어 있어도 된다.
		bIdentity = TRUE;

//	Error( "%d %d", g_bWarning2, bIdentity );
	if( g_bWarning2 == FALSE && bIdentity == FALSE )
	{
		Error( "%s : Reset Transform/Scale이 되어 있지 않다.\r\n\r\n" \
			   "     오브젝트를 모두 선택후 Hierarchy->Adjust Transform->Reset Transform/Scale을 누르기 바란다.\r\n" \
			   "     만약, 이미 Reset을 했는데도 이 메시지가 계속 나온다면 그저 미안하게 생각한다. -_-;;;;;", szName );
		g_bWarning2 = TRUE;
	}
	//Added Sun END------------------------------------------------------------
...
void AsciiExp::ExportLightSettings(LightState* lightstate, GenLight* light, TimeValue t, int indentLevel)
{
	TSTR indent = GetIndent(indentLevel);
	...
	fprintf(pStream,"%s\t\t%s %s\n", indent.data(), ID_LIGHT_COLOR, Format(lightstate->color));
	fprintf(pStream,"%s\t\t%s %s\n", indent.data(), ID_LIGHT_INTENS, Format(lightstate->intens));
	fprintf(pStream,"%s\t\t%s %s\n", indent.data(), ID_LIGHT_ASPECT, Format(lightstate->aspect));
	
	if (lightstate->type != OMNI_LIGHT) {
		fprintf(pStream,"%s\t\t%s %s\n", indent.data(), ID_LIGHT_HOTSPOT, Format(lightstate->hotsize));
		fprintf(pStream,"%s\t\t%s %s\n", indent.data(), ID_LIGHT_FALLOFF, Format(lightstate->fallsize));
	}
	if (lightstate->type != DIR_LIGHT && lightstate->useAtten) {
		fprintf(pStream,"%s\t\t%s %s\n", indent.data(), ID_LIGHT_ATTNSTART, Format(lightstate->attenStart));
		fprintf(pStream,"%s\t\t%s %s\n", indent.data(), ID_LIGHT_ATTNEND,	Format(lightstate->attenEnd));
	}
...
void AsciiExp::ExportNodeTM(INode* node, int indentLevel)
{
	...
	int nFlip = DumpMatrix3(&pivot, indentLevel+2);
	if( nFlip )
	{
		strcpy( buff, FixupName(node->GetName()) );
		Error( "경고:오브젝트 %s의 스케일이 마이너스값이다. 리셋트랜스폼을 해줄것. ", buff );
	}
...
int AsciiExp::DumpMatrix3(Matrix3* m, int indentLevel)
{
	Point3 row;
	TSTR indent = GetIndent(indentLevel);
...
void AsciiExp::ScaleBiped(INode* node, int scale)
{
	if (node->IsRootNode()) return;
//#define __STRIP_BIPED_SCALE__
#ifndef __STRIP_BIPED_SCALE__
         ...
         (c->ClassID() == FOOTPRINT_CLASS_ID))
    {
		...
		if (scale != 0) ScaleBiped(node->GetParentNode(), 0);  //only recurse one level

    //End of biped specific code
#endif //__STRIP_BIPED_SCALE__
