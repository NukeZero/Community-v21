...
public:
	...
	//Added by sun---------------------------------------------------------------
	BOOL	ExportSkeletalDeformer(INode *node, int indentLevel);
	void	ExportPhysiqueData(INode* node, Modifier* mod);
	void	ExportSkinData(INode* node, Modifier* mod, int indentLevel);
	bool	ExportPhysiqueInit(INode* node);
	...
	int 	DumpMatrix3(Matrix3* m, int indentLevel);
...
private:
	...
	int			nCurNode;
	TCHAR		szFmtStr[16];
	int			m_nMaxUnderline;		// max파일명내의 언더라인(_)개수.
