...
//		vPosDest.y += 1.0f;
	...
	case XI_NAT_WASTART01:	// 천사의 날개 시전 효과.
		pObj = new CSfx();
		CreateSfx( pd3dDevice, XI_NAT_WINGANGEL01, vPosSrc, idSrc, vPosDest, idDest, 60 * 2 );		// 날개 2분동안 출력.
		break;
	case XI_NAT_CUPITSTART01:	// 큐피트날개 시전 효과.
		pObj = new CSfx();
		CreateSfx( pd3dDevice, XI_NAT_WINGANGEL01, vPosSrc, idSrc, vPosDest, idDest, 60 * 5 );		// 날개 5분동안 출력.
		break;
...
#else //__ACROBAT_0504
		pObj = new CSfxPartsLink();
#endif //__ACROBAT_0504
		break;
		
#ifdef __YTREEDAY_EVENT
	case XI_EVE_EVENT_FAIRYLY:
		pObj = new CSfxFixed();
		break;
#endif //__YTREEDAY_EVENT
...
CAIInterface* CreateAIInterface( CObj* pObj, DWORD dwAIInterface )
{
	...
	{
		case AII_MOVER:			pAI = new CAIHuman( pObj ); break;
		case AII_MONSTER:		pAI = new CAIMonster( pObj ); break;
		case AII_HUMAN:			pAI = new CAIHuman( pObj ); break;
		case AII_CLOCKWORKS:	pAI = new CAIClockWorks( pObj );	break;
		case AII_PET:			pAI = new CAIPet( pObj );		break;
	}
#endif
	return pAI;
}
