...
BOOL CProject::LoadPropItem( LPCTSTR lpszFileName, CFixedArray< ItemProp >* apObjProp )
{
	...
	{
#ifdef __CLIENT
		MessageBox( g_Neuz.GetSafeHwnd(), "프로퍼티 읽기실패 : Item", "프리프", MB_OK );
		ADDERRORMSG( "프로퍼티 읽기실패 : Item" );
#endif //__CLIENT
		return FALSE;
	}
...
//		itemProp.dwHeelH	= scanner.GetNumber(  );      
		...
		{
			itemProp.dwShopAble	= 1;
			if( itemProp.dwItemRare == (DWORD)-1 )
				itemProp.dwItemRare		= itemProp.dwItemLV;
		}
 #endif	// __SCHOOL0722
 #endif	// __WORLDSERVER
