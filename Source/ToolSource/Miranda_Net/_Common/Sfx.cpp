// Sfx.cpp: implementation of the CSfx class.
//
//////////////////////////////////////////////////////////////////////
...
void CSfxGenNormalDmg01::Process( FLOAT fElapsedTime )
{
...
void CSfxGenSuperDmg01::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
...
void CSfxGenSkillDmg01::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	if(m_pSfxObj->Process())
	{
...
void CSfxGenMonsterDmg01::Process( FLOAT fElapsedTime )
{
	m_nFrame++; // 한프레임 진행시키고
...
void CSfxGenRestoration01::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
...
void CSfxGenCure::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
	if(pObjSrc) SetPos(pObjSrc->GetPos());
...
void CSfxGenIncrease01::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
	if(pObjSrc) SetPos(pObjSrc->GetPos());
...
void CSfxGenLevelUp::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
	if(pObjSrc) {
...
void CSfxTroStretching01::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
	if(pObjSrc) {
...
void CSfxTroStretching02::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	if( m_nSec == 0 )
	{
...
void CSfxTroBlitz::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	if( m_nSec == 0 )
	{
...
void CSfxGenLogin::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
	if(pObjSrc) SetPos(pObjSrc->GetPos());
...
void CSfxGenWarp::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
	if(pObjSrc) SetPos(pObjSrc->GetPos());
...
void CSfxGenPcDie::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
	if(pObjSrc) SetPos(pObjSrc->GetPos());
...
void CSfxGenMonsterSpawn::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
	if(pObjSrc) SetPos(pObjSrc->GetPos());
...
void CSfxGenMoveMark::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	g_pMoveMark=this;
	if(m_pSfxObj->Process())
...
void CSfxGenWaterCircle::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	if(m_pSfxObj->Process())
	{
...
void CSfxGenRainCircle::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	if(m_pSfxObj->Process())
	{
...
void CSfxGenWaterCrown::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	if(m_pSfxObj->Process())
	{
...
void CSfxItemWandAtkAir::Process( FLOAT fElapsedTime )
{
#ifndef __BEAST
...
//		AngleToVector( &vAcc, fAngXZ, fAngH, 0.009f );		// 실시간으로 목표쪽으로 향하는 힘.
...
{
	//m_SfxObj2.SetSfx("wandattack2"); // 맞는 순간의 SFX를 따로 로딩한다.
	m_SfxObj2.SetSfx( "sfx_HitWand01" ); // 맞는 순간의 SFX를 따로 로딩한다.
	m_bHit = FALSE; // 아직 안맞았다.
	m_fCenter = 0;
...
void CSfxItemWandAtk1::Process( FLOAT fElapsedTime )
{
...
//		CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
//		if( pObjSrc ) 
//			m_pSfxObj->m_vPos = pObjSrc->GetPos() + D3DXVECTOR3(.0f,1.0f,.0f);
//	}
	CMover* pObjDest = (CMover*)prj.GetCtrl( m_idDest );
	if( pObjDest ) 
	{
...
int CSfxItemRangeAtk1::SetSfx( LPDIRECT3DDEVICE9 pd3dDevice, int nIndex, 
								 D3DXVECTOR3& vPosSrc, OBJID idSrc, D3DXVECTOR3& vPosDest, OBJID idDest, int nSec )
{
	CSfx::SetSfx( pd3dDevice, nIndex, vPosSrc, idSrc, vPosDest, idDest, nSec );		// 기본초기화 먼저하고.
...
//
//
void CSfxItemRangeAtk1::ShootSfx( float fAngXZ, float fAngY, float fSpeed )
{
	AngleToVector( &m_vDelta, fAngXZ, fAngY, fSpeed );		// 초기 발사힘.	- 완드가 향하고 있는 방향으로 쎄게 방출.
}

	
void CSfxItemRangeAtk1::Process( FLOAT fElapsedTime )
{
#ifndef __BEAST
	FLOAT	fRadius = 0.6f;		// 타겟의 반경.
	CMover* pObjDest = prj.GetMover( m_idDest );		// 타겟측의 포인터.
	...
	{ 
		...
		fAngH = -fAngH;
		D3DXVECTOR3	vAcc;
		AngleToVector( &vAcc, fAngXZ, fAngH, 0.003f );		// 실시간으로 목표쪽으로 향하는 힘.
//		AngleToVector( &vAcc, fAngXZ, fAngH, 0.009f );		// 실시간으로 목표쪽으로 향하는 힘.
		...
		m_pSfxObj->m_vRotate.y = fAngXZ;	// sfx model의 각도도 변화 시켜줌.
...
void CSfxItemRangeAtk1::Process( FLOAT fElapsedTime )
{
	...
	if( m_bHit == FALSE ) 
	{
		...
		if( m_pTail == NULL )	// 아직 할당 안됐으면 할당하고.
		{
			...
			m_pTail = g_TailEffectMng.AddEffect( g_Neuz.m_pd3dDevice, "etc_Tail2.bmp", 2, 0.35f );
...
void CSfxItemYoyoAtk::MakePath(int nType)
{
	...
	for( int i = 0; i < nMaxVertex-1; i ++ )
	{
		...
		for( int j = 0; j < MAX_SF_SLERP+1; j ++ )
		{
			if( m_nMaxSpline >= 30 )
			{
				LPCTSTR szErr = Message( "CSfxItemYoyoAtk Spline : 범위 초과 %d", m_nMaxSpline );
				ADDERRORMSG( szErr );
				break;
			}
	...
	for( i=0; i<m_nMaxSpline; i++ )
	{
		...
		//D3DXMATRIX mWorld = pObj->GetMatrixRotation() * mScal * pObj->GetMatrixTrans();
...
void CSfxItemYoyoAtk::Process( FLOAT fElapsedTime )
{
	CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );	
	...
	if( m_bHit == FALSE ) 
	{
		...
		/*
		D3DXVECTOR3 vPos    = GetPos();
		D3DXVECTOR3 vLength = GetPos() - g_pPlayer->GetPos();
		FLOAT fLen = D3DXVec3LengthSq( &vLength );

		vPos += m_vDelta * m_fBreak;			// 그 벡터로 속도를 결정해서 좌표 이동.

		if( m_bDir )
		{
			// 마법객체 방향벡터로 모델 방향 돌리기~~~
			D3DXVECTOR3 vDir      = D3DXVECTOR3( 0.0f, 0.0f, 1.0f );
			D3DXVECTOR3 vDestNor  = vPosPos - GetPos();
			D3DXVECTOR3 vAxis;
			D3DXQUATERNION   qRot;
	...
	#else //__YTAIL_EFFECT_EX
			m_pTail = g_TailEffectMng.AddEffect( g_Neuz.m_pd3dDevice, "etc_Tail2.bmp", 2, 0.1f );
	#endif //__YTAIL_EFFECT_EX
		}
		...
		if( fAngXZ < 0 )
			fAngXZ += 360.0f;
		AngleToVector( &vPos1, fAngXZ, -fAngH, 0.1f );
		vPos1 += GetPos();
		...
		if( fAngXZ > 360.0f )
			fAngXZ -= 360.0f;
		AngleToVector( &vPos2, fAngXZ, -fAngH, 0.1f );
		vPos2 += GetPos();
...
void CSfxItemYoyoAtk::Render( LPDIRECT3DDEVICE9 pd3dDevice )
{
    ...
    v3[0].position = v3SrcPos;
	v3[1].position = GetPos();
	...
	HRESULT hr = pd3dDevice->DrawPrimitiveUP( D3DPT_LINELIST, 1, &v3[0], sizeof( FVF_SWDFORCE ) );
...
void CSfxItemRangeAtk_JunkBow::Process( FLOAT fElapsedTime )
{
	CMover* pObjDest = (CMover*)prj.GetCtrl( m_idDest );	// 유도탄 형이기때문에 타겟은 항상 읽어봐야 한다.
	if( IsValidObj(pObjDest) )
	{
	...
	if( m_bHit == FALSE ) 
	{
		...
		#else //__YTAIL_EFFECT_EX
				m_pTail[i] = g_TailEffectMng.AddEffect( g_Neuz.m_pd3dDevice, "etc_Tail2.bmp", 2, 0.35f );
		#endif //__YTAIL_EFFECT_EX
			}
		}
...
void CSfxItemRangeAtk1_Allow::Process( FLOAT fElapsedTime )
{
	if( m_pSfxObj->Process() )		// 애니메이션은 루프.
	{
		m_pSfxObj->m_nCurFrame = 0;
...
void CSfxItemRangeAtk1_AllowRain::Process( FLOAT fElapsedTime )
{
	D3DXVECTOR3 vPos = GetPos();
...
//	m_SfxObj2.m_vPos = 그대로 날아간다고 했을때 충돌할 위치를 미리 계산;
...
void CSfxAtkStraight::Process( FLOAT fElapsedTime )
{
	{ 
...
//		CMover* pObjDest = (CMover*)prj.GetCtrl( m_idDest );		// 타겟
//		if( IsInvalidObj( pObjDest ) )
//		{
//			Delete();
//			return;
...
void CSfxItemWandAtk2::Process( FLOAT fElapsedTime )
{
	if(m_nFrame==0) {
		CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
...
void CSfxItemWandAtk3::Process( FLOAT fElapsedTime )
{
	if(m_nFrame==0) {
		CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
		if(pObjSrc) m_pSfxObj->m_vPos=pObjSrc->GetPos()+D3DXVECTOR3(.0f,1.0f,.0f);
...
void CSfxItemWandAtk4::Process( FLOAT fElapsedTime )
{
	if(m_nFrame==0) {
		CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
		if(pObjSrc) m_pSfxObj->m_vPos=pObjSrc->GetPos()+D3DXVECTOR3(.0f,1.0f,.0f);
...
{
//	m_SfxObj2.SetSfx("vagrant_overcutter01");
}
CSfxSkillVagOverCutter::~CSfxSkillVagOverCutter()
{
}
void CSfxSkillVagOverCutter::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	{		
...
void CSfxSkillVagOverCutter::Render( LPDIRECT3DDEVICE9 pd3dDevice )
{
	...
	//m_SfxObj2.m_vRotate.y=GetAngle();	m_pSfxObj->Render( pd3dDevice );
	//if//(m_nFrame>50) {
	//	/m_SfxObj2.Render( pd3dDevice );
	//}
}
...
void CSfxSkillVagCleanHit::Process( FLOAT fElapsedTime )
{
	CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
	if( IsValidObj(pObjSrc) ) 
	{
...
void CSfxFixed::Process( FLOAT fElapsedTime )
{
	CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
	if( IsValidObj(pObjSrc) ) 
	{
...
void CSfxSkillVagBrandish::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	{ 
		CMover* pObjDst = (CMover*)prj.GetCtrl( m_idDest );
...
void CSfxSkillMerKeenWheel::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
//	CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
//	if(pObjSrc) SetPos(pObjSrc->GetPos());
...
{
//	m_SfxObj2.SetSfx("sfx_SklMerSplmash02");
	m_SfxObj2.SetSfx(XI_SKILL_MER_ONE_SPLMASH02);
}
CSfxSkillMerSplmash::~CSfxSkillMerSplmash()
{
}
void CSfxSkillMerSplmash::Process( FLOAT fElapsedTime )
{
	CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
...
void CSfxSkillMerBlindSide::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
//	CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
//	if(pObjSrc) SetPos(pObjSrc->GetPos());
...
// 머셔너리 기본 스킬 5
/*
CSfxSkillMerPanBarrier::CSfxSkillMerPanBarrier()
{
//	m_SfxObj2.SetSfx("sfx_sklMerPanBarrier02"); 
	//m_bSfxObj2 = FALSE;
//	m_timer.Set( SEC( 5 ) );
}
CSfxSkillMerPanBarrier::~CSfxSkillMerPanBarrier()
...
void CSfxSkillMerPanBarrier::Process( FLOAT fElapsedTime )
{
	CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
	if( pObjSrc ) 
	{
...
#ifndef __WORLDSERVER
void CSfxSkillMerPanBarrier::Render( LPDIRECT3DDEVICE9 pd3dDevice )
{
	if( m_bSfxObj2 == FALSE )
	{
		m_pSfxObj->m_vPos = GetPos() + D3DXVECTOR3(.0f,0.1f,.0f);
		m_pSfxObj->m_vRotate.y=GetAngle();
		m_pSfxObj->m_vScale = GetScale();
		m_pSfxObj->Render( pd3dDevice );
	}
	...
	{ 
		m_SfxObj2.m_vPos = GetPos() + D3DXVECTOR3(.0f,0.1f,.0f);
		m_SfxObj2.m_vRotate.y = GetAngle();
		m_SfxObj2.m_vScale = GetScale();
		m_SfxObj2.Render( pd3dDevice );
	}
...
//CSfxSkillMerProtection::CSfxSkillMerProtection()
//{
//	m_vPosDest.x=5.0f;
	//m_SfxObj2.SetSfx("sfx_sklMerProtection02"); 
//	m_bSfxObj2 = FALSE;
	/*
#ifdef __CLIENT
	CMover* pMover = (CMover*)GetActiveObj();
	if( pMover && pMover->GetType() == OT_MOVER )
	{//SI_MER_SHIELD_PANBARRIER
		LPSKILL pSkill = pMover->GetSkill( 0, SI_MER_SHIELD_PROTECTION );
		AddSkillProp* pAddSkillProp = prj.GetAddSkillProp( pSkill->dwSkill,  pSkill->dwLevel );		// UseSkill에서 사용한 스킬의 프로퍼티 꺼냄
		m_timer.Set( pAddSkillProp->dwSkillTime );
	}
#else
	m_timer.Set( SEC( 0 ) );
#endif
...
void CSfxSkillMerProtection::Process( FLOAT fElapsedTime )
{
	CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
	if( pObjSrc ) 
	{
...
#ifndef __WORLDSERVER
void CSfxSkillMerProtection::Render( LPDIRECT3DDEVICE9 pd3dDevice )
{
	if( m_bSfxObj2 == FALSE )
	{
		m_pSfxObj->m_vPos = GetPos() + D3DXVECTOR3(.0f,0.1f,.0f);
...
void CSfxSkillAssBurstcrack::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	if(m_pSfxObj->Process())
	{
...
void CSfxSkillAssTampinghole::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	if(m_pSfxObj->Process())
	{
...
void CSfxNpcDirSteam::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	if(m_pSfxObj->Process())
	{
...
void CSfxSkillMagFireCasting::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	if(m_pSfxObj->Process())
	{
...
void CSfxSkillMagWindCasting::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
	if(m_pSfxObj->Process())
	{
...
//	m_SfxObj2.SetSfx("sfx_sklMagStrongWind01");		// 맞았을때 효과.
	m_bHit = FALSE;
	m_fAngle = 0;
}
CSfxSkillMagStrongWind::~CSfxSkillMagStrongWind()
...
void CSfxSkillMagStrongWind::Process( FLOAT fElapsedTime )
{
	if(m_nFrame==0) 
	{
...
//		}
		...
		if(m_SfxObj2.m_nCurFrame==0)
		{
			...
			int fAngle = m_fAngle;
			if( IsValidObj(pObjDest) && pObjDest->GetProp()->dwClass == RANK_SUPER )	// 보스몹은 밀리지 않음.
			{
				fAngle = 0;		fDmgPower = 0;
			}
			
			DamageToTarget( 0, fAngle, fDmgPower );
...
//	m_SfxObj2.SetSfx("sfx_sklMagSwordwind01");
	m_bHit=FALSE;
}
CSfxSkillMagSwordWind::~CSfxSkillMagSwordWind()
{
}
void CSfxSkillMagSwordWind::Process( FLOAT fElapsedTime )
{
	if(m_nFrame==0) 
	{
		CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
	...
	if(m_bHit==FALSE) 
	{
		...
		vDelta=m_pSfxObj->m_vPos-m_SfxObj2.m_vPos;
/*
#ifdef  __Y0202   // 마법객체 방향벡터로 모델 방향 돌리기~~~
		D3DXVECTOR3 vDir      = D3DXVECTOR3( 0.0f, 0.0f, 1.0f );
		...
		m_pSfxObj->m_vRotate.x=m_SfxObj2.m_vRotate.x=D3DXToDegree(pitch);
		m_pSfxObj->m_vRotate.y=m_SfxObj2.m_vRotate.y=D3DXToDegree(yaw);
		m_pSfxObj->m_vRotate.z=m_SfxObj2.m_vRotate.z=D3DXToDegree(roll);
#else
*/
		FLOAT fAngle;
		if(vDelta.z>0) {
...
//	m_SfxObj2.SetSfx( "sfx_SklMagFireBoomerang01" );
	m_bHit=FALSE;
}
CSfxSkillMagFireBoomerang::~CSfxSkillMagFireBoomerang()
{
}
void CSfxSkillMagFireBoomerang::Process( FLOAT fElapsedTime )
{
	if(m_nFrame==0) 
	{
		CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
...
void CSfxSkillMagFireBomb::Process( FLOAT fElapsedTime )
{
	m_nFrame++;
//	if(m_pSfxObj->m_nCurFrame==0)
//		DamageToTarget();
	if(m_pSfxObj->Process())
	{
		Delete();
...
//	m_SfxObj2.SetSfx("sfx_sklMagHotAir01");		// 타겟측에서 펑 터지며 회전하는 효과
	m_nDmgCnt = 0;
}
CSfxSkillMagHotAir::~CSfxSkillMagHotAir()
{
}
void CSfxSkillMagHotAir::Process( FLOAT fElapsedTime )
{
	CMover* pObjSrc = prj.GetMover( m_idSrc );
...
////////////////////////////////////////////////////////////////
void CSfxDuelParty::Process( FLOAT fElapsedTime )
{
#ifndef __BEAST
	CSfx::Process( fElapsedTime );
...
#ifdef __NPP_050308
			if( pMover->m_nDuel != 2 ||
				( pMover->m_nDuel == 2 && ( pMover->m_nDuelState == 300 || pMover->m_nDuelState == 0 ) ) 
				)		// process중 src무버의 극단듀얼이 끝났으면 삭제.
#else // __NPP_050308
			if( pMover->m_nDuel != 2 )		// process중 src무버의 극단듀얼이 끝났으면 삭제.
#endif // __NPP_050308
			{
				DeleteSfx( pMover );
			}
...
void	CSfxDuelParty::Render( LPDIRECT3DDEVICE9 pd3dDevice )
{
	...
	if( pMover->m_SkillState.HasSkill( BUFF_SKILL, SI_ACR_SUP_DARKILLUSION ) || pMover->IsMode( TRANSPARENT_MODE ) )
		return;
#endif // __NPP_050308
...
void CSfxSetItem::Process( FLOAT fElapsedTime )
{
#ifndef __BEAST
	CSfx::Process( fElapsedTime );
	CMover *pMover = prj.GetMover( m_idSrc );
...
////////////////////////////////////////////////////////////////
void CSfxCollect::Process( FLOAT fElapsedTime )
{
#ifndef __BEAST
	m_nFrame++;
...
////////////////////////////////////////////////////////////////
void CSfxClockWorksCharge::Process( FLOAT fElapsedTime )
{
#ifndef __BEAST
	m_nFrame++;
...
void CSfxClockWorksCannon::Process( FLOAT fElapsedTime )
{
#ifndef __BEAST
	m_nFrame++;
...
/*
		...
		if( pMover )
			Message( "CSfxHitArray::Add : SfxhitArray가 꽉 찼다. %d %s %08x, %d, %d", id, pMover->GetName(), dwAtkFlags, dwSkill, nMaxDmgCnt );
		else
			Message( "CSfxHitArray::Add : SfxhitArray가 꽉 찼다. %d %d %08x, %d, %d", id, objid, dwAtkFlags, dwSkill, nMaxDmgCnt );
*/

//		for( i = 0; i < nMaxSizeOfSfxHit; i ++ )
//		{
//			Message( "- %d %d %d", m_aSfxHit[i].id, m_aSfxHit[i].dwSkill, m_aSfxHit[i].nMaxDmgCnt );
//		}
...
{
//	ASSERT( nIndex >= 0 );
//	ASSERT( nIndex < nMaxSizeOfSfxHit );
...
void CSfxSkillMagIceMissile::Process( FLOAT fElapsedTime )
{
	if(m_nFrame==0) 
	{
		CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
...
void CSfxSkillMagLightningBall::Process( FLOAT fElapsedTime )
{
	if(m_nFrame==0) 
	{
		CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
...
//	m_SfxObj2.SetSfx( "sfx_sklMagEarSpikes01" );
	m_bHit=FALSE;
}
CSfxSkillMagSpikeStone::~CSfxSkillMagSpikeStone()
{
}
void CSfxSkillMagSpikeStone::Process( FLOAT fElapsedTime )
{
	if(m_nFrame==0) 
	{
		CMover* pObjSrc = (CMover*)prj.GetCtrl( m_idSrc );
...
////////////////////////////////////////////////////////////////
/*void CSfxRotate::Process( FLOAT fElapsedTime )
{
#ifndef __BEAST
	CSfx::Process( fElapsedTime );
	CMover *pMover = prj.GetMover( m_idSrc );
	...
	if( m_bHit == FALSE ) 
	{
		...
		if( D3DXVec3LengthSq( &vDelta ) < 0.2f * 0.2f )		// 폭발 거리까지 다가가면 충돌.
		{
			...
			if( m_dwSfxHit != NULL_ID )
			{
				...
				{
					pSfx->SetAngle( m_pSfxObj->m_vRotate.y );
//					pSfx->m_pSfxObj->m_vRotate = m_pSfxObj->m_vRotate;
				}
			}
			Delete();	// this는 삭제.
		}
...
void CSfxShootWave::Process( FLOAT fElapsedTime )
{
	CMover* pObjDest = (CMover*)prj.GetCtrl( m_idDest );
	if( IsValidObj(pObjDest) )
	{
	...
	if( m_bHit == FALSE ) 
	{
///		SetScale( D3DXVECTOR3( 0.f, 0.2f, 0.2f ) );
		D3DXVECTOR3 vDelta = m_SfxObj2.m_vPos - m_pSfxObj->m_vPos;
		D3DXVec3Normalize( &vDelta, &vDelta );		// 타겟쪽 단위벡터 구해서
...
void CSfxShootWave::Render( LPDIRECT3DDEVICE9 pd3dDevice )
{
	...
	for( int i = 0; i < MAX_SHOOTWAVE_TAIL; i ++ )
	{
		if( m_vTail[i].x )
		{
//			m_pSfxObj->m_nCurFrame = m_nTailFrame[i];
...
////////////////////////////////////////////////////////////////
// 무버의 특정 파츠에 링크되는 Sfx
void CSfxPartsLink::Process( FLOAT fElapsedTime )
{
#ifndef __BEAST
	m_nFrame++;
...
#ifndef __ACROBAT_0504	
	// 밀리스킬 동작 끝나면 해제.
	if( (pMover->m_pActMover->GetState() & OBJSTA_ATK_ALL) != OBJSTA_ATK_MELEESKILL )
	{
		Delete();		// sfx 삭제.
	}
#endif //__ACROBAT_0504
...
#ifdef __ACROBAT_0504
void CSfxPartsLinkBlade::Process( FLOAT fElapsedTime ) // 블레이드 파츠이펙트는 밀리 스킬끝나면 소멸
{
#ifndef __BEAST
	CSfxPartsLink::Process(fElapsedTime);
...
void CSfxPartsLinkJst::Process( FLOAT fElapsedTime ) // 제스터 파츠이펙트는 끝프레임이면 소멸
{
#ifndef __BEAST
	CSfxPartsLink::Process(fElapsedTime);
...
void CSfxAllowPartsLink::Process( FLOAT fElapsedTime )
{
#ifndef __BEAST
	m_nFrame++;
...
void CSfxReady::Process( FLOAT fElapsedTime )
{
	CSfx::Process( fElapsedTime );
...
void CSfxPartsLinkShoulder::Process( FLOAT fElapsedTime )
{
#ifndef __BEAST
	CSfx::Process( fElapsedTime );
