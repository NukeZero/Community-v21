...
BOOL CResManager::AddControl( DWORD dwAppletId, LPWNDCTRL pWndCtrl )
{
	...
	pWndControl->bTile      = pWndCtrl->bTile;
#ifdef __DAISY
...
//	if( lpDefData ) return FALSE;
	...
	pWndApplet->bTile     = pWndApp->bTile;
...
BOOL CResManager::Load( LPCTSTR lpszName )
{
	...
	while( scanner.tok != FINISHED )
	{
		...
		*/
		...
		//pWndApplet->d3dFormat = D3DFMT_A4R4G4B4;
		pWndApplet->d3dFormat = (D3DFORMAT)scanner.GetNumber();//D3DFMT_A4R4G4B4; 
...
//		strcpy( pWndApplet->szHelpKey, script.Token );
		...
		while( *scanner.token != '}' )
		{
			...
			/*
#ifdef __DAISY
			if( dwWndType == WTYPE_TEXT || dwWndType ==WTYPE_EDITCTRL || dwWndType == WTYPE_COMBOBOX || dwWndType == WTYPE_LISTBOX || dwWndType == WTYPE_TREECTRL || dwWndType == WTYPE_LISTCTRL )
			{
				pWndCtrl->strTexture = DEF_CTRL_TEXT;
				pWndCtrl->bTile = TRUE;
				if( dwWndType == WTYPE_LISTBOX || dwWndType == WTYPE_TREECTRL || dwWndType == WTYPE_LISTCTRL )
				{
					pWndCtrl->dwWndStyle |= WBS_VSCROLL;

				}
				pWndCtrl->dwWndStyle &= ~WBS_NODRAWFRAME;

			}
#endif
			*/
			/*
			if( pWndCtrl->dwWndStyle & WBS_RADIO )
			{
				pWndCtrl->rect.bottom = pWndCtrl->rect.top + 16;
				pWndCtrl->strTexture = DEF_CTRL_RADIO; 
			}
			if( pWndCtrl->dwWndStyle & WBS_CHECK )
			{
				pWndCtrl->rect.bottom = pWndCtrl->rect.top + 16;
				pWndCtrl->strTexture = DEF_CTRL_CHECK;
			}
			*/
...
*/
	...
	{ 
		...
		// Style
		string.Format( _T( " 0x%x" ), lpWndApplet->dwWndStyle );
		file.PutWideString( string );		
...
#ifdef __V050322_CACHEQUEST
			// Texture tile
			string.Format( _T( "%d " ), pWndCtrl->bTile );
			file.PutWideString( string );
#endif
