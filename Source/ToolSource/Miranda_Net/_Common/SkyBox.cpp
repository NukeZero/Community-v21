...
void CSkyBox::Process(FLOAT fElapsedTime)
{
	/*
	if( m_pWorld ) 
	{
...
void CSkyBox::Execute()
{
}
void CSkyBox::Render( D3DXVECTOR3 Pos, D3DXVECTOR3 LookAt, LPDIRECT3DDEVICE9 pd3dDevice )
{
	//m_pWorld=pWorld;
#ifndef __BEAST
	D3DXVECTOR3 vecPos( 0, 0, 0 );
	D3DXVECTOR3 vecLookAt = LookAt;
	D3DXVECTOR3 vecTempLookAt;
	//vecLookAt -= pWorld->m_pCamera->GetPos();//m_vPos2; 위치 
	vecLookAt -= Pos;
...
*/	
	...
	if(g_GameTimer.m_nHour == 17 || g_GameTimer.m_nHour == 18 ) 
	{
		...
		pd3dDevice->SetRenderState( D3DRS_TEXTUREFACTOR, D3DCOLOR_ARGB( 255-(nMin*255/119), 0, 0, 0 ) );
		pd3dDevice->SetTexture( 0, m_pSkyBoxTexture);
		//pd3dDevice->DrawPrimitive( D3DPT_TRIANGLESTRIP, 0, 85 );//56);
		pd3dDevice->DrawPrimitive( D3DPT_TRIANGLELIST, 0, 348 / 3 );
		pd3dDevice->SetRenderState( D3DRS_TEXTUREFACTOR, D3DCOLOR_ARGB( nMin*255/119, 0, 0, 0 ) );
		pd3dDevice->SetTexture( 0, m_pSkyBoxTexture2);
		//pd3dDevice->DrawPrimitive( D3DPT_TRIANGLESTRIP, 0, 85 );//56);
		pd3dDevice->DrawPrimitive( D3DPT_TRIANGLELIST, 0, 348 / 3 );
	}
	else 
...
//	pd3dDevice->SetRenderState( D3DRS_FOGEND,     FtoDW(100.0f) );
	...
	if( g_GameTimer.m_nHour < 6 ) 
	{
		pd3dDevice->SetTexture( 0, m_pCloudTexture3);
...
//	if(m_fSunAngle>-20.0f)
	...
	//		D3DXMatrixRotationX( &mat,-m_fSunAngle*3.1415926f/180.0f);
			...
			pd3dDevice->SetTexture( 0, m_pMoonTexture[0]);
...
#endif //__YMOON_1019
	if( FAILED( LoadTextureFromRes( pd3dDevice, MakePath( DIR_WEATHER, "sundisk.bmp"),
									   &m_pSunTexture ) ) )
	{
		return D3DAPPERR_MEDIANOTFOUND;
	}
...
{
#ifdef __YMOON_1019	
	SAFE_RELEASE(m_pMoonTexture[0]);
	SAFE_RELEASE(m_pMoonTexture[1]);
#endif //__YMOON_1019
