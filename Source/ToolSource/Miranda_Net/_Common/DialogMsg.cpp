...
#ifndef __BUGFIX_DIALOGMSG1112
	for(int i = 0; i < m_VendortextArray.GetSize(); i++)
		delete (LPCUSTOMTEXT)m_VendortextArray.GetAt( i );
...
//{{AFX
		delete (LPCUSTOMTEXT)m_textArray.GetAt(i);
//}}AFX
		...
		else
		{
			...
			if( pObj->IsCull() == FALSE )
			{
				...
				if( lpCustomText->m_nKind == CHAT_SHOUT )
				{
					...
					if( pShoutTex )
					{
						p2DRender->RenderTextureEx( CPoint(x-20, y-20), CPoint( fWidth+50.0f, fHeight+40.0f ), pShoutTex, nAlpha, 1.0f, 1.0f, FALSE );
						goto g_ShoutChat;
					}
				}
				...
				if( nWidth >= 6 )
				{
					point =  CPoint( x + ( 3 * nWidth / 5 ) * 8, y );
					nIndex = 9;
				}
				else
				if( nWidth == 4 || nWidth == 5 ) 
				{
					point =  CPoint( x + 3 * 8, y );
					nIndex = 9;
				}
				else
				if( nWidth == 3 ) 
				{
					point =  CPoint( x + 1 * 8, y );
					nIndex = 9;
				}
				pTexture = m_texPack_ex[lpCustomText->m_nKind].GetAt( nIndex );

				SetTextureVertex( pVertices, point.x, point.y, pTexture->m_fuLT, pTexture->m_fvLT );
				pVertices++;
				SetTextureVertex( pVertices, point.x + 8.0f, point.y, pTexture->m_fuRT, pTexture->m_fvRT);
				pVertices++;
				SetTextureVertex( pVertices, point.x, point.y + 8.0f, pTexture->m_fuLB, pTexture->m_fvLB);
				pVertices++;
				...
				SetTextureVertex( pVertices, point.x, point.y + 8.0f, pTexture->m_fuLB, pTexture->m_fvLB);
				pVertices++;
				SetTextureVertex( pVertices, point.x + 8.0f, point.y + 8.0f, pTexture->m_fuRB, pTexture->m_fvRB);
				pVertices++;
...
//				p2DRender->TextOut_EditString( point.x, point.y, lpCustomText->m_string, 0, 0, 0 );
	...
	for( i = 0; i < m_VendortextArray.GetSize(); i++ )
	{
		...
		if( pObj->IsCull() == FALSE )
		{
			...
			ptTex1.x = (int)(vOut.x+8)+ptTex2.x;
			ptTex1.y = vOut.y-fGap;
			...
			DRAWVERTEX vertex2[4];
			DRAWVERTEX* pVertices2 = vertex2; 
			
			DWORD dwColor = D3DCOLOR_ARGB( nAlpha, 255, 255, 255 );
			
			SetDrawVertex( pVertices2, vOut.x, vOut.y-fGap, dwColor );
			pVertices2++;
			SetDrawVertex( pVertices2, vOut.x+fWidth, vOut.y-fGap, dwColor );
			pVertices2++;
			SetDrawVertex( pVertices2, vOut.x+fWidth, vOut.y+fHeight-fGap, dwColor );
			pVertices2++;
			SetDrawVertex( pVertices2, vOut.x, vOut.y+fHeight-fGap, dwColor );
			pVertices2++;
			
			pd3dDevice->SetVertexShader( NULL );
			pd3dDevice->SetTexture( 0, NULL );
			pd3dDevice->SetFVF( D3DFVF_DRAWVERTEX );
			pd3dDevice->DrawPrimitiveUP( D3DPT_TRIANGLEFAN, 2, vertex2, sizeof( DRAWVERTEX ) );
			
			p2DRender->RenderRect( CRect( vOut.x, vOut.y-fGap+1, vOut.x+fWidth+1, vOut.y+fHeight-fGap+1 ), D3DCOLOR_XRGB(0,0,0) );
#endif //__YNEWPERSONSHOP_EX1026
			...
			p2DRender->TextOut_EditString( x, y-fGap, lpCustomText->m_string, 0, 0, 0 );
		}
	}
}
#endif
...
{
	if( nEmoticonIdx < m_texEmoticonUser.GetNumber() )
...
{
	if( nEmoticonIdx < m_texEmoticon.GetNumber() )
...
void CDialogMsg::AddTexture( CObj* pObj, CTexture* pTexture )
{
	...
	lpCustomText->m_bInfinite	= ( pObj->GetType() == OT_MOVER && ( (CMover*)pObj )->m_vtInfo.IsVendorOpen() );
	lpCustomText->m_pTexture = pTexture;
	m_textArray.Add( lpCustomText );
}

void CDialogMsg::AddMessage( CObj* pObj, LPCTSTR lpszMessage, DWORD RGB, int nKind )
{
	if( pObj->GetType() == OT_MOVER )
	{
		CScanner scanner;
		...
		{			
			...
			{
				if( strlen(m_EmiticonCmd[ j ].m_szCommand) && stricmp( scanner.token, m_EmiticonCmd[ j ].m_szCommand ) == 0 )
				{
