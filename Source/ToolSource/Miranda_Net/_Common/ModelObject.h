...
//#include "..\..\Resource\defineNeuz.h"

//#define		BONE_LFOREARM		12	// 왼쪽 팔둑
//#define		BONE_LHAND			13
//#define		BONE_RHAND			17
...
//	D3DXVECTOR3		*m_pVertexListWorld;	// r/w 영향받는 본에 의해 실시간으로 월드로 변환될 벡터(바이페드에만 사용)
//	D3DXMATRIX		*m_mUpdate;				// 각 오브젝트의 애니메이션 계산후 변환된 매트릭스 - 계층 개수만큼 생성됨 단일오브젝이면 디폴트로 LocalTM을 카피시킴.
	...
	//	LPDIRECT3DINDEXBUFFER8		m_pd3d_IB;	// AGP에 생성되는 인덱스버퍼(역시 위와 같다)
...
//	D3DXMATRIX	*m_pBaseBone;		// 본이 있는 오브젝을 읽었을때 그 본의 포인터를 받아둔다. 참조용.	X0617들어가면서 빠짐,
...
//	D3DXVECTOR3	m_vLastKeyPos;		// 마지막으로 계산됐던 키포지션값
//	D3DXVECTOR3	m_vLastDelta;	
//	D3DXVECTOR3	m_vLastPath;		// path처리에 사용. 이전에 읽었던 m_pPath->m_vPos값.
...
//	void	ResetSpeed( void ) { m_fKeySpeed = (float)m_pMotion->m_dKps; }
	...
	{ 
		...
		return NULL; 
#else //__YPARTS_EX
		return m_Element[nNum].m_pObject3D; 
#endif //__YPARTS_EX
	}		// 이것은 상이만 쓴다..ㅡㅡ^;
	...
	{ 
		...
		return NULL;
#else //__YPARTS_EX
		m_Element[nParts].m_nEffect |= nEffect; 
		return &m_Element[nParts];
#endif //__YPARTS_EX
	} 
...
#else
	void	SetTextureEx( BOOL bUse )		// 확장텍스쳐로 렌더링 하는 모드냐?
	{
		m_Element[0].m_pObject3D->SetTextureEx( bUse );		// 몬스터는 [0]하나만 쓴다고 가정하고 하자.
	}
#endif
	...
	{ 
		if( m_pMotion == NULL )
		{
			LPCTSTR szErr = Message( "IsAttrSound : pMotion==NULL %d %d", m_fFrameOld, m_fFrameCurrent );
			ADDERRORMSG( szErr );
		}
	...
	BOOL GetPosBone( D3DXVECTOR3* pOut, const char* bonename );		//gmpbigsun : 본좌표 추출 
	BOOL GetPosBone( D3DXVECTOR3* pOut, const int idx );
//	void GetPathDelta( D3DXVECTOR3 *pOut );		// // m_fFrameCurrent의 Path Delta값을 보간하여 리턴

//#ifdef __COLA		//gmpbigsun : 왜 콜라만? ㅋㅋ
...
//#endif
...
#ifdef __XEFFECT1006
	void	CreateParticle( int nParts, const D3DXMATRIX *pmWorld, int nType, int nLevel );
#else
	void	CreateParticle( int nParts, const D3DXMATRIX *pmWorld );
#endif
	...
	void	RenderEffect( LPDIRECT3DDEVICE9 pd3dDevice, const D3DXMATRIX *mWorld );
	//gmpbigsun : Render bone
	BOOL	RenderBone( LPDIRECT3DDEVICE9 pd3Device, const D3DXMATRIX* mWorld, CModelObject* pBox );
...
#endif
	BOOL	RenderBB( LPDIRECT3DDEVICE9 pd3dDevice, const D3DXMATRIX *mWorld );

//	HRESULT	SetVertexBuffer( O3D_ELEMENT *pElem );
//	HRESULT	SendDeviceBuffer( O3D_ELEMENT *pElem, BOOL bSendIB );
//	void	TransformSkinVertex( O3D_ELEMENT *pElem );
//	void	Animate( void );
//	void	AnimateBone( const D3DXMATRIX *pmWorld );
//	void	AnimateBone( BOOL bSkinning = TRUE );
	...
	// collision detect
	BOOL	Intersect( const D3DXVECTOR3 &vRayOrig, const D3DXVECTOR3 &vRayDir, const D3DXMATRIX &mWorld, D3DXVECTOR3* pvIntersect, FLOAT* pfDist, BOOL bColl = FALSE  ) { if( IntersectRayTri( vRayOrig, vRayDir, mWorld, pvIntersect, pfDist, bColl ) ) return TRUE; else return FALSE; }
