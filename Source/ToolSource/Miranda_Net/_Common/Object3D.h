...
#else
	BOOL	m_bTextureEx;			// 확장텍스쳐 모드로 렌더 해야하는가.
#endif
...
public:
	...
	D3DXVECTOR3	m_vForce3, m_vForce4;			// 추가검광 시작점,끝점
...
#else
	void	SetTextureEx( GMOBJECT *pObj, BOOL bUse );
	void	SetTextureEx( BOOL bUse )
	{
		for( int i = 0; i < m_pGroup->m_nMaxObject; i ++ )
			SetTextureEx( &(m_pGroup->m_pObject[i]), bUse );		
	}
	void	LoadTextureEx( GMOBJECT *pObj, MATERIAL *pmMaterial[16] );
#endif
...
static 	BOOL IsAnimateFile( LPCTSTR szFileName );		// szFileName이 애니메이션이 있냐?
	...
	// 충돌/교차 루틴들.
	D3DXVECTOR3 *IntersecRayVertex( D3DXVECTOR3 *pOut, const D3DXVECTOR3 &vRayOrig, const D3DXVECTOR3 &vRayDir );
	int		SlideVectorXZ( D3DXVECTOR3 *pOut, const D3DXVECTOR3 &vPos, const D3DXVECTOR3 &vEnd , const D3DXMATRIX &mWorld, BOOL bCollObj = TRUE );
	...
	BOOL	IsTouchOBB_Line( const D3DXVECTOR3 &vStart, const D3DXVECTOR3 &vEnd, const D3DXMATRIX &mWorld, D3DXVECTOR3* pvIntersect );
