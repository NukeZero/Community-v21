...
const DWORD Terrain::TerrainVertex::FVF = D3DFVF_XYZ | D3DFVF_NORMAL | D3DFVF_TEX1;
...
void  Terrain::Save(CFileIO &File)
{
	int		nLength = 0;
	File.Write(&_numVertsPerRow, sizeof(int));
	File.Write(&_numVertsPerCol, sizeof(int));

	nLength = strlen(m_FileName);	
	File.Write(&nLength, sizeof(int));
	File.Write(m_FileName, nLength);

}


void  Terrain::Load(CFileIO &File)
{

	int nLength		= 0;
	memset(m_FileName, 0, 128);
	File.Read(&_numVertsPerRow, sizeof(int));
	File.Read(&_numVertsPerCol, sizeof(int));
	File.Read(&nLength, sizeof(int));
	File.Read(m_FileName, nLength);
	loadTexture(m_FileName);
	Resize(_numVertsPerRow, _numVertsPerCol, 4);

}



void  Terrain::Resize(int numVertsPerRow, int numVertsPerCol, int cellSpacing)
{
...
bool Terrain::Render(D3DXMATRIX* world)
{
	...
	m_pDevice->SetStreamSource(0, _vb, 0, sizeof(TerrainVertex));
	m_pDevice->SetFVF(TerrainVertex::FVF);
	m_pDevice->SetIndices(_ib);
