...
inline void chASSERTFAIL( LPCSTR file, int line, PCSTR expr, LPSTR szFmt, ... )
{
	...
	{
		wsprintf( pszBuffer, "Buffer overflow reporting '%*.*s'", 50, 50, pszBuffer );
		return;
	}
	va_end( marker );
...
#define	chFAIL()	{	chASSERTFAIL( __FILE__, __LINE__, "", "" );	return;		}

#define	chASSERT1( x, format, p1 )	if( !( x ) )	{	chASSERTFAIL( __FILE__, __LINE__, #x, format, p1 );		return;		}
#define	chASSERT2( x, format, p1, p2 )	if( !( x ) )	{	chASSERTFAIL( __FILE__, __LINE__, #x, format, p1, p2 );		return;		}
#define	chFAIL1( format, p1 )	{	chASSERTFAIL( __FILE__, __LINE__, "", format, p1 );	return;		}
