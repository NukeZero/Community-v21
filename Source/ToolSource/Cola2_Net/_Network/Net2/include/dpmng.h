...
//	Operations
	...
	BOOL	JoinSession( LPCSTR lpSession, DWORD dwcrc = 0 );
	BOOL	StartServer( u_short uPort, BOOL bSingleThreaded = FALSE, DWORD dwcrc = 0 );
	BOOL	ConnectToServer( LPCSTR lpConnection, u_short uPort, BOOL bSingleThreaded = FALSE, DWORD dwcrc = 0 );
...
template <class T>
BOOL CDPMng<T>::ConnectToServer( LPCSTR lpConnection, u_short uPort, BOOL bSingleThreaded, DWORD dwcrc )
	{	return( CreateDPObject( bSingleThreaded ) && InitConnection( (LPVOID)lpConnection, uPort ) && JoinSession( NULL, dwcrc ) );	}
...
template <class T>
BOOL CDPMng<T>::JoinSession( LPCSTR lpSession, DWORD dwcrc )
	{
