...
public:
static	\
	MemPooler< CClientSockE<T> >*	m_pPool;
public:
	void*	operator new( size_t nSize )	{	return CClientSockE<T>::m_pPool->Alloc();	}
	void*	operator new( size_t nSize, LPCSTR lpszFileName, int nLine )	{	return CClientSockE<T>::m_pPool->Alloc();	}
	void	operator delete( void* lpMem )	{	CClientSockE<T>::m_pPool->Free( (CClientSockE<T>*)lpMem );	}
	void	operator delete( void* lpMem, LPCSTR lpszFileName, int nLine )	{	CClientSockE<T>::m_pPool->Free( (CClientSockE<T>*)lpMem );	}
};
...
#ifdef __MASSIVE
template <class T>
MemPooler< CClientSockE<T> >*	CClientSockE<T>::m_pPool	= new MemPooler< CClientSockE<T> >( 1024 );
#else	// __MASSIVE
template <class T>
MemPooler< CClientSockE<T> >*	CClientSockE<T>::m_pPool	= new MemPooler< CClientSockE<T> >( 1 );
#endif	// __MASSIVE
