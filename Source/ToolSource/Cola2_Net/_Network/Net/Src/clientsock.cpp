...
#ifdef MASSIVE
//CMemPool<CClientSock>*	CClientSock::m_pPool	= new CMemPool<CClientSock>( 1024 );
MemPooler<CClientSock>*	CClientSock::m_pPool	= new MemPooler<CClientSock>( 1024 );
#else
//CMemPool<CClientSock>*	CClientSock::m_pPool	= new CMemPool<CClientSock>( 4 );
...
#else	// __CRC
		if( nRemnant < m_pRecvBuffer->GetHeaderLength() )
#endif	// __CRC
		{
...
#ifdef __SO1014
				if( uPacketSize > MAX_BUFFER )
				{
					WriteError( "FETCH//100//%d", uPacketSize );
					WSASetLastError( ERROR_BAD_NET_NAME );
					return NULL;
				}
#endif	// __SO1014
