...
BOOL CDPSock::CreateIoWorker( u_long uIoWorker )
{
	...
	for( int iIoWorker = 0; iIoWorker < uIoWorker; iIoWorker++ )
	{
...
BOOL CDPSock::CloseIoWorker( void )
{
	...
	{
		for( int iIoWorker = 0; iIoWorker < m_uIoWorker; iIoWorker++ )
...
#ifdef __CRC
BOOL CDPSock::JoinToServer( DWORD dwcrc )
#else	// __CRC
...
#endif	// __CRC
{
	...
	{
		if( WaitForSingleObject( m_hProtocolId, 10000 ) == WAIT_TIMEOUT )
		{
			TRACE( "WAIT_TIMEOUT\n" );
#ifdef __SOCKLIB0516
			PostQueuedCompletionStatus( m_phCompletionPort[iIoWorker], (DWORD)-4, (DWORD)pSock->GetHandle(), NULL );
#else	// __SOCKLIB0516
			PostQueuedCompletionStatus( m_hCompletionPort, (DWORD)-4, (DWORD)pSock->GetHandle(), NULL );
#endif	// __SOCKLIB0516
			return FALSE;
		}
	}
...
#ifdef __CRC
BOOL CDPSock::JoinToServerE( DWORD dwcrc )
#else	// __CRC
...
#else	// __SOCKLIB0516
		fOk		= GetQueuedCompletionStatus( pDPSock->GetCompletionPort(), &dwBytes, (LPDWORD)&hSocket, &lpov, INFINITE );
#endif	// __SOCKLIB0516
