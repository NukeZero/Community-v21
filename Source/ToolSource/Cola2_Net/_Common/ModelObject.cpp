...
//#ifndef __VIEWER_ONLY
	...
	{
		if( !m_pBone )
			g_pMainFrm->LoadBone_Auto( (LPTSTR)szFileName, this );		//gmpbigsun : 외부본을 자동으로 읽는다 
...
void	CModelObject::FrameMove( D3DXVECTOR3 *pvSndPos, float fSpeed )
{
	...
	} else
	{
		...
		//gmpbigsun(2010_0513): cola에서 효과음을 출력하자
	if( m_pMotion )
	{
		MOTION_ATTR *pAttr = IsAttrSound();
		if( pAttr )
...
GMOBJECT* CModelObject::GetGMObject( )
{
	CObject3D* pObj3D = GetObject3D( );
	if( pObj3D )
	{
		return pObj3D->GetGMOBJECT( );
	}
