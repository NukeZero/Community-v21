...
LPDIRECT3DTEXTURE9	g_pReflectMap3 = NULL;
...
HRESULT		CreateSkinningVS( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR szFileName )
{
 ...
 //       return hr;
	...
	//if( FAILED( hr = D3DXAssembleShaderFromResource(NULL, MAKEINTRESOURCE(IDR_DATA2), NULL, NULL, 0, &pCode, NULL ) ) )
	//	return hr;
...
: m_AccessLock( "CObject3DMng::m_AccessLock" )
...
//	CMclAutoLock	Lock( m_AccessLock );
	...
	for( i = 0; i < MAX_OBJECT3D_CACHE; i ++ )
	{
		...
		if( pObject3D )
		{
			if( _strcmpi(pObject3D->m_szFileName, szFileName) == 0 && pObject3D->m_pd3dDevice == pd3dDevice )		// 같은걸 찾았으면 그걸 리턴
			{
	...
	if( nIdx == -1 )	
	{
		Message( "%s : 읽을 수 있는 한계를 초과했다", szFileName );
	}
...
void CObject3DMng :: Process( void )
{
	...
	CObject3D	**pAry, **pAry2;
	CObject3D	*pObject3D, *pObject3D2;
	int			i, j;
	
	if( (dwCurrentTime - m_tmTimer) > SEC(10) )	// 10초단위로 체크.
	{
		m_tmTimer = dwCurrentTime;
		pAry = m_pObject3DAry;
		for( i = 0; i < MAX_OBJECT3D; i ++ )		// 메모리매니저 검색
		{
			pObject3D = *pAry++;
			if( pObject3D )				// 객체가 로딩되어 있다.
			{
				if( dwCurrentTime - pObject3D->m_tmCreate > SEC(60) )	// 생성된지 60초가 지나도록 사용되지 않았다면
				{
					m_pObject3DAry[i] = NULL;	// 메모리 매니저에서 삭제
					pAry2 = m_pCache;
					for( j = 0; j < MAX_OBJECT3D_CACHE; j ++ )	// 캐쉬에서도 같은 포인터를 찾아서 삭제
					{
						pObject3D2 = *pAry2++;
						if( pObject3D == pObject3D2 )
						{
							m_pCache[j] = NULL;
							m_nCachePos = j;		// 캐쉬 포인터는 빈곳으로 세팅.
						}
					}
					SAFE_DELETE( pObject3D );	// 실객체 삭제
				}
			}
		}
	}
...
int		CObject3D :: LoadObject( LPCTSTR szFileName )
{
	...
	BOOL bRet;
#ifdef	__COLA
	bRet = resFp.Open( szFileName, "rb" );
#else
	bRet = resFp.Open( MakePath( DIR_MODEL, szFileName ), "rb" );
#endif
	if( bRet == FALSE )	
	{
	...
	if(nVer == 22)
	{
		resFp.Read( &m_vForce3, sizeof(D3DXVECTOR3), 1 );	// 추가 검광3,4
...
//		pGroup->m_pObject = new GMOBJECT[ pGroup->m_nMaxObject ];		// geometry 갯수만큼 할당
...
#if !defined(__YENV)// || defined(__YENV_WITHOUT_BUMP)
	...
	if( bNormalObj == TRUE )
	{
//		_mUpdate = new D3DXMATRIX[ m_nMaxObject ];
...
#else
		pObject->m_pMtrlBlkTexture = new LPDIRECT3DTEXTURE9[ pObject->m_nMaxMtrlBlk * 2 ];	// 확장텍스쳐랑 같이 쓴다. 없으면 걍 널이다.
		memset( pObject->m_pMtrlBlkTexture, 0, sizeof(LPDIRECT3DTEXTURE9) * (pObject->m_nMaxMtrlBlk * 2) );
#endif
			...
			if( bIsMaterial )
			{
				if( mMaterialAry[ pObject->m_pMtrlBlk[i].m_nTextureID ] )
				{
				...
				#endif //__YENV_WITHOUT_BUMP
					{
						...
						strcpy( szTexture, strFileName );
						strcpy( szTexture, ::GetFileTitle( strFileName ) );
						strcat( szTexture, "-n." );
						strcat( szTexture, ::GetFileExt( strFileName ) );
...
//
int		CObject3D::SlideVectorXZ( D3DXVECTOR3 *pOut, const D3DXVECTOR3 &vPos, const D3DXVECTOR3 &vEnd , const D3DXMATRIX &mWorld, BOOL bCollObj )
{
...
//			bRet = D3DXIntersectTri( v1, v2, v3, &vInvPos, &vInvDir, &fU, &fV, &fDist );
			...
			if( bRet && fDist >= 0.0f )	// 반대방향 면은 검사하지 않음.
			{
				...
				if( fDist < 1.0f )
				{
					...
					if( vN.x == 0 && vN.z == 0 )
						Message( "CActionMover::ProcessCollisionGround : 충돌한 면의 노말이 완전 수직이다" );
...
//					D3DXVec3TransformCoord( pIntersect, &vIntersect, &mTM );	// 충돌지점을 mTM적용시켜서 리턴.
					...
					if( D3DXVec3Dot( &vN, &vDir ) < -0.19080f )	// 일정각 이상 기울어진 면만 슬라이드를 적용시킨다. 두벡터다 노말이어야 한다.
						return 1;
					vDir.y = -0.01f;
...
//
D3DXVECTOR3 *CObject3D::IntersecRayVertex( D3DXVECTOR3 *pOut, const D3DXVECTOR3 &vRayOrig, const D3DXVECTOR3 &vRayDir )
{
	int		i, j;
	GMOBJECT	*pObject = m_pGroup->m_pObject;
	int		nMax;
	D3DXVECTOR3	*pVertex, v;
	...
	for( i = 0; i < m_pGroup->m_nMaxObject; i ++ )
	{
		...
		for( j = 0; j < nMax; j ++ )
		{
			...
			{
				D3DXVec3TransformCoord( &v, pVertex, &m_pGroup->_mUpdate[i] );		// v = vertex * WorldTM
				if( IsTouchRayAABB3( vRayOrig, vRayDir, v, D3DXVECTOR3(0.025f, 0.025f, 0.025f)) )
				{
					*pOut = v;
					return pVertex;
				}
			...
			for( j = 0; j < nMax; j++ )
			{
				...
				if( bRet && fDist >= 0.0f )	// 반대방향 면(-)은 검사하지 않음.
				{
					if( fDist < fMinDist )	// 레이원점으로부터 가장 가까운 거리를 찾음.
					{
						...
						bFlag = TRUE;
#ifdef	__COLA
						D3DXVec3TransformCoord( &_vPool[0], v1, &m_Group[0]._mUpdate[i] );
						D3DXVec3TransformCoord( &_vPool[1], v2, &m_Group[0]._mUpdate[i] );
						D3DXVec3TransformCoord( &_vPool[2], v3, &m_Group[0]._mUpdate[i] );
#endif
...
//
BOOL	CObject3D::IsTouchOBB_Line( const D3DXVECTOR3 &vStart, const D3DXVECTOR3 &vEnd, 
...
#else
void	CObject3D::LoadTextureEx( GMOBJECT *pObj, MATERIAL *pmMaterial[16] )
#endif
{
	int		i;
	char	szTexture[ 256 ];
...
#else // ex2
void	CObject3D::SetTextureEx( GMOBJECT *pObj, BOOL bUse )
#endif // not ex2
{
#ifdef __XTEXTURE_EX2
	...
	if( nNumEx >= 8 )
	{
		Message( "CObject3D::SetTextureEx : nNumEx = %d", nNumEx );
		return;
	}
...
//	if( bUse )
	...
	pTextureEx = pObj->m_pMtrlBlkTexture + pObj->m_nMaxMtrlBlk;		// 확장부분 포인터.
	...
	if( bUse )
	{
		...
		if( pTextureEx[0] == NULL )		// 확장 텍스쳐가 로딩된적이 없다.
		{
			LoadTextureEx( pObj, mMaterial );		// 확장텍스쳐를 로딩함.
			for( i = 0; i < pObj->m_nMaxMtrlBlk; i ++ )
			{
				nID = pObj->m_pMtrlBlk[i].m_nTextureID;
				if( mMaterial[ nID ] )
...
void CObject3D::SetState( MATERIAL_BLOCK* pBlock, int nEffect, DWORD dwBlendFactor )
{
	...
	static D3DXVECTOR3 kUV( 0.0f, 0.0f, 0.0f );

	if( nEffect & XE_MTE )
	{
		//gmpbigsun : 0 : alpha map, 1 : animationed uv map
		//effect multi texture ( UV ) 
	...
	//	m_pd3dDevice->SetRenderState( D3DRS_ALPHAREF, 0x20 );
		m_pd3dDevice->SetRenderState( D3DRS_ALPHABLENDENABLE, TRUE );
	    
		m_pd3dDevice->SetTexture( 0, g_pReflectMap );
	
		m_pd3dDevice->SetTexture( 1, g_pReflectMap2 );
//
...
//		m_pd3dDevice->SetTextureStageState( 1, D3DTSS_ALPHAARG2, D3DTA_TFACTOR  );

		kUV += D3DXVECTOR3( 0.002f, 0.002f, 0.002f );
		m_pd3dDevice->SetVertexShaderConstantF( 91, (float*)&kUV, 1 );		//texture uv ( skinning.vsh )

		static D3DXVECTOR3 kUV2( 0.0f, 0.0f, 0.0f );
		static const D3DXVECTOR3 kPositive( 0.01f, 0.01f, 0.01f );
		static const D3DXVECTOR3 kNegative( -0.01f, -0.01f, -0.01f );
		static const D3DXVECTOR3* pDelta = &kPositive;
		...
		//light
		m_pd3dDevice->SetVertexShaderConstantF( 93, (float*)&kUV2, 1 );
		m_pd3dDevice->SetVertexShaderConstantF( 94, (float*)&kUV2, 1 );
	}
...
#else
	if( m_bTextureEx )
		pBlkTexture += pObj->m_nMaxMtrlBlk;
#endif
...
#endif
		if( nEffect != XE_MTE )		//gmpbigsun : Texture Effect를 쓰지 않는경우만 ...
		{
...
//
//
void	CObject3D :: RenderBB( LPDIRECT3DDEVICE9 pd3dDevice, const D3DXMATRIX *mWorld )
{
