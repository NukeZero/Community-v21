...
public:
	...
	void	Create( int nType, FLOAT fFadeSpeed = 0.030f );
	int		CreateTail( const D3DXVECTOR3 &vPos1, const D3DXVECTOR3 &vPos2 );
	void	Clear( void );
	int		GetType( void ) { return m_nType; }
	...
	virtual HRESULT RestoreDeviceObjects( LPDIRECT3DDEVICE9 pd3dDevice );
    virtual HRESULT InvalidateDeviceObjects();
	virtual HRESULT ChangeTexture( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR szFileName, int nType );
...
//extern CTailEffect		g_Tail;		// 테스트용. g_pPlayer만 우선 쓰도록 한다.
#endif // CLIENT

#endif // tailEffectMng.h