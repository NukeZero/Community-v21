...
{ 
	...
	{
		LPCTSTR szErr = Message( "Projection : ActiveMover==NULL 0x%08x", (int)g_pPlayer );
		ADDERRORMSG( szErr );
	}
...
#ifdef __XUZHU
//#ifdef _DEBUG	
//	RenderShadowMapInfo( m_pd3dDevice );		// 
//#ifdef __XGLARE
//	if( g_Option.m_nBloom )
//		g_Glare.RenderGlareEffect( m_pd3dDevice );
//#endif
//#endif
#endif
...
//							m_pd3dDevice->SetRenderState( D3DRS_RANGEFOGENABLE, TRUE );
			...
			if( g_Option.m_nShadow == 2 )
			{
				...
				//				D3DXMatrixTranslation( &mWorldShadow, GetPos().x, m_pActMover->m_fCurrentHeight, GetPos().z );
				g_Shadow.SetGroup(0);
				float bias = -0.0001f;
...
//				pd3dDevice->SetRenderState(D3DRS_DEPTHBIAS, 1 );
				...
				for( i = 0; i < m_nObjCullSize; i++)
				{
					...
					//////////////////////////////////////////
						...
						{
							pMoverShadow->GetWorld()->GetLandTri( pMoverShadow->GetPos().x, pMoverShadow->GetPos().z, vec3Tri );
							FLOAT ffHeight = pMoverShadow->GetWorld()->GetLandHeight( pMoverShadow->GetPos().x, pMoverShadow->GetPos().z );
							...
							v3Pos = pMoverShadow->GetPos();
							v3Pos.y = ffHeight+0.1f;
							
							D3DXMatrixRotationQuaternion( &mRot, &qDirMap);
						}

						D3DXMatrixTranslation( &mWorldShadow, pMoverShadow->GetPos().x, pMoverShadow->m_pActMover->m_fCurrentHeight, pMoverShadow->GetPos().z );
...
//				pd3dDevice->SetRenderState(D3DRS_DEPTHBIAS, 0 );
	...
	if( m_pCamera ) 
	{
		LPWATERHEIGHT lpWaterHeight = GetWaterHeight( m_pCamera->m_vPos.x, m_pCamera->m_vPos.z );
	...
	for( i = 0; i < m_nObjCullSize; i++)
	{
		...
		if( pObj && m_bViewName && pFont )
		{
			if( pObj->GetType() == OT_MOVER )
			{
				...
				if( pObj->m_fDistCamera < fMax ) // && /*((CMover*)pObj)->IsPlayer() && pObj != pObj->GetActiveObj() ) */|| CObj::m_pObjHighlight == pObj )
	#else //__GUILDCOMBAT
...
void CWorld::SetLight( BOOL bLight )
{
	...
	else
	{
		if( pLight )
		{
			...
			{
				0.3f,  0.3f,  0.5f,    0.2f, 0.2f, 0.3f, // 0 12
				0.3f,  0.3f,  0.5f,    0.2f, 0.2f, 0.3f, // 1
				0.3f,  0.3f,  0.5f,    0.2f, 0.2f, 0.3f, // 2
				0.3f,  0.3f,  0.5f,    0.3f, 0.3f, 0.4f, // 3
				0.3f,  0.3f,  0.5f,    0.3f, 0.3f, 0.4f, // 4
				0.3f,  0.4f,  0.5f,    0.4f, 0.4f, 0.4f, // 5
				...
				0.3f,  0.3f,  0.4f,    0.2f, 0.2f, 0.3f, // 19 8
				0.3f,  0.3f,  0.4f,    0.2f, 0.2f, 0.3f, // 20 8
				0.3f,  0.3f,  0.5f,    0.2f, 0.2f, 0.3f, // 21 9
				0.3f,  0.3f,  0.5f,    0.2f, 0.2f, 0.3f, // 22 10
				0.3f,  0.3f,  0.5f,    0.3f, 0.3f, 0.4f  // 23 11
			};
			int nHour = 8, nMin = 0;
	#ifdef __CLIENT
...
HRESULT CWorld::RestoreDeviceObjects( LPDIRECT3DDEVICE9 pd3dDevice )
{
	...
	for( i = 0; i < m_nLandHeight; i++ )
	{
		for(int j = 0; j < m_nLandWidth; j++ )
		{
			...
			if( pLand && pLand->m_texMiniMap.m_pTexture )
			{
				//pLand->m_texMiniMap.CreateTexture( m_pd3dDevice, 256, 256, 0, D3DUSAGE_RENDERTARGET, g_pD3dApp->m_d3dpp.BackBufferFormat, D3DPOOL_DEFAULT );
...
//#ifdef __CLIENT
//			SAFE_RELEASE( m_apLand[ i ]->m_texMiniMap.m_pTexture );
//#endif
		}
	}
	if( m_apLand != NULL )
...
BOOL CWorld::IsPickTerrain( RECT rect, POINT point, D3DXMATRIX* pmatProj, D3DXMATRIX* pmatView )
{
	...
	do 
	{
		...
		if( VecInWorld( vPickRayCur ) )
		{
			...
			if( pLand  && pLand->isVisibile() )
			{
				int tempx = vPickRayCur.x;//( (int)vPickRayCur.x / MPU ) * MPU;//(pLand->m_nWorldX+x*PATCH_SIZE+px)*MPU;
				int tempy = vPickRayCur.z;//( (int)vPickRayCur.z / MPU ) * MPU;//(pLand->m_nWorldY+y*PATCH_SIZE+py)*MPU;
				v1=D3DXVECTOR3( ( tempx       ), GetLandHeight( ( FLOAT ) tempx      , ( FLOAT ) tempy       ), ( tempy       ) );
				v2=D3DXVECTOR3( ( tempx + MPU ), GetLandHeight( ( FLOAT ) tempx + MPU, ( FLOAT ) tempy       ), ( tempy       ) );
				v3=D3DXVECTOR3( ( tempx       ), GetLandHeight( ( FLOAT ) tempx      , ( FLOAT ) tempy + MPU ), ( tempy + MPU ) );
				v4=D3DXVECTOR3( ( tempx + MPU ), GetLandHeight( ( FLOAT ) tempx + MPU, ( FLOAT ) tempy + MPU ), ( tempy + MPU ) );
