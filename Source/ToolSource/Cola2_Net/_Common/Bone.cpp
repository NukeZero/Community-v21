...
: m_AccessLock( "CBonesMng::m_AccessLock" )
...
//	CMclAutoLock	Lock( m_AccessLock );
	...
	for( i = 0; i < MAX_BONES; i ++ )
	{
		...
		{
			if( _strcmpi(pBones->m_szName, szFileName) == 0 )		// 같은걸 찾았으면 그걸 리턴
...
#ifdef __INFOCRITSECT_DEBUG__
: m_AccessLock( "CMotionMng::m_AccessLock" )
#endif	// __INFOCRITSECT_DEBUG__
{
	Init();
}
...
//	CMclAutoLock	Lock( m_AccessLock );
	...
	for( i = 0; i < MAX_MOTION_CACHE; i ++ )
	{
		...
		if( pMotion )
		{
			if( _strcmpi(pMotion->m_szName, szFileName) == 0 )		// 같은걸 찾았으면 그걸 리턴
			{
#ifdef __WORLDSERVER
				m_AccessLock.Leave( theLineFile );	// unlock1
#endif	// __WORLDSERVER
