...
void C2DRender::TextOut_EditString( int x,int y, CEditString& strEditString, int nPos, int nLines, int nLineSpace )
{
	...
	if( m_clipRect.RectLapRect( rect ) )
	{
		...
		y += m_ptOrigin.y;
		int nBegin = x;
		m_pFont->DrawText( x, y, 0, strEditString, nPos, nLines, nLineSpace );
		if( nLines == 0 )
			nLines = strEditString.GetLineCount();
		int nMax = nPos + nLines;
		if( nMax > strEditString.GetLineCount() )
...
//}}AFX
			...
			{ 
				...
				{
					m_pFont->DrawText( x+2, y+1, 0xffcfcfcf, temp, 0, wCodePage );
					m_pFont->DrawText( x + 1, y, dwColor, temp, 0, wCodePage );
				}
				if( dwStyle & ESSTY_BOLD )
#ifdef __THAILAND__
					m_pFont->DrawText( x, y, 0xff1010ff, temp, 0, wCodePage );
#else	// __THAILAND__
					m_pFont->DrawText( x + 1, y, dwColor, temp, 0, wCodePage );
#endif	// __THAILAND__
 				if( dwStyle & ESSTY_UNDERLINE )
	...
	if( m_pFont )
	{
		...
		if( m_clipRect.RectLapRect( rect ) )
		{
			if( dwShadowColor & 0xff000000 )
				m_pFont->DrawText( x + m_ptOrigin.x + 1, y + m_ptOrigin.y + 1, dwShadowColor, (TCHAR*)pszString);
			m_pFont->DrawText( x + m_ptOrigin.x, y + m_ptOrigin.y, dwColor, (TCHAR*)pszString);
		}
	}
}
void C2DRender::TextOut( int x,int y, FLOAT fXScale, FLOAT fYScale, LPCTSTR pszString, DWORD dwColor, DWORD dwShadowColor )
{
	if( m_pFont )
	{
		...
		if( m_clipRect.RectLapRect( rect ) )
		{
			if( dwShadowColor & 0xff000000 )
				m_pFont->DrawText( x + m_ptOrigin.x + 1, y + m_ptOrigin.y + 1, dwShadowColor, (TCHAR*)pszString);
			m_pFont->DrawText( x + m_ptOrigin.x, y + m_ptOrigin.y, fXScale, fYScale, dwColor, (TCHAR*)pszString);
		}
	}
}
void C2DRender::TextOut( int x,int y, int nValue, DWORD dwColor, DWORD dwShadowColor )
{
	if( m_pFont )
	{
		...
		{
			if( dwShadowColor & 0xff000000 )
				m_pFont->DrawText( x + m_ptOrigin.x + 1, y + m_ptOrigin.y + 1, dwShadowColor, (TCHAR*)szString);
			m_pFont->DrawText( x + m_ptOrigin.x, y + m_ptOrigin.y, dwColor, (TCHAR*)szString);
		}
	}
}
...
void CTexturePack::MakeVertex( C2DRender* p2DRender, CPoint point, int nIndex, TEXTUREVERTEX** ppVertices )
{
	...
	FLOAT left   = point.x;
	FLOAT top    = point.y;
	FLOAT right  = point.x + ( /*fScaleX **/ pTexture->m_size.cx );
	FLOAT bottom = point.y + ( /*fScaleY **/ pTexture->m_size.cy );
	...
	(*ppVertices)++;
	SetTextureVertex( *ppVertices, point.x, bottom, pTexture->m_fuLB, pTexture->m_fvLB);
	(*ppVertices)++;
...
void CTexturePack::MakeVertex( C2DRender* p2DRender, CPoint point, int nIndex, TEXTUREVERTEX2** ppVertices, DWORD dwColor )
{
	...
	(*ppVertices)++;
	SetTextureVertex2( *ppVertices, point.x, bottom, pTexture->m_fuLB, pTexture->m_fvLB, dwColor );
	(*ppVertices)++;
...
BOOL CTexturePack::LoadScript( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR pszFileName )
{
	...
	do {
		...
		{	
			scanner.GetToken();	
			TCHAR strFileName[128]; strcpy(strFileName,scanner.token);	
