...
const int	MAX_PROPMOVER =		1000;		// MoverProp배열의 최대갯수 
...
#if defined(__TESTSERVER) || defined(__INTERNALSERVER)
	int *p = NULL;		// 콜스택을 추적하기 위해 죽인다. 테섭에서만.
	*p = 1;
#endif
}
...
public:
	...
	BOOL	AddItem( DWORD dwGiftbox, DWORD dwItem, DWORD dwProbability, int nNum );
	BOOL	Open( DWORD dwGiftbox, LPDWORD pdwItem, int* pnNum );
	void	Verify( void );
};
#endif	// __WORLDSERVER
...
//	Constructions
	...
	int		GenRandomOptItem( int nLevel, FLOAT fPenalty, ItemProp* pItemProp );
	void	Arrange( void );
	void	Free( void );
