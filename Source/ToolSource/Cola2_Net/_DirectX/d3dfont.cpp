...
HRESULT CD3DFontAPI::GetTextExtent( const TCHAR* strText, SIZE* pSize )
{
	...
	int nIndex  = 0;
	int nMaxLen = 0;
	for( unsigned int i=0; i<OffsetIndex.size(); i++ )
	{
                            ...
                            const TCHAR* strText, DWORD dwFlags )
{
	...
	if( m_nOutLine )
	{
		...
		SetRect( &rc, iSx+m_nOutLine, iSy+m_nOutLine, 0, 0 );        
		...
		SetRect( &rc, iSx+m_nOutLine, iSy-m_nOutLine, 0, 0 );        
		m_pFont->DrawText( m_pD3DXSprite, strText, -1, &rc, DT_NOCLIP, D3DXCOLOR( r_b,g_b,b_b,a ) ); 
		SetRect( &rc, iSx+m_nOutLine, iSy+0, 0, 0 );        
		m_pFont->DrawText( m_pD3DXSprite, strText, -1, &rc, DT_NOCLIP, D3DXCOLOR( r_b,g_b,b_b,a ) ); 
		SetRect( &rc, iSx-m_nOutLine, iSy+0, 0, 0 );        
		m_pFont->DrawText( m_pD3DXSprite, strText, -1, &rc, DT_NOCLIP, D3DXCOLOR( r_b,g_b,b_b,a ) ); 
		SetRect( &rc, iSx+0, iSy-m_nOutLine, 0, 0 );        
		m_pFont->DrawText( m_pD3DXSprite, strText, -1, &rc, DT_NOCLIP, D3DXCOLOR( r_b,g_b,b_b,a ) ); 
		SetRect( &rc, iSx-0, iSy+m_nOutLine, 0, 0 );        
		m_pFont->DrawText( m_pD3DXSprite, strText, -1, &rc, DT_NOCLIP, D3DXCOLOR( r_b,g_b,b_b,a ) ); 

		SetRect( &rc, iSx, iSy, 0, 0 );        
...
LPDIRECT3DTEXTURE9 CD3DFont::CreateTexture()
{
	...
	for( int i = 0; i < nLength; i++ )
		*pDstRow++ = static_cast<WORD>(m_dwBgColor);
...
void CD3DFont::MakeOutLine( int nWidth, WORD* pDst16 )
{
	...
	for( int y = 0; y < m_sizeBitmap.cy; y++ )
	{
		...
		for( int x = 0; x < m_sizeBitmap.cx; x++ )
		{
			...
			{
				for( int y2 = (int)y + nCurY - m_nOutLine, y3 = 0; y2 <= ( y + nCurY + (int)m_nOutLine ); y2++, y3++ )
				{
					for( int x2 = (int)x + nCurX - m_nOutLine, x3 = 0; x2 <= ( x + nCurX + (int)m_nOutLine ); x2++, x3++ )
					{
						if( x2 >= 0 && x2 < nWidth && y2 >= 0 && y2 < nWidth )
						{
							...
							if( ( pDst16[ nOffset] & 0xf000 ) != 0xf000 )
							{
								...
								if( dwColor )
								{
									if( ( pDst16[ nOffset] & 0xf000 ) < (WORD)dwColor )///&& ( pDst16[ nOffset] & 0xf000 ) != 0xf000 )
										pDst16[ nOffset] = ( pDst16[ nOffset] & 0x0fff ) | (WORD)dwColor;
								}
							}
						}
					}
