	...
	while(1)
	{
		...
		if( script.tokenType == NUMBER )
		{
			...
			if( nCommand == AICMD_SCAN )		// scan명령어가 나왔었다.
			{
				...
				pProp->m_nScanJob = nNum;				// JOB_xxx가 들어감. MAX_JOB이면 모든 유저를 스캔
			}
		}
		if( nCommand == AICMD_SCAN )
		{
			if( script.tokenType == IDENTIFIER )	// scan명령뒤에 뭔가가 잘못됐다.
			{
				SYNTAX_ERROR;
				return FALSE;
			}
...
//			} else
		...
		if( script.tokenType == NUMBER )
		{
			...
			{
				int nJob = MAX_JOB;
				if( script.m_strDefine.IsEmpty() == FALSE )	// 파라메터 읽은게 define이었다.
				{
					nJob = atoi( script.token );			// 이 파라메터는 직업.
					script.GetToken();						// 뒤에 레인지 파라메터 더읽음.
				}
...
BOOL CProject::LoadPropMoverEx_AI( LPCTSTR szFileName, CScript &script, int nVal )
{
	...
	if( script.token[0] != '{' )
	{
		Message( "%s(%d) : MoverID=%d AI 다음에 { 없음.", szFileName, script.GetLineNum(), nVal );
		return FALSE;
	}
