...
#ifdef __Y_PATROL
TCHAR szRoot[ MAX_OBJARRAY ][32] = { "obj", "ani", "ctrl", "sfx", "item", "mvr", "region","obj","region" };
#else //__Y_PATROL
...
int		CModelMng::GetModelElemCount( DWORD dwType )
{
	int		nSize = 0;
	
	if( dwType < MAX_OBJTYPE )
	{
		nSize	= m_aaModelElem[ dwType ].GetSize();
	}
...
void CModelMng::MakeMotionName( TCHAR* pszMotionName, DWORD dwType, DWORD dwIndex, DWORD dwMotion )
{
	...
	if( (int)( dwMotion ) >= lpModelElem->m_nMax )
		dwMotion = MTI_STAND;
...
BOOL CModelMng::AddModel( DWORD dwType, LPMODELELEM pModelElem )
{
	...
	{
		if( apModelElem->GetAt( i ) && _stricmp( ((LPMODELELEM)apModelElem->GetAt( i ))->m_szName, pModelElem->m_szName ) == 0 )
		{
			return FALSE;
		}		
	}
