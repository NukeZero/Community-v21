...
int CScript::GetToken( BOOL bComma )
{
	...
	m_strKey	= "";
//	BOOL bStrId	= Token.Find( "IDS", 0 ) == 0;
//	BOOL bFound	= FALSE;
	string str;
...
//	BOOL bStrFile	= strFile.Find( ".txt.txt", 0 ) > 0;
//	if( bStrId && !bFound && !bStrFile )
//	{
//	}
...
{
#ifdef _DEBUG
	return;			//gmpbigsun : too long time
#endif;
	...
	while( tok != FINISHED )
	{
		...
		if( str.Find( "IDS", 0 ) != 0 )
		{
			Error( "LoadString: %s", m_szFileName );
			GetToken();
			continue;
		}
		GetLastFull();
