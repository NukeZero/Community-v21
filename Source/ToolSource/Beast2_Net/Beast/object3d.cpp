...
//		for( i = 0; i < 16; i ++ )	mMaterialAry[i] = NULL;
		...
		for( i = 0; i < pObject->m_nMaxMaterial; i ++ )
		{
			...
			if( IsEmpty(szBitmap) == FALSE )
				mMaterialAry[i] = g_TextureMng.AddMaterial( m_pd3dDevice, &mMaterial, szBitmap, pObject );
		}
	}
		...
		for( i = 0; i < pObject->m_nMaxMtrlBlk; i ++ )
		{
			if( bIsMaterial )
			{
				if( mMaterialAry[ pObject->m_pMtrlBlk[i].m_nTextureID ] )
				{
					...
					std::string strCurrentFileName = pObject->m_MaterialAry[ pObject->m_pMtrlBlk[i].m_nTextureID ].strBitMapFileName;

					//@@ FIXME [ YT ( 2012 / 10 / 29 ) ] : 여기에 두면 스키닝 오브젝트 텍스쳐가 여기로 안들어옴.
					// 어디선가 하는것 같은데........
					//g_TextureMng.Add_Link_GmObject( strCurrentFileName.c_str(), pObject, pObject->m_pMtrlBlk[i].m_nTextureID );
					__asm nop
				}
//				{
//					// 이럴땐 컨버트.
		...
		for( i = 0; i < m_Group[k].m_nMaxObject; i ++ )
		{
			...
			for( j = 0; j < pObject->m_nMaxMtrlBlk; j ++ )		// 매터리얼 블럭을 돌면서
			{
				...
				if( strcmp( szBitMapFileName[j], szBuff ) == 0 )	// szSrc랑 같은 파일명이 있으면
				{
					MATERIAL	*pMtrl;
					D3DMATERIAL9	mMtrl;
					pMtrl = g_TextureMng.AddMaterial( m_pd3dDevice, &mMtrl, szDest, pObject );		// szDest로 읽어서
...
static D3DXVECTOR3	_vPool[5120];				//fixed by gmpbigsun
...
void	CObject3D::SetTexture( LPCTSTR szTexture )
{
	...
	pMtrl = g_TextureMng.AddMaterial( m_pd3dDevice, &mMtrl, szTexture, &m_Group[0].m_pObject[0] );
...
//	char	szNum[16];
	...
	for( i = 0; i < pObj->m_nMaxMaterial; i ++ )
	{
		...
		if( IsEmpty(szTexture) == FALSE )
			pmMaterial[i] = g_TextureMng.AddMaterial( m_pd3dDevice, &mMaterial, szTexture, pObj );
	}
}
