...
// 툴전용, 컬링적용 안함. No landscape version
FLOAT CWorld::IntersectRayTerrain3( const D3DXVECTOR3 &vPickRayOrig, const D3DXVECTOR3 &vPickRayDir )
{
	...
	do 
	{
		...
		for( int z = -MPU; z <= MPU; z += MPU )
		{
			for( int x = -MPU; x <= MPU; x += MPU )
			{
				...
				else if( IntersectTriangle( v2, v4, v3, vPickRayOrig, vPickRayDir, &vecIntersect, &fDist ) ) 
				{
					if( fDist < fNearDist ) {	fNearDist = fDist; bTriangle1 = TRUE; }
				}
			}
