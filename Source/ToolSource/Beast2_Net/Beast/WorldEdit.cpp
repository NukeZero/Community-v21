...
void CWorld::RenderBoundBox()
{
	...
	for( int i = 0; i < nSize; i++)
	{
		...
		g_pMainFrame->m_dlgPatrol.RenderPath( pObj );		
	}
...
void CWorld::RenderAxis()
{
	if(m_bViewAxis)
	{
		...
		matTrans = m_pCamera->m_matInvView * matView * matProj;
		D3DXVec3TransformCoord( &vecX, &vecX, &matTrans);
		D3DXVec3TransformCoord( &vecY, &vecY, &matTrans);
		D3DXVec3TransformCoord( &vecZ, &vecZ, &matTrans);

		g_2DRender.TextOut( (int)( ((-1.0f - vecX.x) / (-2.0f) * viewPort.Width) + 6 ), (int)( ((-1.0f + vecX.y) / (-2.0f) * viewPort.Height) - 6 ), "X", 0xffff0000 );
		g_2DRender.TextOut( (int)( ((-1.0f - vecY.x) / (-2.0f) * viewPort.Width) - 6 ), (int)( ((-1.0f + vecY.y) / (-2.0f) * viewPort.Height) - 6 ), "Y", 0xff0000ff );
		g_2DRender.TextOut( (int)( ((-1.0f - vecZ.x) / (-2.0f) * viewPort.Width) - 3 ), (int)( ((-1.0f + vecZ.y) / (-2.0f) * viewPort.Height) - 12 ), "Z", 0xff00ff00 );
...
void CWorld::RenderGizmo()
{
	...
	float dRange = sqrt( vInverval.x * vInverval.x + vInverval.y * vInverval.y + vInverval.z * vInverval.z ) / 15.0f;
...
{
	//gmpbigsun
	//for( ObjItor iter = m_arySelect.begin(); iter != m_arySelect.end(); ++iter )
	//	(*iter)->m_bSnaped = FALSE;

	if( IsValidSelectObj( pObj ) == FALSE )
		return;

	ObjItor i = m_arySelect.begin();
	for( ; i != m_arySelect.end(); i++ )
	{
		if( *i == pObj )
			break;
	}
...
void CWorld::UpdateCheckSnap( )
{
	//gmpbigsun : 선택한 오브젝트가 충돌했는지 여부를 판단해서 CObj::m_bSnaped flag를 갱신한다.
	CObj* pCurObj = NULL;
	CObj* pNextObj = NULL;
	for( ObjItor iter = m_arySelect.begin(); iter != m_arySelect.end(); )
	{
		pCurObj = *iter;
		++iter;
		if( iter == m_arySelect.end() )
			break;
		
		pNextObj = *iter;
		pCurObj->UpdateBoundBox( );
		pNextObj->UpdateBoundBox( );

		BOOL bColl = TestOBBIntersect( &pCurObj->m_OBB, &pNextObj->m_OBB );
		if( bColl )
		{
			pCurObj->m_bSnaped = true;
			pNextObj->m_bSnaped = true;
		}
		

	//	DoSnap( pCurObj, pNextObj );
	}
}

void CWorld::DoSnap( CObj* pTarget, CObj* pSrc )
{
// 	CModel* pTargetModel = pTarget->m_pModel;
// 	CModel* pSrcModel = pSrc->m_pModel;
// 
// 	D3DXVECTOR3 kTarPos = pTarget->GetPos( );
// 	D3DXVECTOR3 kSrcPos = pSrc->GetPos( );
// 	
// 	D3DXVECTOR3  kDir = ( kTarPos + pTargetModel->m_vMin ) - ( kSrcPos - pSrcModel->m_vMin );
// 	
// 	kSrcPos += kDir ;
// 
// 	pSrc->SetPos( kSrcPos );
...
{
	for( int i = 0; i < (int)( m_arySelect.size() ); i++ )
	{
		CObj* pObj = (CObj*)m_arySelect.at( i );	
		if( pObj->GetType() == OT_REGION )
			((CRegion*)pObj)->m_bEditable = FALSE;
...
//#ifdef __NEWPROCESS
		...
		if( pObj->m_ppViewPtr )
		{
			...
			{
				CPatrolPath::GetInstance()->DeletePatrolPath( (CPathObj*)pObj );
			}
	#endif //__Y_PATROL
...
void CWorld::InvisibleCollisonSelectObj( )
{
	CObj* pObj = NULL;
	for( int i = 0; i < (int)( m_arySelect.size() ); i++)
	{
		pObj = m_arySelect[ i ];
		pObj->SetInvisibleCollision( !pObj->IsInvisibleCollision( ) );
	}
}
...
void CWorld::CopySelectObj()
{
	...
	UnselectObj();
	for( int i = 0; i < arySelect.GetSize(); i++)
	{
...
#ifdef __BEAST
#ifdef __CSC_UNDO_REDO
	CCommandMgrBeast::GetInstance()->m_nLandScape = lx + lz * m_nLandWidth;
#endif //__CSC_UNDO_REDO
#endif //__BEAST
...
//	SetCol( x , z, color );
	...
	for(int i = (int)( -fRadius ); i < fRadius + 1; i++ )  
	{
		for(int j = (int)( -fRadius ); j < fRadius + 1; j++ ) 
		{
			...
			if( distance <= fRadius ) 
			{
				...
				else
				{
					...
					// 원래 칼라를 얻는다.
					DWORD dwSrcColor = GetCol( (int)( x + j ), (int)( z + i ) );
					...
					SetCol( (int)( x + j ), (int)( z + i ), D3DCOLOR_ARGB( 0, dwRed + 128, dwGreen + 128, dwBlue + 128 ) );
				}
			}
		}
	}
...
void CWorld::SetWaterHeight(int x, int z, DWORD dwWaterHeight, DWORD dwWaterTexture )
{
...
#ifndef __NewWater
	int tx; int tz;
	if (m_apLand[mX + mZ * m_nLandWidth]->usingNewWater)
	{
		tx = (x % MAP_SIZE);
		tz = (z % MAP_SIZE);
	}
	...
	{
#endif
		tx = (x % MAP_SIZE) / PATCH_SIZE;
		tz = (z % MAP_SIZE) / PATCH_SIZE;
	}

#ifdef __BEAST
#ifdef __CSC_UNDO_REDO
	CCommandMgrBeast::GetInstance()->m_nLandScape = mX + mZ * m_nLandWidth;
#endif //__CSC_UNDO_REDO
#endif //__BEAST
...
void CWorld::SetHeightAttribute( int x, int z, DWORD dwAttr, FLOAT fRadius )
{
	...
	for( int yc = (int)( -fRadius ); yc < fRadius; yc += MPU ) 
	{
		for( int xc = (int)( -fRadius ); xc < fRadius; xc += MPU )
		{
