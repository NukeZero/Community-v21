...
public:
static	\
MemPooler< CClientSock<T> >*	m_pPool;
public:
	void*	operator new( size_t nSize )	{	return CClientSock<T>::m_pPool->Alloc();	}
	void*	operator new( size_t nSize, LPCSTR lpszFileName, int nLine )	{	return CClientSock<T>::m_pPool->Alloc();	}
	void	operator delete( void* lpMem )	{	CClientSock<T>::m_pPool->Free( (CClientSock<T>*)lpMem );	}
	void	operator delete( void* lpMem, LPCSTR lpszFileName, int nLine )	{	CClientSock<T>::m_pPool->Free( (CClientSock<T>*)lpMem );	}
};
...
#ifdef __MASSIVE
template <class T>
MemPooler< CClientSock<T> >*	CClientSock<T>::m_pPool		= new MemPooler< CClientSock<T> >( 1024 );
#else	// __MASSIVE
template <class T>
MemPooler< CClientSock<T> >*	CClientSock<T>::m_pPool		= new MemPooler< CClientSock<T> >( 1 );
#endif	// __MASSIVE
