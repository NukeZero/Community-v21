...
void	CActionMover::ProcessState1( CMover* pMover,  DWORD dwState, int nParam )
{
	...
	case OBJSTA_STAND:
...

		{
			...
			{
				// SetMotion 자체에서 동작을 수정하므로 전투/비전투 모두 구분이 필요 없음
				//if( m_dwStateFlag & OBJSTAF_COMBAT )	// 전투모드에 따라서 다르게
				if( pModel->IsEndFrame() )
				{
#ifdef __S0504_FLY
...
//		if( m_dwState != OBJSTA_FMOVE )		break;		// 전진중일때만 동작 나옴
	...
	case OBJSTA_COLLECT:
		if( pMover->IsNPC() )
		{
			Message( "OBJSTA_COLLECT : 웬 몬스터가 채집으로 들어왔다 %s", pMover->GetName() );
			break;
		}
 #ifdef __WORLDSERVER
		// 서버에선 CUser::OnActCollecting()으로 들어간다.
	...
	// 점프-발구름
	case OBJSTA_SJUMP1:
//		m_vDelta.x = m_vDelta.z = 0;	// 발구름 중엔 좌표 이동 없음
//		if( pModel->IsEndFrame() )		// 발구름이 다 끝나면 상승동작으로 바꿈
		{
			SetJumpState( OBJSTA_SJUMP2 );
#ifdef __XJUMP0530
			m_vDelta.y = 0.2f + (pMover->GetParam( DST_JUMPING, 0 ) / 1000.0f);
#else
			m_vDelta.y = 0.2f; //m_vDelta.y = m_fJumpPower;
#endif
	...
	case OBJSTA_SJUMP2:
		...
		if( m_vDelta.y <= 0 )		// 아래로 떨어지기 시작했으면 하강으로 전환
		{
			...
			pMover->SetMotion( MTI_JUMP3, ANILOOP_CONT );
			pModel->SetMotionBlending( FALSE );
		}
		break;
	// 점프-하강
	case OBJSTA_SJUMP3:
		if( (GetState() & OBJSTA_DMG_ALL) == 0 )			// 데미지중이 아닐때만 하강동작 변환.
		{
...
//				pModel->SetMotionBlending( TRUE );		// 착지 동작은 블렌딩을 하지 않는게 자연스럽다.
			}
		}
		break;
	// 점프-착지
	...
	case OBJSTA_SJUMP4:
//		m_vDelta.x = m_vDelta.z = 0;	// 착지중엔 좌표 이동 없음
	...
	case OBJSTA_DMG_FLY:
		if( m_bGround )
		{
//			if( GetState() & OBJSTA_SJUMP4 )	// 처음 뜰때도 m_bGround가 1이기땜에 하강하고 있을때 바닥에 닿은것만 유효!
...
#ifdef __CLIENT
				pMover->ClearDestObj();		// 클라가 먼저 일어나서 걸어오는걸 방지하기 위해서 지워줌.
#endif
			} else
			{
				SetState( OBJSTA_DMG_ALL, OBJSTA_DEAD );		// HP가 없으면 바로 죽음.
//				pMover->SetMotion( MTI_DMGDIE, ANILOOP_CONT );		// 털썩! 떨어진후 죽는 모습.
			}
		}
		break;
	...
	case OBJSTA_DMG_LIVE:
		if( pModel->IsEndFrame() )
		{
			...
			ResetState( OBJSTA_ATK_ALL );	// 프레임 끝나면 클리어.
		}
		break;
			...
			{				
				if( pMover->m_nResource == 0 )		// 자원이 다 떨어졌냐
				{
					if( m_nDeadCnt > 10800 - 200 )	// 나타난지 얼마 안된건데 벌써 자원이 떨어졌다면 뭔가 잘못된거다.
						Message( "OBJSTA_DEAD:%d %s", m_nDeadCnt, pMover->GetName() );
...
//		m_vDelta.y = -0.01f;		// 서서히 땅속으로 꺼짐. - 필요없는 코드라 지웠음.
#ifndef __ONLINE
		if( m_nCount++ > SEC1 )
		{
			pMover->Delete();
	...
	else
	{
		if( pModel->IsAttrHit() )	// 현재 프레임에 타격속성이 있는가?
		{
			if( nSkill == SI_BLD_DOUBLESW_SILENTSTRIKE )	// 싸이런 스트라이크는 예외다.
			{
				...
				if( nSkill == SI_BIL_KNU_BELIALSMESHING )	// 베리알 스매싱도 앞으로 전진.
			{
				D3DXVECTOR3 vForce;
			...
			m_nMotionHitCount++;
#else //__YHITCOUNT
			pMover->OnMeleeSkill( 0 );	// 무버의 핸들러를 호출.
#endif //__YHITCOUNT
...
void	CActionMover::ProcessStateMagicCasting( DWORD dwState, int nParam )
{
	...
	if( dwState == OBJSTA_ATK_CASTING2 )	// 시전시간 딜레이중
	{
//		if( pMover->IsFinishCastingTime(m_nCount++) )		// 캐스팅 시간이 끝났다면..
...
//			pMover->m_nAtkCnt = 1;		// 카운트 시작.
	...
	//if( pModel->m_SparkInfo.m_bUsed == FALSE )
	{
...
//	float fTheta = D3DXToRadian( pMover->GetAngle() );
	...
	TRACE( "M%3.2f ", pModel->m_fFrameCurrent );
...
//#endif
	...
	{
		TRACE( "MagicSkill Hit, " );
#ifdef __YMOTIONHITCOUNT
		pMover->OnMagicSkill( 0, m_nMotionHitCount );	// 무버의 핸들러를 호출.
		m_nMotionHitCount++;
#else //__YHITCOUNT
		pMover->OnMagicSkill( 0, 0 );	// 무버의 핸들러를 호출.
#endif //__YHITCOUNT
...
//#ifdef __CLIENT
		...
		int		nSkill = pMover->GetActParam(0);
		OBJID	idTarget = pMover->GetActParam(1);
//		int		nSkillIdx = pMover->GetActParam(2);
		DWORD	dwLevel = pMover->GetActParam(3);
...
//		if( nSkillIdx >= 0 )
//		{
//			pSkill = pMover->GetSkill( 0, nSkillIdx );
...
//#endif
				...
				{	// 타 플레이어는 페이크아이템이므로 여기서에서 값을 빼온다.
					...
					nOption = pMover->m_anEquipOption[ PARTS_RWEAPON ] & 0xFF;
...
#else	// xEffect1006
					pParticles = g_ParticleMng.CreateParticle( 0, vPos, vVel, CMover::GetActiveMover()->GetPos().y );
			...
			if( --pModel->m_nPause == 0 )		// 포즈카운트가 다 끝나면 뒤로 밀리기 시작
			{
				if( pHitObj )
		...
		{
	#if defined(__ACROBAT_0504) && defined(__CLIENT)
			ItemProp* pItemProp = pMover->GetActiveHandItemProp();
...
#else	// !__ACROBAT_0504
			pMover->OnAttackRange();		// 무버의 원거리공격 핸들러를 호출.
#endif	// !__ACROBAT_0504 
		}
		break;
	}
...
ContinueAttack( )
{
	...
	{
		// 직업레벨 X이상이면 다음연타가능
		if( (pMover->GetLevel() >= nReqLevel) && 
			((m_dwCtrlMsg & CTRLMSG_LDOWN) || ((CWndWorld *)g_WndMng.m_pWndWorld)->m_bAutoAttack) )	
		{
			CMover* pHitObj	= prj.GetMover( m_objidHit );
			if( IsValidObj( (CObj*)pHitObj ) &&		// 다음 스킬이 예약되어있으면 여기서 끝낸다.
				g_WndMng.m_pWndWorld->GetNextSkill() == NEXTSKILL_NONE ) 
