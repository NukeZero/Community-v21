...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#ifdef __WORLDSERVER
MemPooler<CAIClockWorks>*	CAIClockWorks::m_pPool		= new MemPooler<CAIClockWorks>( 8 );
#else	// __WORLDSERVER
 #ifndef __ONLINE
 MemPooler<CAIClockWorks>*	CAIClockWorks::m_pPool		= new MemPooler<CAIClockWorks>( 4 );
 #else	// not Online(Single)
 MemPooler<CAIClockWorks>*	CAIClockWorks::m_pPool		= NULL;	//new MemPooler<CAIClockWorks>( 4 );
 #endif // onLine
...
CAIClockWorks::CAIClockWorks( CObj* pObj ) : CAIInterface( pObj, 0 )
{
	Init();
}
...
void CAIClockWorks::InitAI()
{
	...
	SendAIMsg( AIMSG_SETSTATE, STATE_RAGE, NULL_ID );	// 최초 대기상태로 초기화.
...
BOOL	CAIClockWorks::SelectTarget( void )
{
	...
	{
		CMover* pTarget = ScanTarget( pMover, 80 );		// 80미터 반경내에서 아무나 고른다.
		if( pTarget )
		{
...
#ifdef __DEV
//		Message( "보스의 방어도가 증가되었음. 엔터를 치시고 계속 즐겜을 하시오" );
#endif
	}
...
#else	// __POSTAIMSG1008
			SendAIMsg( AIMSG_END_APPEAR );		// 등장씬 애니메이션이 끝났음을 알림.
#endif	// __POSTAIMSG1008
...
//	if( m_nEvent == EPT_DEFENSE )		// 철벽방어상태.
//	{
//	} else
	//------------- 공격중이 아님. --------------------
	{
		...
		if( timeGetTime() >= m_tmReattack + (2000 - m_tmAddReattack) )		// 리어택 시간이 됐는가?  기본적으로 5초간격에 AddReattack으로 줄어들 수 있음.
		{
			...
			{
//				Message( "CAIClockWorks::MoveProcessRage : 타겟이 없다" );
				return FALSE;
			}
...
#endif	// __POSTAIMSG1008
{
...
#else	// __POSTAIMSG1008
		m_idLastAttacker = m_AIMsg.dwParam1;
#endif	// __POSTAIMSG1008
	...
	OnMessage( AIMSG_ARRIVAL )
	{
		...
		default:
			Message( "CAIClockWorks::StateRage - AIMSG_ARRIVAL m_nAttackType=%d", m_nAttackType );
		}
...
//			if( 타점 이후 부터 )
		...
		case CAT_QUAKE:		// 밟는 공격 타점.
			...
			break;
		default:
			Message( "CAIClockWorks::StateRage : AIMSG_ATTACK_MELEE %d", m_nAttackType );
			break;
		}
