                            ...
                            const TCHAR* strText, DWORD dwFlags )
{
	...
	if( m_nOutLine )
	{
...
#endif //__V050322_CACHEQUEST
		SetRect( &rc, sx+m_nOutLine, sy+m_nOutLine, 0, 0 );        
		...
		SetRect( &rc, sx+m_nOutLine, sy-m_nOutLine, 0, 0 );        
		m_pFont->DrawText( m_pD3DXSprite, strText, -1, &rc, DT_NOCLIP, D3DXCOLOR( r_b,g_b,b_b,a ) ); 
		SetRect( &rc, sx+m_nOutLine, sy+0, 0, 0 );        
		m_pFont->DrawText( m_pD3DXSprite, strText, -1, &rc, DT_NOCLIP, D3DXCOLOR( r_b,g_b,b_b,a ) ); 
		SetRect( &rc, sx-m_nOutLine, sy+0, 0, 0 );        
		m_pFont->DrawText( m_pD3DXSprite, strText, -1, &rc, DT_NOCLIP, D3DXCOLOR( r_b,g_b,b_b,a ) ); 
		SetRect( &rc, sx+0, sy-m_nOutLine, 0, 0 );        
		m_pFont->DrawText( m_pD3DXSprite, strText, -1, &rc, DT_NOCLIP, D3DXCOLOR( r_b,g_b,b_b,a ) ); 
		SetRect( &rc, sx-0, sy+m_nOutLine, 0, 0 );        
		m_pFont->DrawText( m_pD3DXSprite, strText, -1, &rc, DT_NOCLIP, D3DXCOLOR( r_b,g_b,b_b,a ) ); 
...
void CD3DFont::MakeOutLine( int nWidth, WORD* pDst16 )
{
	...
	for( int y = 0; y < m_sizeBitmap.cy; y++ )
	{
		...
		for( int x = 0; x < m_sizeBitmap.cx; x++ )
		{
			...
			{
				for( int y2 = (int)y + nCurY - m_nOutLine, y3 = 0; y2 <= ( y + nCurY + m_nOutLine ); y2++, y3++ )
				{
					for( int x2 = (int)x + nCurX - m_nOutLine, x3 = 0; x2 <= ( x + nCurX + m_nOutLine ); x2++, x3++ )
					{
						if( x2 >= 0 && x2 < nWidth && y2 >= 0 && y2 < nWidth )
						{
							...
							if( ( pDst16[ nOffset] & 0xf000 ) != 0xf000 )
							{
								...
								if( dwColor )
								{
									if( ( pDst16[ nOffset] & 0xf000 ) < dwColor )///&& ( pDst16[ nOffset] & 0xf000 ) != 0xf000 )
										pDst16[ nOffset] = ( pDst16[ nOffset] & 0x0fff ) | dwColor;
								}
							}
						}
					}
...
FONTTEX* CD3DFont::GetFontTex(const char* begin, const char* end, WORD wCodePage )
{
	...
	size.cy = min(size.cy,  m_dwMaxHeight);
