...
void CGuildWarMng::Result( CGuildWar* pWar, CGuild* pDecl, CGuild* pAcpt, int nType, int nWptDecl, int nWptAcpt )
{
	...
	if( nType < WR_TRUCE )
	{
		...
		int nMaxWinPoint	= ( 10 + pWin->m_nWinPoint * 0.05 ) * 5;
		...
		pWin->m_nWin++;
		nWinPoint	= ( 10 + pLose->m_nWinPoint * 0.02 );
...
#ifdef __WORLDSERVER
BOOL CGuildWarMng::CreateWorker( void )
{
	DWORD dwThreadId;
...
void CGuildWarMng::CloseWorker( void )
{
	CLOSE_THREAD( m_hWorker, m_hCloseWorker );
}

UINT CGuildWarMng::_Worker( LPVOID pParam )
{
	CGuildWarMng* pGuildWarMng	= (CGuildWarMng*)pParam;
	pGuildWarMng->Worker();
	return 0;
}

void CGuildWarMng::Worker( void )
{
