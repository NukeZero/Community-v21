...
CNpcProperty::CNpcProperty()
{
	...
	m_timerScript.Set( MIN( 1 ) + xRandom( SEC( 30 ) ) );
#else
	m_timerScript.Set( MIN( 1 ) + xRandom( SEC( 10 ) ) );
#endif
}
...
{
	m_timerScript.Set( SEC( 15 ) + SEC( xRandom( 10 ) ) );
}
#endif
...
BOOL CNpcProperty::LoadDialog( LPCHARACTER lpCharacter )
{
	...
	m_Dialog.Free();
	TRACE("dialog file: %s\n", lpCharacter );
	m_Dialog.LoadScript( MakePath( DIR_DIALOG, prj.m_nLanguage, szDialog ) );
#endif
	return TRUE;
}
