...
public:
	...
	#ifdef __ONLINE	// interpolation	- not server
	...
	void			SetAIInterface( DWORD dwAIInterface );
	ObjProp*		GetProp();
...
virtual	BOOL		SetIndex( LPDIRECT3DDEVICE9 pd3dDevice, DWORD dwIndex, BOOL bInitProp = FALSE ) { return SetTypeIndex( pd3dDevice, m_dwType, dwIndex, bInitProp ); }
	...
	static void		LockMethod( int nMethod, unsigned int uiLine, const char* rostrFileName );	
	static void		UnlockMethod( unsigned int uiLine, const char* rostrFileName );	

	#ifdef __INFOCRITSECT_DEBUG__
		void		Lock( unsigned int uiLine, const char* rostrFileName )	{	m_AccessLock.Enter( uiLine, rostrFileName );	}
		void		Unlock( unsigned int uiLine, const char* rostrFileName )	{	m_AccessLock.Leave( uiLine, rostrFileName );	}
	...
	#ifdef __INFOCRITSECT_DEBUG__
		void		Lock( unsigned int uiLine, const char* rostrFileName )	  {}
		void		Unlock( unsigned int uiLine, const char* rostrFileName )  {}
	#else	// __INFOCRITSECT_DEBUG__
		void		Lock()			{ }
		void		Unlock()		{ }
	#endif	// __INFOCRITSECT_DEBUG__
...
{
#ifdef __INFOCRITSECT_DEBUG__
		m_csMethod.Enter( uiLine, rostrFileName );
#else	// __INFOCRITSECT_DEBUG__
		m_csMethod.Enter();
#endif	// __INFOCRITSECT_DEBUG__
		m_nMethod	= nMethod;
...
{
#ifdef __INFOCRITSECT_DEBUG__
		m_csMethod.Leave( uiLine, rostrFileName );
#else	// __INFOCRITSECT_DEBUG__
		m_csMethod.Leave();
#endif	// __INFOCRITSECT_DEBUG__
}
...
inline FLOAT GetDegree( D3DXVECTOR3 vDestPos, D3DXVECTOR3 vSrcPos )
{
	...
	FLOAT fRadian = acos( (double)fDot );
