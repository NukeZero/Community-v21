...
void CSnow::Process( FLOAT fElapsedTime )
{
...
//				m_nDirSpeed *= -1;
			do
			{
				...
				//if( vPos.x < 0 || vPos.x > WORLD_SIZE || vPos.z < 0 || vPos.z > WORLD_SIZE )
				//	continue;
				break;
			}
			while( 1 );
...
void CRain::Process( FLOAT fElapsedTime )
{
	D3DXVECTOR3 vPos = GetPos();
	D3DXVECTOR3 vPosCenter = m_pObjCenter->GetPos();
	//vPos.x = vPos2.x;
...
void CFireRain::Process( FLOAT fElapsedTime )
{
	/*
	CPtZ ptz = GetPtz();
	if(ptz.z > 0)
...
{
#if !( defined( __BEAST ) || defined( __WORLDSERVER ) )
	//if(m_nCloud >= 70)
	{
