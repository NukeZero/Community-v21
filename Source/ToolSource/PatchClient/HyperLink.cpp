...
void CHyperLink::PreSubclassWindow() 
{
    ...
    {
        FLASSERT(!m_strURL.IsEmpty());    // Window and URL both NULL. DUH!
...
{
    FLASSERT(nCtlColor == CTLCOLOR_STATIC);
...
LONG CHyperLink::GetRegKey(HKEY key, LPCTSTR subkey, LPTSTR retdata, size_t cchRetData)
{
    HKEY hkey;
    ...
    if (retval == ERROR_SUCCESS) {
        ...
        FLStrcpy(retdata,cchRetData,data);
        RegCloseKey(hkey);
    }
...
HINSTANCE CHyperLink::GotoURL(LPCTSTR url, int showcmd)
{
    ...
    if ((UINT)result <= HINSTANCE_ERROR) {
        ...
        if (GetRegKey(HKEY_CLASSES_ROOT, _T(".htm"), key, _countof(key)) == ERROR_SUCCESS) {
            FLStrcat(key, _countof( key ), _T("\\shell\\open\\command"));

            if (GetRegKey(HKEY_CLASSES_ROOT,key,key,_countof(key)) == ERROR_SUCCESS) {
                TCHAR *pos;
				...
				FLStrcat( key, _countof( key ), _T( " " ) );
				FLStrcat( key, _countof( key ), url );
