...
static void EnableDialog   (BOOL bEnable = TRUE) { m_bEnableDialog = bEnable; } // 다이얼로그를 가능하게 할지 결정한다.
	...
	//void SetGlobal(CUIntArray &globalArray);
	/*
	void SetGlobal1(int nVal) {m_globalArray[0] = nVal; }
	void SetGlobal2(int nVal) {m_globalArray[1] = nVal; }
	void SetGlobal3(int nVal) {m_globalArray[2] = nVal; }
	void setGlobal4(int nVal) {m_globalArray[3] = nVal; }
	*/
