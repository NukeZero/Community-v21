...
void PaintTexture( LPWORD pDest, LPIMAGE pImage, CPoint pt, CSize sizeSurface )
{
	...
	if( pImage->nImgBit == IMGBIT_24 )
	{
		for( int y = pt.y; y < pt.y + sizeTexture.cy; y++, dwOffsetDest += dwLine )
		{
			...
			{
				if( dwOffsetDest >= 0 && (int)dwOffsetDest < nSizeSurface )
				{
...
BOOL GetLangScript( CScanner& scanner, wchar_t wszToken[] )
{
	int nLang = 0;
	...
	if( scanner.Token != ',' )
	{
		AfxMessageBox( "LANG_ .... 다음에 ','을 찾을수 없다" );
		return FALSE;
	}
	
	scanner.GetToken(); // string
	wcscpy( wszToken, scanner.m_wszToken );
	
	scanner.GetToken(); // ,
	if( scanner.Token != ',' && scanner.Token != ')' )
	{
		AfxMessageBox( "문자열 다음에 ','을 찾을수 없다" );
		return FALSE;
	}
