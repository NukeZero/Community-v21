#include "stdafx.h"
#include "FLConfigFileEncryption.h"

//#include "..\..\program\_CommonDefine\FlyffCommonDefine.h"

FLConfigFileEncryption::FLConfigFileEncryption()
{
...
void	FLConfigFileEncryption::SetSourceFile( const TCHAR* pszFileName )
{
	::memset( m_szSourceFileName, 0, sizeof(m_szSourceFileName) );
...
bool	FLConfigFileEncryption::IsSetSourceFile()
{
...
TCHAR*	FLConfigFileEncryption::GetSourceFileName()
{
	return	( m_szSourceFileName );
}

TCHAR*	FLConfigFileEncryption::GetEncryptionFileName()
{
	return	( m_szEncFileName );
}

TCHAR*	FLConfigFileEncryption::GetDecryptionFileName()
{
	return	( m_szDecFileName );
}


DWORD	FLConfigFileEncryption::EncryptionFile()
{
...
DWORD	FLConfigFileEncryption::EncryptionFile( const TCHAR* pszFileName )
{
	...
	FILE* pFileEnc = _tfopen( m_szEncFileName, "w" );
	if( pFileEnc == NULL )
	{
...
DWORD	FLConfigFileEncryption::DecryptionFile()
{
	if( IsSetSourceFile() == true )
	{
...
DWORD	FLConfigFileEncryption::DecryptionFile( const TCHAR* pszFileName )
{
	if( IsSetSourceFile() == false )
	{
		return	( FILE_ENCRYPTION_NOT_INITIALIZE );
...
void	FLConfigFileEncryption::Encrypt( const TCHAR* szIn, TCHAR* szOut )
{
	unsigned char szEncrypt[512];
	::memset( szEncrypt, 0, sizeof( szEncrypt ) );

	for( size_t nCount = 0; nCount < 16; ++nCount )
	{
...
void	FLConfigFileEncryption::Decrypt( const TCHAR* szIn, TCHAR* szOut )
{
