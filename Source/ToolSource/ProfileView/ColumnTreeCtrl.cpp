...
void CCustomTreeChildCtrl::SetItemProgress(CRect rect, int nPercent)
{
	LPPROGRESSENTRY lpProgressEntry = new ProgressEntry;
	lpProgressEntry->m_nPercent = nPercent;

	lpProgressEntry->m_Progress.Create(PBS_SMOOTH | WS_CHILD | WS_VISIBLE, rect, this, 1);
	lpProgressEntry->m_Progress.SetRange(0, 100);
	lpProgressEntry->m_Progress.SetPos(nPercent);
	m_vecProgressEntry.push_back(SpProgressEntry(lpProgressEntry));
}

void CCustomTreeChildCtrl::CleanAll()
{
	this->DeleteAllItems();
	vector<SpProgressEntry>::iterator iter;
	for(iter=m_vecProgressEntry.begin(); iter!=m_vecProgressEntry.end(); iter++)
	{
		SpProgressEntry spProgressEntry = *(iter);
		spProgressEntry->m_Progress.ShowWindow(SW_HIDE);
		spProgressEntry->m_Progress.SetPos(0);
	}
}

void CCustomTreeChildCtrl::SetProgressPercent(int nIndex, int nPercent)
{
	vector<SpProgressEntry>::iterator iter = m_vecProgressEntry.begin();
	iter += nIndex;
	SpProgressEntry spProgressEntry = *(iter);
	spProgressEntry->m_Progress.ShowWindow(SW_SHOW);
	spProgressEntry->m_Progress.SetPos(nPercent);
}
...
void CColumnTreeCtrl::SetItemProgress(HTREEITEM hItem, int nSubItem, int nPercent)
{
	CRect rcItem;
	if(m_Tree.GetItemRect(hItem, &rcItem, FALSE))
	{
		int nStartX = 0;
		int nWidth = 0;
		...
		rcItem.left = nStartX;
		rcItem.right = nStartX + nWidth;

		m_Tree.SetItemProgress(rcItem, nPercent);
	}
}

void CColumnTreeCtrl::SetProgressPercent(int nIndex, int nPercent)
{
	m_Tree.SetProgressPercent(nIndex, nPercent);
}

void CColumnTreeCtrl::CleanAll()
{
	m_Tree.CleanAll();
}
...
BOOL CColumnTreeCtrl::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT *pResult)
{
	...
	if(pHdr->code==HDN_ITEMCHANGINGW && pHdr->idFrom == HeaderID)
	{
		OnHeaderItemChanging(pHdr,pResult);
		return TRUE; // do not forward
	}
	
	if(pHdr->code==HDN_ITEMCHANGEDW && pHdr->idFrom == HeaderID)
	{
	...
	if(pHdr->code==TVN_ITEMEXPANDINGW && pHdr->idFrom == TreeID)
	{
	...
	if(pHdr->code==TVN_ITEMEXPANDEDW && pHdr->idFrom == TreeID)
	{
