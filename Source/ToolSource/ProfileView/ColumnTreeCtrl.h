...
typedef struct ProgressEntry
{
	...
	ProgressEntry() : m_nPercent(0) {}
} PROGRESSENTRY, *LPPROGRESSENTRY;

typedef	boost::shared_ptr<PROGRESSENTRY> SpProgressEntry;
typedef	vector<SpProgressEntry> VecSpProgressEntry;
...
public:
	VecSpProgressEntry m_vecProgressEntry;
...
public:
	...
	void SetItemProgress(CRect rect, int nPercent);
	void SetProgressPercent(int nIndex, int nPercent);
	void CleanAll();
...
public:
	...
	void SetItemProgress(HTREEITEM hItem, int nSubItem, int nPercent);
	void SetProgressPercent(int nIndex, int nPercent);

	HTREEITEM HitTest(CPoint pt, UINT* pFlags=NULL) const;
	HTREEITEM HitTest(CTVHITTESTINFO* pHitTestInfo) const;

	BOOL ShowTreeWindow(int nCmdShow) {return m_Tree.ShowWindow(nCmdShow);}
