	...
	)
{
	...
	{
		FLDUMPMSG( _T( "%s not found" ), pFileName );
		return E_FAIL;
	}
	...
	if( file.Read( pSrcData, nSrcDataSize ) >= 1 )
	{
		...
		if( FAILED( hr ) )
		{
			FLDUMPMSG( _T( "D3DXCreateTextureFromFileInMemoryEx %s %s" ), pFileName, DXGetErrorString9( hr ) );
		}
	}
...
{
	memset( g_szCrashClient, 0, sizeof(g_szCrashClient) );

	FLStrcat( g_szCrashClient, _countof( g_szCrashClient ), "\nPaintTexture - IN" );
	...
	FLStrcat( g_szCrashClient, _countof( g_szCrashClient ), szstrError );
