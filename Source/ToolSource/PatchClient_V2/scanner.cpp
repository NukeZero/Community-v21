...
#pragma	warning( push )
#pragma warning( disable : 4995 )	// warning C4995: 'wsprintf': 이름이 #pragma deprecated로 표시되었습니다.
#include <comdef.h>
#pragma	warning( pop )
...
const char*	CharNextEx(const char* strText, WORD codePage)
{
	...
	else if( __CURRENT_LANG == LANG_VTN )
	{
...
{
	int nLength;
	if( __CURRENT_LANG == LANG_VTN && lpMultiByteStr != NULL && cchMultiByte > 0 )
	{
...
{
	int nLength;
	if( __CURRENT_LANG == LANG_VTN && lpWideCharStr != NULL && cchWideChar > 0 )
	{
		...
		if( *(m_pProg-1) != '"' )
		{
			...
			if( m_bErrorCheck )
			{
				if( ( pszCur - token ) >= MAX_TOKENSTR )
					FLERROR_LOG( PROGRAM_NAME, _T( "line(%d) 파일 %s에서 \"%s\" 스트링 길이가 %d바이트를 초과했음." ), GetLineNum(), m_strFileName, token, MAX_TOKENSTR );
				else
					FLERROR_LOG( PROGRAM_NAME, _T( "line(%d) 파일 %s에서 \"%s\" 스트링이 따옴표로 끝나지 않음." ), GetLineNum(), m_strFileName, token );
			}
		}
		goto EXIT;
	}
