...
CHECK_TYPE CBetaPatchClientApp::CheckSingleInstance()
{
	...
	if( hNeuzMutex != NULL )
	{
	#if __CURRENT_LANG == LANG_JAP || __CURRENT_LANG == LANG_KOR || __CURRENT_LANG == LANG_ENG || __CURRENT_LANG == LANG_THA || __CURRENT_CNTRY == CNTRY_HK ||			__CURRENT_LANG == LANG_FRA || __CURRENT_LANG == LANG_GER || __CURRENT_LANG == LANG_ID
...
void CBetaPatchClientApp::RunClient()
{
	...
	TCHAR szBuffer[2048];
	int nCount = FLSPrintf( szBuffer, _countof( szBuffer ), "Neuz.exe sunkist %s", AfxGetApp()->m_lpCmdLine );
	if( nCount <= 0 )
		return;
