...
public:
	...
	void	Check1()
	{
		...
		m_nStackCnt ++;
		if( m_nMaxStack >= MAX_PROF_STACK )		FLERROR_LOG( PROGRAM_NAME, _T( "CProf : stack overflow" ) );
	...
	void	Check2( LPCTSTR szMsg )
	{
		...
		m_nMaxStack --;
		if( m_nMaxStack < 0 )		FLERROR_LOG( PROGRAM_NAME, _T( "CProf : stack under flow" ) );
...
//		if( IsEmpty(sz) )
	...
	{
		Show2( fFPS );
		FLERROR_LOG( PROGRAM_NAME, _T( "%s" ), m_szResult );
	}
